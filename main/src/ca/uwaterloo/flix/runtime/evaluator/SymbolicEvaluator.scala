  ...
  def eval(exp0: Expression, env0: Environment, lenv0: LabelEnv, enumerator: Enumerator, root: FinalAst.Root)(implicit flix: Flix): Context = {
    ...
    def eval(pc0: PathConstraint, exp0: Expression, env0: Environment, lenv0: LabelEnv, qua0: Quantifiers)(implicit flix: Flix): Context = exp0 match {
      ...
      /**
        * Unary.
        */
      case Expression.Unary(sop, op, exp, _, loc) =>
          ...
          case (pc, qua, v) => op match {
            ...
            case UnaryOperator.Minus => v match {
              ...
              case SymVal.AtomicVar(id, tpe) =>
                val newVar = Symbol.freshVarSym("v", loc)
      ...
      /**
        * Binary.
        */
      case Expression.Binary(sop, op, exp1, exp2, _, loc) =>
      ...
      case Expression.InvokeMethod(method, exp, args, tpe, loc) => throw InternalCompilerException("Not yet supported.")
