...
object Namer extends Phase[WeededAst.Program, NamedAst.Root] {
        ...
        case None =>
          ...
          val tparams = tparams0 match {
            ...
            case TypeParams.Explicit(tps) => tps map {
              case p => NamedAst.TypeParam(p, Type.freshTypeVarXXXDeprecated(), loc)
            }
          }
  ...
  private def typeEnvFromFreeVars(idents: List[Name.Ident])(implicit flix: Flix): Map[String, Type.Var] =
    idents.foldLeft(Map.empty[String, Type.Var]) {
      case (macc, ident) => macc.get(ident.name) match {
        case None => macc + (ident.name -> Type.freshTypeVarXXXDeprecated())
        case Some(tvar) => macc
      }
    }
  ...
  private def visitExp(exp0: WeededAst.Expression, env0: Map[String, Symbol.VarSym], tenv0: Map[String, Type.Var])(implicit flix: Flix): Validation[NamedAst.Expression, NameError] = exp0 match {
    ...
    case WeededAst.Expression.Wild(loc) =>
      NamedAst.Expression.Wild(Type.freshTypeVarXXXDeprecated(), loc).toSuccess
    ...
    case WeededAst.Expression.VarOrDef(name, loc) if name.isUnqualified =>
      ...
      env0.get(name.ident.name) match {
        case None =>
          // Case 1: reference.
          NamedAst.Expression.Def(name, Type.freshTypeVarXXXDeprecated(), loc).toSuccess
        case Some(sym) =>
          // Case 2: variable.
    ...
    case WeededAst.Expression.Hole(name, loc) =>
      val tpe = Type.freshTypeVarXXXDeprecated()
      NamedAst.Expression.Hole(name, tpe, Type.freshTypeVarXXXDeprecated(), loc).toSuccess
    ...
    case WeededAst.Expression.Apply(exp1, exp2, loc) =>
      mapN(visitExp(exp1, env0, tenv0), visitExp(exp2, env0, tenv0)) {
        case (e1, e2) => NamedAst.Expression.Apply(e1, e2, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
      }
      ...
      flatMapN(visitFormalParam(fparam0, tenv0)) {
        case p =>
          ...
          mapN(visitExp(exp, env1, tenv0)) {
            case e => NamedAst.Expression.Lambda(p, e, Type.freshTypeVarXXXDeprecated(), loc)
          }
      }

    case WeededAst.Expression.Unary(op, exp, loc) => visitExp(exp, env0, tenv0) map {
      case e => NamedAst.Expression.Unary(op, e, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
    }

    case WeededAst.Expression.Binary(op, exp1, exp2, loc) =>
      mapN(visitExp(exp1, env0, tenv0), visitExp(exp2, env0, tenv0)) {
        case (e1, e2) => NamedAst.Expression.Binary(op, e1, e2, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
      }
    ...
    case WeededAst.Expression.IfThenElse(exp1, exp2, exp3, loc) =>
      ...
      mapN(e1, e2, e3) {
        NamedAst.Expression.IfThenElse(_, _, _, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
      }
    ...
    case WeededAst.Expression.Stm(exp1, exp2, loc) =>
      ...
      val e2 = visitExp(exp2, env0, tenv0)
      mapN(e1, e2) {
        NamedAst.Expression.Stm(_, _, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
      }
    ...
    case WeededAst.Expression.Let(ident, exp1, exp2, loc) =>
      ...
      mapN(visitExp(exp1, env0, tenv0), visitExp(exp2, env0 + (ident.name -> sym), tenv0)) {
        case (e1, e2) => NamedAst.Expression.Let(sym, e1, e2, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
      }
    ...
    case WeededAst.Expression.LetRec(ident, exp1, exp2, loc) =>
      ...
      mapN(visitExp(exp1, env1, tenv0), visitExp(exp2, env1, tenv0)) {
        case (e1, e2) => NamedAst.Expression.LetRec(sym, e1, e2, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
      }
    ...
    case WeededAst.Expression.Match(exp, rules, loc) =>
      ...
      mapN(expVal, rulesVal) {
        case (e, rs) => NamedAst.Expression.Match(e, rs, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
      }
    ...
    case WeededAst.Expression.Tag(enum, tag, expOpt, loc) => expOpt match {
      case None =>
        ...
        NamedAst.Expression.Tag(enum, tag, None, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc).toSuccess
      case Some(exp) =>
        // Case 2: The tag has an expression. Perform naming on it.
        visitExp(exp, env0, tenv0) map {
          case e => NamedAst.Expression.Tag(enum, tag, Some(e), Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
        }
    }

    case WeededAst.Expression.Tuple(elms, loc) =>
      traverse(elms)(e => visitExp(e, env0, tenv0)) map {
        case es => NamedAst.Expression.Tuple(es, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
      }

    case WeededAst.Expression.RecordEmpty(loc) =>
      NamedAst.Expression.RecordEmpty(Type.freshTypeVarXXXDeprecated(), loc).toSuccess

    case WeededAst.Expression.RecordSelect(exp, label, loc) =>
      mapN(visitExp(exp, env0, tenv0)) {
        case e => NamedAst.Expression.RecordSelect(e, label, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
      }

    case WeededAst.Expression.RecordExtend(label, value, rest, loc) =>
      mapN(visitExp(value, env0, tenv0), visitExp(rest, env0, tenv0)) {
        case (v, r) => NamedAst.Expression.RecordExtend(label, v, r, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
      }

    case WeededAst.Expression.RecordRestrict(label, rest, loc) =>
      mapN(visitExp(rest, env0, tenv0)) {
        case r => NamedAst.Expression.RecordRestrict(label, r, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
      }

    case WeededAst.Expression.ArrayLit(elms, loc) =>
      traverse(elms)(e => visitExp(e, env0, tenv0)) map {
        case es => NamedAst.Expression.ArrayLit(es, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
      }

    case WeededAst.Expression.ArrayNew(elm, len, loc) =>
      mapN(visitExp(elm, env0, tenv0), visitExp(len, env0, tenv0)) {
        case (es, ln) => NamedAst.Expression.ArrayNew(es, ln, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
      }

    case WeededAst.Expression.ArrayLoad(base, index, loc) =>
      mapN(visitExp(base, env0, tenv0), visitExp(index, env0, tenv0)) {
        case (b, i) => NamedAst.Expression.ArrayLoad(b, i, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
      }
    ...
    case WeededAst.Expression.ArrayStore(base, index, elm, loc) =>
      mapN(visitExp(base, env0, tenv0), visitExp(index, env0, tenv0), visitExp(elm, env0, tenv0)) {
        case (b, i, e) => NamedAst.Expression.ArrayStore(b, i, e, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
      }

    case WeededAst.Expression.ArrayLength(base, loc) =>
      visitExp(base, env0, tenv0) map {
        case (b) => NamedAst.Expression.ArrayLength(b, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
      }
    ...
    case WeededAst.Expression.ArraySlice(base, startIndex, endIndex, loc) =>
      mapN(visitExp(base, env0, tenv0), visitExp(startIndex, env0, tenv0), visitExp(endIndex, env0, tenv0)) {
        case (b, i1, i2) => NamedAst.Expression.ArraySlice(b, i1, i2, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
      }

    case WeededAst.Expression.VectorLit(elms, loc) =>
      traverse(elms)(e => visitExp(e, env0, tenv0)) map {
        case es => NamedAst.Expression.VectorLit(es, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
      }

    case WeededAst.Expression.VectorNew(elm, len, loc) =>
      visitExp(elm, env0, tenv0) map {
        case e => NamedAst.Expression.VectorNew(e, len, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
      }

    case WeededAst.Expression.VectorLoad(base, index, loc) =>
      visitExp(base, env0, tenv0) map {
        case b => NamedAst.Expression.VectorLoad(b, index, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
      }

    case WeededAst.Expression.VectorStore(base, index, elm, loc) =>
      mapN(visitExp(base, env0, tenv0), visitExp(elm, env0, tenv0)) {
        case (b, e) => NamedAst.Expression.VectorStore(b, index, e, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
      }

    case WeededAst.Expression.VectorLength(base, loc) =>
      visitExp(base, env0, tenv0) map {
        case b => NamedAst.Expression.VectorLength(b, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
      }

    case WeededAst.Expression.VectorSlice(base, startIndex, optEndIndex, loc) =>
      visitExp(base, env0, tenv0) map {
        case b => NamedAst.Expression.VectorSlice(b, startIndex, optEndIndex, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
      }

    case WeededAst.Expression.Ref(exp, loc) =>
      visitExp(exp, env0, tenv0) map {
        case e => NamedAst.Expression.Ref(e, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
      }

    case WeededAst.Expression.Deref(exp, loc) =>
      visitExp(exp, env0, tenv0) map {
        case e => NamedAst.Expression.Deref(e, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
      }

    case WeededAst.Expression.Assign(exp1, exp2, loc) =>
      mapN(visitExp(exp1, env0, tenv0), visitExp(exp2, env0, tenv0)) {
        case (e1, e2) => NamedAst.Expression.Assign(e1, e2, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
      }
    ...
    case WeededAst.Expression.Ascribe(exp, expectedType, expectedEff, loc) =>
      ...
      mapN(expVal, expectedTypVal, expectedEffVal) {
        case (e, t, f) => NamedAst.Expression.Ascribe(e, t, f, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
      }
    ...
    case WeededAst.Expression.Cast(exp, declaredType, declaredEff, loc) =>
      ...
      mapN(expVal, declaredTypVal, declaredEffVal) {
        case (e, t, f) => NamedAst.Expression.Cast(e, t, f, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
      }
    ...
    case WeededAst.Expression.TryCatch(exp, rules, loc) =>
      ...
      mapN(expVal, rulesVal) {
        case (e, rs) => NamedAst.Expression.TryCatch(e, rs, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
      }
    ...
    case WeededAst.Expression.InvokeConstructor(className, args, sig, loc) =>
      ...
      mapN(argsVal, sigVal) {
        case (as, sig) => NamedAst.Expression.InvokeConstructor(className, as, sig, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
      }
    ...
    case WeededAst.Expression.InvokeMethod(className, methodName, exp, args, sig, loc) =>
      ...
      mapN(expVal, argsVal, sigVal) {
        case (e, as, sig) => NamedAst.Expression.InvokeMethod(className, methodName, e, as, sig, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
      }
    ...
    case WeededAst.Expression.InvokeStaticMethod(className, methodName, args, sig, loc) =>
      ...
      val sigVal = traverse(sig)(visitType(_, tenv0))
      mapN(argsVal, sigVal) {
        case (as, sig) => NamedAst.Expression.InvokeStaticMethod(className, methodName, as, sig, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
      }

    case WeededAst.Expression.GetField(className, fieldName, exp, loc) =>
      mapN(visitExp(exp, env0, tenv0)) {
        case e => NamedAst.Expression.GetField(className, fieldName, e, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
      }

    case WeededAst.Expression.PutField(className, fieldName, exp1, exp2, loc) =>
      mapN(visitExp(exp1, env0, tenv0), visitExp(exp2, env0, tenv0)) {
        case (e1, e2) => NamedAst.Expression.PutField(className, fieldName, e1, e2, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
      }

    case WeededAst.Expression.GetStaticField(className, fieldName, loc) =>
      NamedAst.Expression.GetStaticField(className, fieldName, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc).toSuccess

    case WeededAst.Expression.PutStaticField(className, fieldName, exp, loc) =>
      mapN(visitExp(exp, env0, tenv0)) {
        case e => NamedAst.Expression.PutStaticField(className, fieldName, e, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
      }
    ...
    case WeededAst.Expression.GetChannel(exp, loc) =>
      visitExp(exp, env0, tenv0) map {
        case e => NamedAst.Expression.GetChannel(e, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
      }

    case WeededAst.Expression.PutChannel(exp1, exp2, loc) =>
      mapN(visitExp(exp1, env0, tenv0), visitExp(exp2, env0, tenv0)) {
        case (e1, e2) => NamedAst.Expression.PutChannel(e1, e2, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
      }
    ...
    case WeededAst.Expression.SelectChannel(rules, default, loc) =>
      ...
      mapN(rulesVal, defaultVal) {
        case (rs, d) => NamedAst.Expression.SelectChannel(rs, d, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
      }

    case WeededAst.Expression.ProcessSpawn(exp, loc) =>
      visitExp(exp, env0, tenv0) map {
        case e => NamedAst.Expression.ProcessSpawn(e, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
      }

    case WeededAst.Expression.ProcessPanic(msg, loc) =>
      NamedAst.Expression.ProcessPanic(msg, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc).toSuccess

    case WeededAst.Expression.FixpointConstraintSet(cs0, loc) =>
      mapN(traverse(cs0)(visitConstraint(_, env0, tenv0))) {
        case cs =>
          NamedAst.Expression.FixpointConstraintSet(cs, Type.freshTypeVarXXXDeprecated(), loc)
      }

    case WeededAst.Expression.FixpointCompose(exp1, exp2, loc) =>
      mapN(visitExp(exp1, env0, tenv0), visitExp(exp2, env0, tenv0)) {
        case (e1, e2) => NamedAst.Expression.FixpointCompose(e1, e2, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
      }

    case WeededAst.Expression.FixpointSolve(exp, loc) =>
      visitExp(exp, env0, tenv0) map {
        case e => NamedAst.Expression.FixpointSolve(e, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
      }

    case WeededAst.Expression.FixpointProject(qname, exp, loc) =>
      mapN(visitExp(exp, env0, tenv0)) {
        case e => NamedAst.Expression.FixpointProject(qname, e, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
      }

    case WeededAst.Expression.FixpointEntails(exp1, exp2, loc) =>
      mapN(visitExp(exp1, env0, tenv0), visitExp(exp2, env0, tenv0)) {
        case (e1, e2) => NamedAst.Expression.FixpointEntails(e1, e2, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
      }
    ...
    case WeededAst.Expression.FixpointFold(qname, init, f, constraints, loc) =>
      mapN(visitExp(init, env0, tenv0), visitExp(f, env0, tenv0), visitExp(constraints, env0, tenv0)) {
        case (e1, e2, e3) => NamedAst.Expression.FixpointFold(qname, e1, e2, e3, Type.freshTypeVarXXXDeprecated(), Type.freshEffectVar(), loc)
      }
  }
  ...
  private def visitPattern(pat0: WeededAst.Pattern)(implicit flix: Flix): (NamedAst.Pattern, Map[String, Symbol.VarSym]) = {
    ...
    def visit(p: WeededAst.Pattern): NamedAst.Pattern = p match {
      case WeededAst.Pattern.Wild(loc) => NamedAst.Pattern.Wild(Type.freshTypeVarXXXDeprecated(), loc)
      case WeededAst.Pattern.Var(ident, loc) =>
        // make a fresh variable symbol for the local variable.
        val sym = Symbol.freshVarSym(ident)
        m += (ident.name -> sym)
        NamedAst.Pattern.Var(sym, Type.freshTypeVarXXXDeprecated(), loc)
      ...
      case WeededAst.Pattern.Tag(enum, tag, pat, loc) => NamedAst.Pattern.Tag(enum, tag, visit(pat), Type.freshTypeVarXXXDeprecated(), loc)
      case WeededAst.Pattern.Tuple(elms, loc) => NamedAst.Pattern.Tuple(elms map visit, Type.freshTypeVarXXXDeprecated(), loc)
      case WeededAst.Pattern.Array(elms, loc) => NamedAst.Pattern.Array(elms map visit, Type.freshTypeVarXXXDeprecated(), loc)
      case WeededAst.Pattern.ArrayTailSpread(elms, ident, loc) => ident match {
        case None =>
          val sym = Symbol.freshVarSym("_")
          NamedAst.Pattern.ArrayTailSpread(elms map visit, sym, Type.freshTypeVarXXXDeprecated(), loc)
        case Some(id) =>
      ...
      case WeededAst.Pattern.ArrayHeadSpread(ident, elms, loc) => ident match {
        ...
        case Some(id) =>
          val sym = Symbol.freshVarSym(id)
          m += (id.name -> sym)
          NamedAst.Pattern.ArrayHeadSpread(sym, elms map visit, Type.freshTypeVarXXXDeprecated(), loc)
      }
    }
  ...
  private def visitPattern(pat0: WeededAst.Pattern, env0: Map[String, Symbol.VarSym])(implicit flix: Flix): NamedAst.Pattern = {
    def visit(p: WeededAst.Pattern): NamedAst.Pattern = p match {
      ...
      case WeededAst.Pattern.Array(elms, loc) => NamedAst.Pattern.Array(elms map visit, Type.freshTypeVarXXXDeprecated(), loc)
      case WeededAst.Pattern.ArrayTailSpread(elms, ident, loc) => ident match {
        case None => NamedAst.Pattern.ArrayTailSpread(elms map visit, Symbol.freshVarSym("_"), Type.freshTypeVarXXXDeprecated(), loc)
        case Some(value) =>
          val sym = env0(value.name)
          NamedAst.Pattern.ArrayTailSpread(elms map visit, sym, Type.freshTypeVarXXXDeprecated(), loc)
      }
      case WeededAst.Pattern.ArrayHeadSpread(ident, elms, loc) => ident match {
        case None => NamedAst.Pattern.ArrayHeadSpread(Symbol.freshVarSym("_"), elms map visit, Type.freshTypeVarXXXDeprecated(), loc)
        case Some(value) =>
          val sym = env0(value.name)
          NamedAst.Pattern.ArrayHeadSpread(sym, elms map visit, Type.freshTypeVarXXXDeprecated(), loc)
      }
  ...
  private def visitHeadPredicate(head: WeededAst.Predicate.Head, outerEnv: Map[String, Symbol.VarSym], headEnv0: Map[String, Symbol.VarSym], ruleEnv0: Map[String, Symbol.VarSym], tenv0: Map[String, Type.Var])(implicit flix: Flix): Validation[NamedAst.Predicate.Head, NameError] = head match {
    case WeededAst.Predicate.Head.Atom(qname, den, terms, loc) =>
      ...
      } yield NamedAst.Predicate.Head.Atom(qname, den, ts, Type.freshTypeVarXXXDeprecated(), loc)

    case WeededAst.Predicate.Head.Union(exp, loc) =>
      for {
        e <- visitExp(exp, outerEnv ++ headEnv0 ++ ruleEnv0, tenv0)
      } yield NamedAst.Predicate.Head.Union(e, Type.freshTypeVarXXXDeprecated(), loc)
  }
  ...
  private def visitBodyPredicate(body: WeededAst.Predicate.Body, outerEnv: Map[String, Symbol.VarSym], headEnv0: Map[String, Symbol.VarSym], ruleEnv0: Map[String, Symbol.VarSym], tenv0: Map[String, Type.Var])(implicit flix: Flix): Validation[NamedAst.Predicate.Body, NameError] = body match {
    case WeededAst.Predicate.Body.Atom(qname, den, polarity, terms, loc) =>
      ...
      NamedAst.Predicate.Body.Atom(qname, den, polarity, ts, Type.freshTypeVarXXXDeprecated(), loc).toSuccess
  ...
  private def getExplicitTypeParams(tparams0: List[Name.Ident])(implicit flix: Flix): List[NamedAst.TypeParam] = tparams0 map {
    case p =>
      ...
      val tvar = Type.freshTypeVarXXXDeprecated()
