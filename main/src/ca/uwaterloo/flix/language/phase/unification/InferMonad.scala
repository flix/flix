...
object InferMonad {
  ...
  /**
    ...
    */
  def point[A](x: A): InferMonad[A] = InferMonad { case (s, renv) => Ok((s, Nil, renv, x)) }
...
/**
  ...
  */
case class InferMonad[+A](run: (Substitution, RigidityEnv) => Result[(Substitution, List[(Type, Type)], RigidityEnv, A), TypeError]) {
  /**
    * Applies the given function `f` to the value in the monad.
    */
  def map[B](f: A => B): InferMonad[B] = {
    def runNext(s0: Substitution, renv0: RigidityEnv): Result[(Substitution, List[(Type, Type)], RigidityEnv, B), TypeError] = {
  ...
  def transformError[B](f: TypeError => TypeError): InferMonad[A] = {
    def runNext(s0: Substitution, renv0: RigidityEnv): Result[(Substitution, List[(Type, Type)], RigidityEnv, A), TypeError] = {
      run(s0, renv0) match {
        case Ok(t) => Ok(t)
        case Err(e) => Err(f(e))
      }
