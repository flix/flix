...
object TypedAstPrinter {
  ...
  private def print(e: TypedAst.Expr): DocAst.Expr = e match {
    ...
    case Expr.StructNew(_, _, _, _, _, _) => throw new RuntimeException("JOE TODO")
    case Expr.StructGet(_, _, _, _, _, _) => throw new RuntimeException("JOE TODO")
    case Expr.StructPut(_, _, _, _, _, _, _) => throw new RuntimeException("JOE TODO")
