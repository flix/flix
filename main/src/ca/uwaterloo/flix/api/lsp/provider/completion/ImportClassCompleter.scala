...
object ImportClassCompleter extends Completer {
  ...
  def getCompletions(context: CompletionContext)(implicit flix: Flix, index: Index, root: TypedAst.Root, delta: DeltaContext): Iterable[TraitCompletion] = {
    val regex = raw"\s*import\s+(?:.*\s+)*(.*)".r
    context.prefix match {
      case regex(trt) =>
        val path = trt.split('.').toList
  ...
  /**
    ...
    */
  private def javaClassCompletionsFromPrefix(prefix: List[String])(implicit root: TypedAst.Root): Iterable[TraitCompletion] = {
    root.names(prefix).map(clazz => {
      val label = prefix match {
        case Nil => clazz
        case v => v.mkString("", ".", s".$clazz")
      }
      Completion.TraitCompletion(label)
    })
  }
}
