...
object Stratifier {
  ...
  private def visitExp(exp0: Expr)(implicit g: LabelledPrecedenceGraph, sctx: SharedContext, root: Root, flix: Flix): Expr = exp0 match {
    ...
    case Expr.ApplyDef(symUse, exps, map, itpe, tpe, eff, loc) =>
      val es = exps.map(visitExp)
      Expr.ApplyDef(symUse, es, map, itpe, tpe, eff, loc)
    ...
    case Expr.ApplySig(symUse, exps, sel, map, itpe, tpe, eff, loc) =>
      val es = exps.map(visitExp)
      Expr.ApplySig(symUse, es, sel, map, itpe, tpe, eff, loc)
