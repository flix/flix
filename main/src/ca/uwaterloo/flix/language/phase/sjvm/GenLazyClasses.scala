...
object GenLazyClasses {
  ...
  def gen()(implicit root: Root, flix: Flix): Map[String, JvmClass] = {
    ...
    def genAUX[T <: PType](tpe: RType[T]): (String, JvmClass) = {
      ...
      val className = Instructions.getInternalName(eeType)
  ...
  private def genByteCode[T <: PType](className: String, innerType: RType[T])(implicit root: Root, flix: Flix): Array[Byte] = {
    ...
    visitor.visit(AsmOps.JavaVersion, ACC_PUBLIC + ACC_FINAL, className, null, "java/lang/Object", null)

    AsmOps.compileField(visitor, "initialized", Instructions.getDescriptor(RBool()), isStatic = false, isPrivate = true)
    AsmOps.compileField(visitor, "expression", Instructions.getDescriptor(RReference(null)), isStatic = false, isPrivate = true)
    ...
    compileLazyConstructor(visitor, className)
...
//  }
  ...
  /**
    ...
    */
  private def compileForceMethod[T <: PType](visitor: ClassWriter, classString: String, valueType: RType[T])(implicit root: Root, flix: Flix): Unit = ???
//  {
//    /*
//    force(context) :=
//
...
//  }
  ...
  /**
    ...
    */
  def compileLazyConstructor(visitor: ClassWriter, classType: String)(implicit root: Root, flix: Flix): Unit = ???
//  {
//    /*
//    Lazy$tpe(expression) :=
//
