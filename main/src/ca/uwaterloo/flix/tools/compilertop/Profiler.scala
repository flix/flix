...
object Profiler {
  /**
    ...
    * @param substTypeSize total AST node count across the types bound to `Kind.Star` variables in the def's final substitution, from the most recent typing pass.
    ...
    */
  final case class DefnStats(sym: Symbol.DefnSym, isActive: Boolean, totalNanos: Long, byPhase: Map[String, Long], byPhaseCount: Map[String, Long], byPhaseAlloc: Map[String, Long], cns: Long, tvars: Long, evars: Long, substTypeSize: Long, inlined: Long, classBytes: Long, allocBytes: Long, loc: SourceLocation) {
  ...
  /**
    ...
    * @param substTypeSize `Kind.Star` substitution term size from the most recent `FlixEvent.SubstitutionSizeDef`. Set (not accumulated), like [[constraints]].
...
final class Profiler(phaseProvider: () => Option[String]) extends FlixListener {
  ...
  override def notify(e: FlixEvent): Unit = e match {
    case FlixEvent.NewConstraintsDef(sym, tconstrs) =>
      ...
      c.tvars.set(tv)
      c.evars.set(ev)
    case FlixEvent.SubstitutionSizeDef(sym, typeSize) =>
      // `set` (not accumulate), mirroring NewConstraintsDef: a re-typing of
      // the same def replaces the old reading rather than doubling it.
      countersFor(sym).substTypeSize.set(typeSize.toLong)
  ...
  def snapshot(): Vector[Profiler.DefnStats] = {
    ...
    it.map { e =>
      ...
      Profiler.DefnStats(key.sym, isActive = inFlight.isDefined,
        ...
        c.constraints.get(), c.tvars.get(), c.evars.get(), c.substTypeSize.get(), c.inlined.get(), c.classBytes.get(), c.allocBytes.get(), loc)
    }.toVector
  }
  ...
  private def countersFor(sym: Symbol.DefnSym): Profiler.Counters =
    stats.computeIfAbsent(Profiler.DefnSymWithLoc(sourceOf(sym), sym.loc), _ => Profiler.Counters(
      ...
      substTypeSize = new AtomicLong(0L),
