...
object FormatExpression {
  ...
  def format(e0: TypedAst.Expression): String = e0 match {
    ...
    case TypedAst.Expression.UnaryDeprecated(op, exp, tpe, eff, loc) => s"Unary($op, $exp)"
    case TypedAst.Expression.Unary(sop, exp, tpe, eff, loc) => s"Unary($sop, $exp)"
    case TypedAst.Expression.BinaryDeprecated(op, exp1, exp2, tpe, eff, loc) => s"Binary($op, $exp1, $exp2)"
