...
object Deriver {
  ...
  private def mkEqInstance(enum0: KindedAst.Enum, loc: SourceLocation, root: KindedAst.Root)(implicit flix: Flix): Validation[KindedAst.Instance, DerivationError] = enum0 match {
    case KindedAst.Enum(_, _, _, _, tparams, _, _, tpe, _) =>
      ...
      val cconstrs = getTraitConstraintsForTypeParams(tparams, eqClassSym, loc)

      Validation.success(KindedAst.Instance(
        doc = Ast.Doc(Nil, loc),
        ann = Ast.Annotations.Empty,
        mod = Ast.Modifiers.Empty,
        trt = Ast.TraitSymUse(eqClassSym, loc),
        tpe = tpe,
        tconstrs = cconstrs,
        assocs = Nil,
        defs = List(defn),
  ...
  private def mkEqSpec(enum0: KindedAst.Enum, param1: Symbol.VarSym, param2: Symbol.VarSym, loc: SourceLocation, root: KindedAst.Root)(implicit flix: Flix): KindedAst.Spec = enum0 match {
    case KindedAst.Enum(_, _, _, _, tparams, _, _, tpe, _) =>
      ...
      KindedAst.Spec(
        ...
        sc = Scheme(
          tparams.map(_.sym),
          List(Ast.TraitConstraint(Ast.TraitConstraint.Head(eqClassSym, loc), tpe, loc)),
          Nil,
          Type.mkPureUncurriedArrow(List(tpe, tpe), Type.mkBool(loc), loc)
        ),
        tpe = Type.mkBool(loc),
        eff = Type.Cst(TypeConstructor.Pure, loc),
        tconstrs = List(Ast.TraitConstraint(Ast.TraitConstraint.Head(eqClassSym, loc), tpe, loc)),
        econstrs = Nil,
        loc = loc
      )
  }
  ...
  private def mkOrderInstance(enum0: KindedAst.Enum, loc: SourceLocation, root: KindedAst.Root)(implicit flix: Flix): Validation[KindedAst.Instance, DerivationError] = enum0 match {
    case KindedAst.Enum(_, _, _, _, tparams, _, _, tpe, _) =>
      ...
      val tconstrs = getTraitConstraintsForTypeParams(tparams, orderClassSym, loc)
      Validation.success(KindedAst.Instance(
        doc = Ast.Doc(Nil, loc),
        ann = Ast.Annotations.Empty,
        mod = Ast.Modifiers.Empty,
        trt = Ast.TraitSymUse(orderClassSym, loc),
        tpe = tpe,
        tconstrs = tconstrs,
        assocs = Nil,
        defs = List(defn),
  ...
  private def mkCompareSpec(enum0: KindedAst.Enum, param1: Symbol.VarSym, param2: Symbol.VarSym, loc: SourceLocation, root: KindedAst.Root): KindedAst.Spec = enum0 match {
    case KindedAst.Enum(_, _, _, _, tparams, _, _, tpe, _) =>
      ...
      KindedAst.Spec(
        ...
        sc = Scheme(
          tparams.map(_.sym),
          List(Ast.TraitConstraint(Ast.TraitConstraint.Head(orderClassSym, loc), tpe, loc)),
          Nil,
        ...
        eff = Type.Cst(TypeConstructor.Pure, loc),
        tconstrs = List(Ast.TraitConstraint(Ast.TraitConstraint.Head(orderClassSym, loc), tpe, loc)),
        econstrs = Nil,
        loc = loc
      )
  }
  ...
  private def mkToStringInstance(enum0: KindedAst.Enum, loc: SourceLocation, root: KindedAst.Root)(implicit flix: Flix): Validation[KindedAst.Instance, DerivationError] = enum0 match {
    case KindedAst.Enum(_, _, _, _, tparams, _, _, tpe, _) =>
      ...
      val tconstrs = getTraitConstraintsForTypeParams(tparams, toStringClassSym, loc)

      Validation.success(KindedAst.Instance(
        doc = Ast.Doc(Nil, loc),
        ann = Ast.Annotations.Empty,
        mod = Ast.Modifiers.Empty,
        trt = Ast.TraitSymUse(toStringClassSym, loc),
        tpe = tpe,
        tconstrs = tconstrs,
        assocs = Nil,
        defs = List(defn),
  ...
  private def mkToStringSpec(enum0: KindedAst.Enum, param: Symbol.VarSym, loc: SourceLocation, root: KindedAst.Root)(implicit flix: Flix): KindedAst.Spec = enum0 match {
    case KindedAst.Enum(_, _, _, _, tparams, _, _, tpe, _) =>
      ...
      KindedAst.Spec(
        ...
        sc = Scheme(
          tparams.map(_.sym),
          List(Ast.TraitConstraint(Ast.TraitConstraint.Head(toStringClassSym, loc), tpe, loc)),
          Nil,
          Type.mkPureArrow(tpe, Type.mkString(loc), loc)
        ),
        tpe = Type.mkString(loc),
        eff = Type.Cst(TypeConstructor.Pure, loc),
        tconstrs = List(Ast.TraitConstraint(Ast.TraitConstraint.Head(toStringClassSym, loc), tpe, loc)),
        econstrs = Nil,
        loc = loc
      )
  }
  ...
  private def mkHashInstance(enum0: KindedAst.Enum, loc: SourceLocation, root: KindedAst.Root)(implicit flix: Flix): Validation[KindedAst.Instance, DerivationError] = enum0 match {
    case KindedAst.Enum(_, _, _, _, tparams, _, _, tpe, _) =>
      ...
      val tconstrs = getTraitConstraintsForTypeParams(tparams, hashClassSym, loc)
      Validation.success(KindedAst.Instance(
        doc = Ast.Doc(Nil, loc),
        ann = Ast.Annotations.Empty,
        mod = Ast.Modifiers.Empty,
        trt = Ast.TraitSymUse(hashClassSym, loc),
        tpe = tpe,
        tconstrs = tconstrs,
        defs = List(defn),
        assocs = Nil,
  ...
  private def mkHashSpec(enum0: KindedAst.Enum, param: Symbol.VarSym, loc: SourceLocation, root: KindedAst.Root)(implicit flix: Flix): KindedAst.Spec = enum0 match {
    case KindedAst.Enum(_, _, _, _, tparams, _, _, tpe, _) =>
      ...
      KindedAst.Spec(
        ...
        sc = Scheme(
          tparams.map(_.sym),
          List(Ast.TraitConstraint(Ast.TraitConstraint.Head(hashClassSym, loc), tpe, loc)),
          Nil,
          Type.mkPureArrow(tpe, Type.mkInt32(loc), loc)
        ),
        tpe = Type.mkInt32(loc),
        eff = Type.Cst(TypeConstructor.Pure, loc),
        tconstrs = List(Ast.TraitConstraint(Ast.TraitConstraint.Head(hashClassSym, loc), tpe, loc)),
        econstrs = Nil,
        loc = loc
      )
  }
  ...
  private def mkSendableInstance(enum0: KindedAst.Enum, loc: SourceLocation, root: KindedAst.Root)(implicit flix: Flix): Validation[KindedAst.Instance, DerivationError] = enum0 match {
    case KindedAst.Enum(_, _, _, _, tparams, _, _, tpe, _) =>
      ...
      val tconstrs = getTraitConstraintsForTypeParams(tparams, sendableClassSym, loc)

      Validation.success(KindedAst.Instance(
        doc = Ast.Doc(Nil, loc),
        ann = Ast.Annotations.Empty,
        mod = Ast.Modifiers.Empty,
        trt = Ast.TraitSymUse(sendableClassSym, loc),
        tpe = tpe,
        tconstrs = tconstrs,
        defs = Nil,
        assocs = Nil,
  ...
  /**
    ...
    */
  private def getTraitConstraintsForTypeParams(tparams: List[KindedAst.TypeParam], clazz: Symbol.TraitSym, loc: SourceLocation): List[Ast.TraitConstraint] = tparams.collect {
    case tparam if tparam.sym.kind == Kind.Star && !tparam.name.isWild =>
      Ast.TraitConstraint(Ast.TraitConstraint.Head(clazz, loc), Type.Var(tparam.sym, loc), loc)
  }
