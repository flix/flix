...
object TreeExample {
  ...
  private def showTree(t: Tree): Doc = t match {
    case Node(s, ts) => group(text(s) <> nest(s.length, showBracket(ts)))
  }
  ...
  private def showTree1(t: Tree): Doc = t match {
    case Node(s, ts) => group(text(s) <> showBracket1(ts))
  }
  ...
  def main(args: Array[String]): Unit = {
    def trial(layoutN: Int, w: Int, st: Tree => Doc): Unit = {
      println(s"Layout $layoutN($w):")
      println(pretty(w, st(exampleTree)))
      println()
    }
    trial(1, 0, showTree)
    trial(1, 30, showTree)
    trial(1, 50, showTree)
    trial(1, 0, showTree1)
    trial(1, 30, showTree1)
    trial(1, 50, showTree1)
  }
}
