...
object ResolvedAstPrinter {
  ...
  private def print(exp0: ResolvedAst.Expr): DocAst.Expr = exp0 match {
    ...
    case Expr.ApplyDef(DefSymUse(sym, _), exps, _) => DocAst.Expr.ApplyDef(sym, exps.map(print).toList)
    case Expr.ApplyLocalDef(LocalDefSymUse(sym, _), exps, _) => DocAst.Expr.App(printVarSym(sym), exps.map(print).toList)
    case Expr.ApplyOp(symUse, exps, _) => DocAst.Expr.ApplyOp(symUse.sym, exps.map(print).toList)
    case Expr.ApplySig(SigSymUse(sym, _), exps, _) => DocAst.Expr.App(DocAst.Expr.AsIs(sym.name), exps.map(print).toList)
