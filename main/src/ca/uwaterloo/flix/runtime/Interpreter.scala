...
object Interpreter {
  ...
  def eval(expr: Expression, root: Root, env: mutable.Map[String, AnyRef] = mutable.Map.empty): AnyRef = expr match {
    ...
    case Expression.Var(ident, _, _, loc) => env.get(ident.name) match {
      case None =>
        throw InternalRuntimeException(s"Key '${ident.name}' not found in environment: '${env.mkString(",")}'.")
      case Some(v) => v
    }
    ...
    case Expression.Apply(name, args, _, _) =>
      ...
      var i = 0
      while (i < args.length) {
        closureEnv(constant.formals(i).ident.name) = eval(args(i), root, env)
        i = i + 1
      }
    ...
    case Expression.ApplyClosure(exp, args, tpe, loc) =>
      ...
      eval(exp, root, env) match {
        case Value.Closure(ref, envVar, clEnv) =>
          ...
          closureEnv(envVar.name) = clEnv
          eval(constant.exp, root, closureEnv)
        case Value.HookClosure(hook) =>
          val evalArgs = args.map(eval(_, root, env))
          hook match {
          case Ast.Hook.Safe(name, inv, _) =>
            val wargs: Array[IValue] = evalArgs.map(new WrappedValue(_)).toArray
