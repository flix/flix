...
object SymbolProvider {
  ...
  def processWorkspaceSymbols(query: String)(implicit root: Root): List[SymbolInformation] = {
    ...
    val classes = root.classes.values.collect { case c if c.sym.name.startsWith(query) => mkTraitSymbolInformation(c) }
  ...
  def processDocumentSymbols(uri: String)(implicit root: Root): List[DocumentSymbol] = {
    ...
    val classes = root.classes.values.collect { case c if c.sym.loc.source.name == uri => mkTraitDocumentSymbol(c) }
  ...
  /**
    ...
    */
  private def mkTraitSymbolInformation(c: TypedAst.Trait) = c match {
  ...
  /**
    ...
    */
  private def mkTraitDocumentSymbol(c: TypedAst.Trait): DocumentSymbol = c match {
