...
object LiftedAstPrinter {
  ...
  def print(e: LiftedAst.Expression): DocAst.Expression = e match {
    ...
    case ApplyClo(exp, args, _, _, _) => DocAst.Expression.AppClo(print(exp), args.map(print))
    case ApplyDef(sym, args, _, _, _) => DocAst.Expression.App(sym, args.map(print))
    case ApplyCloTail(exp, args, _, _, _) => DocAst.Expression.AppCloTail(print(exp), args.map(print))
    case ApplyDefTail(sym, args, _, _, _) => DocAst.Expression.AppDefTail(sym, args.map(print))
    case ApplySelfTail(sym, _, actuals, _, _, _) => DocAst.Expression.AppSelfTail(sym, actuals.map(print))
    case Unary(sop, _, exp, _, _, _) => DocAst.Expression.Unary(OperatorPrinter.print(sop), print(exp))
    case Binary(sop, _, exp1, exp2, _, _, _) => DocAst.Expression.Binary(print(exp1), OperatorPrinter.print(sop), print(exp2))
