  ...
  private def visitDecl(decl0: WeededAst.Declaration, ns0: Name.NName, prog0: NamedAst.Root)(implicit genSym: GenSym): Validation[NamedAst.Root, NameError] = decl0 match {
    ...
    case WeededAst.Declaration.Relation(doc, mod, ident, attr, loc) =>
      ...
      prog0.relations.get(ns0) match {
        case None =>
          ...
          val relation = NamedAst.Relation(doc, mod, Symbol.mkRelSym(ns0, ident), attr.map(a => visitAttribute(a, Map.empty)), loc)
    ...
    case WeededAst.Declaration.Lattice(doc, mod, ident, attr, loc) =>
      ...
      prog0.lattices.get(ns0) match {
        case None =>
          // Case 1: The namespace does not yet exist. So the table does not yet exist.
          val lattice = NamedAst.Lattice(doc, mod, Symbol.mkLatSym(ns0, ident), attr.map(k => visitAttribute(k, Map.empty)), loc)
