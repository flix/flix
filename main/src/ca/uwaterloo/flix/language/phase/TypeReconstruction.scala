...
object TypeReconstruction {
  ...
  /**
    ...
    */
  private def visitExp(exp0: KindedAst.Expr, root: KindedAst.Root, subst0: Substitution): TypedAst.Expr = {
    /**
      * Reconstructs types in the given expression.
      */
    def visit(exp0: KindedAst.Expr, subst0: Substitution): TypedAst.Expr = exp0 match {
      ...
      case KindedAst.Expr.Apply(exp, exps, tvar, pvar, loc) =>
        val e = visit(exp, subst0)
        val es = exps.map(visit(_, subst0))
      ...
      case KindedAst.Expr.Binary(sop, exp1, exp2, tvar, loc) =>
        val e1 = visit(exp1, subst0)
        val e2 = visit(exp2, subst0)
      ...
      case KindedAst.Expr.Match(matchExp, rules, loc) =>
        val e1 = visit(matchExp, subst0)
        val rs = rules map {
          case KindedAst.MatchRule(pat, guard, exp) =>
            val p = visitPattern(pat, root, subst0)
            val g = guard.map(visit(_, subst0))
            val b = visit(exp, subst0)
      ...
      case KindedAst.Expr.RelationalChoose(_, exps, rules, tvar, loc) =>
        ...
        val rs = rules.map {
          case KindedAst.RelationalChooseRule(pat0, exp) =>
            ...
            TypedAst.RelationalChooseRule(pat, visit(exp, subst0))
        }
        val tpe = subst0(tvar)
      ...
      case KindedAst.Expr.Tuple(elms, loc) =>
        val es = elms.map(visit(_, subst0))
      ...
      case KindedAst.Expr.RecordExtend(field, value, rest, tvar, loc) =>
        val v = visit(value, subst0)
        val r = visit(rest, subst0)
      ...
      case KindedAst.Expr.InvokeConstructor(constructor, args, loc) =>
        val as = args.map(visit(_, subst0))
      ...
      case KindedAst.Expr.SelectChannel(rules, default, tvar, loc) =>
        val rs = rules map {
          case KindedAst.SelectChannelRule(sym, chan, exp) =>
            val c = visit(chan, subst0)
            val b = visit(exp, subst0)
            TypedAst.SelectChannelRule(sym, c, b)
        }
        val d = default.map(visit(_, subst0))
        val eff = Type.Impure
    ...
    def visitConstraint(c0: KindedAst.Constraint): TypedAst.Constraint = {
      ...
      val head = visitHeadPredicate(head0, root, subst0)
      val body = body0.map(b => visitBodyPredicate(b, root, subst0))
  ...
  /**
    ...
    */
  private def visitPattern(pat0: KindedAst.Pattern, root: KindedAst.Root, subst0: Substitution): TypedAst.Pattern = {
    /**
  ...
  /**
    ...
    */
  private def visitHeadPredicate(head0: KindedAst.Predicate.Head, root: KindedAst.Root, subst0: Substitution): TypedAst.Predicate.Head = head0 match {
    case KindedAst.Predicate.Head.Atom(pred, den0, terms, tvar, loc) =>
      val ts = terms.map(t => visitExp(t, root, subst0))
  ...
  /**
    ...
    */
  private def visitBodyPredicate(body0: KindedAst.Predicate.Body, root: KindedAst.Root, subst0: Substitution): TypedAst.Predicate.Body = body0 match {
    case KindedAst.Predicate.Body.Atom(pred, den0, polarity, fixity, terms, tvar, loc) =>
      val ts = terms.map(t => visitPattern(t, root, subst0))
    ...
    case KindedAst.Predicate.Body.Functional(outVars, exp, loc) =>
      val e = visitExp(exp, root, subst0)
