...
object EntryPoint {
  ...
  /**
    ...
    */
  private def visitEntryPoint(defn: TypedAst.Def, root: TypedAst.Root, classEnv: Map[Symbol.TraitSym, Ast.TraitContext])(implicit flix: Flix): Validation[TypedAst.Def, EntryPointError] = {
  ...
  /**
    ...
    */
  private def checkEntryPointArgs(defn: TypedAst.Def, classEnv: Map[Symbol.TraitSym, Ast.TraitContext], root: TypedAst.Root)(implicit flix: Flix): Validation[Unit, EntryPointError] = defn match {
  ...
  /**
    ...
    */
  private def checkEntryPointResult(defn: TypedAst.Def, root: TypedAst.Root, classEnv: Map[Symbol.TraitSym, Ast.TraitContext])(implicit flix: Flix): Validation[Unit, EntryPointError] = defn match {
    case TypedAst.Def(sym, TypedAst.Spec(_, _, _, _, _, declaredScheme, _, declaredEff, _, _, _), _) =>
      ...
      if (Scheme.equal(unitSc, resultSc, classEnv, ListMap.empty)) { // TODO ASSOC-TYPES better eqEnv
        ...
        if (ClassEnvironment.holds(Ast.TraitConstraint(Ast.TraitConstraint.Head(toString, SourceLocation.Unknown), resultTpe, SourceLocation.Unknown), classEnv)) {
