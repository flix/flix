  ...
  private object Exprs {
    ...
    private def visitIntrinsic(tree: Tree, args: List[Expr])(implicit sctx: SharedContext): Validation[Expr, CompilationMessage] = {
      ...
      (intrinsic, args) match {
        ...
        case ("JVM_VALUE", e1 :: Nil) => Validation.Success(Expr.JvmReflection(e1, proxy = false, loc))
        case ("JVM_TYPE_OF_PROXY", e1 :: Nil) => Validation.Success(Expr.JvmReflection(e1, proxy = true, loc))
        case _ =>
