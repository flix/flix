...
import ca.uwaterloo.flix.tools.fmt.Doc.{empty, hardStack, hardline, line, nest, pretty, space, text, Layout}
...
object PrettyPrinter {
  ...
  private def formatTree(tree: Tree): Doc = tree.kind match {
    ...
    case TreeKind.Expr.AmbiguousNew => prettyAmbiguousNew(tree)
    ...
    case TreeKind.Type.Schema => prettySpacedCommaBracket(tree)
  ...
  private def prettyFCons(tree: Tree): Doc =
    filterEmpty(tree.children) match {
      case Array(head, tail) =>
        val headDoc = head match {
          case t: Tree =>
            filterEmpty(t.children).filterNot(isToken(_, TokenKind.ColonColon)) match {
              case Array() => prettyChild(head)
              case Array(single) => prettyChild(single)
              case inner => joinWithGap(inner)
            }
          case _ => prettyChild(head)
        }
        headDoc <> space <> text("::") <> space <> prettyChild(tail)
      case _ => prettyFallback(tree)
    }
  ...
  /** Formatting for struct field writes: `obj->field = value`. */
  private def prettyStructPut(tree: Tree): Doc =
    localLayout(tree) {
      joinChildren(filterEmpty(tree.children),
        TokenKind.ArrowThinRTight -> text("->"),
        TokenKind.Equal -> (space <> text("=") <> space))
    }

  /** Formatting for struct field reads: `obj->field`. */
  private def prettyStructGet(tree: Tree): Doc =
    localLayout(tree) {
      joinChildren(filterEmpty(tree.children), TokenKind.ArrowThinRTight -> text("->"))
    }

  private def prettyRegion(tree: Tree): Doc = {
    val children = filterEmpty(tree.children)
    val (header, fromBlock) = children.span(child => !isTree(child, TreeKind.Expr.Block))
    fromBlock.headOption match {
      case None => spaceJoin(children, Set.empty)
      case Some(block) =>
        localLayout(tree) {
          spaceJoin(header, Set.empty) <+> prettyChild(block)
        }
    }
  }
  ...
  private def prettyRun(tree: Tree): Doc = {
    val children = filterEmpty(tree.children)
    val (preamble, handlers) = children.span(child => !isTree(child, TreeKind.Expr.RunWithBodyExpr))
    if (handlers.isEmpty) return spaceJoin(children, Set.empty)

    val preambleDoc = spaceJoin(preamble, Set.empty)
    localLayout(tree) {
      preambleDoc <+> Doc.column(col =>
        handlers.map(prettyChild).reduceLeftOption { (accumulated, handler) =>
          accumulated <> Doc.nestAbsolute(col, line) <> handler
        }.getOrElse(empty)
      )
    }
  }
  ...
  private def prettyHandler(tree: Tree): Doc =
    prettyBracket(tree, filterEmpty(tree.children),
      headerJoin = children => spaceJoin(children, Set.empty),
      formatBody = alignedHandlerBody,
      flatPad = space)

  private def runWithRulePreEqualWidth(rule: Tree): Int = {
    val (_, rest) = extractAnnAndDoc(rule)
    splitAtToken(rest, TokenKind.Equal) match {
      case None => 0
      case Some((sigParts, _, _)) => pretty(Layout.SingleLine, buildSig(sigParts)).length
    }
  }
  ...
  private def alignedRunWithRule(tree: Tree, maxWidth: Int): Doc = {
    ...
    splitAtToken(rest, TokenKind.Equal) match {
      case None =>
        prepend(annDoc, buildSig(rest))

      case Some((sigParts, eqToken, bodyParts)) =>
        ...
        val bodyOnSameLine = bodyStartsOnSameLineAs(eqToken, bodyParts)

        val ruleDoc = Doc.setLayout(layoutOfChildren(rest),
          attachBody(sigPadded <+> text("="), body, bodyIsPreserved, bodyOnSameLine))
  ...
  private def prettyIfThenElse(tree: Tree): Doc = {
    ...
    splitAtBracket(children) match {
      ...
      case Some(BracketSplit(_, open, condChildren, close, tail)) =>
        ...
        val (thenChildren, elseChildren) = splitAtToken(tail, TokenKind.KeywordElse) match {
          case None => (tail, Array.empty[SyntaxTree.Child])
          case Some((before, _, after)) => (before, after)
        }
        ...
        val thenPart = if (thenIsBlock) condPart <+> thenBody else condPart <> nest(Indent, line <> thenBody)
        ...
        localLayout(tree) {
          if (elseChildren.isEmpty) {
            ...
            thenPart <> elseConnector <> text("else") <> nest(Indent, line <> elseBody)
          }
        }
    }
  }
  ...
  private def prettyArrowRuleFragment(tree: Tree, headerJoin: Array[SyntaxTree.Child] => Doc): Doc = {
    val children = filterEmpty(tree.children)
    splitAtToken(children, TokenKind.ArrowThickR) match {
      case None => prettyFallback(tree)
      case Some((before, arrowToken, body)) =>
        val headerDoc = headerJoin(before :+ arrowToken)
        ...
        if (body.isEmpty) headerDoc
          ...
          val bodyOnSameLine = bodyStartsOnSameLineAs(arrowToken, body)
          localLayout(tree) {
            attachBody(headerDoc, bodyDoc, bodyIsPreserved, bodyOnSameLine)
          }
        }
    }
  }
  ...
  /**
    * Formatting for comprehension-like constructs whose generators live inside a
    * bracket pair and are separated by `;` (par-yield, monadic/applicative for).
    *
    * The body is laid out on a single line (with `; ` separators, the close
    * bracket pulled onto a fresh line) when it fits on one source line; otherwise
    * each generator goes on its own line. When `requireParensSameLine` is set,
    * the compact form is additionally suppressed unless the bracket pair itself
    * occupied a single source line.
    */
  private def prettyComprehension(tree: Tree, requireParensSameLine: Boolean): Doc = {
    val children = filterEmpty(tree.children)
    splitAtBracket(children) match {
      case None => prettyFallback(tree)
      case Some(BracketSplit(header, open, body, close, tail)) =>
        ...
        val singleLine =
          (!requireParensSameLine || parensSameLine(children)) &&
            layoutOfChildren(body) == Layout.SingleLine
        if (singleLine) {
          val bodyDoc = joinChildren(body, TokenKind.Semi -> (text(";") <> space))
          Doc.setLayout(Layout.MultiLine,
            headerDoc <+> text(open) <> bodyDoc <> text(close) <> nest(Indent, hardline <> tailDoc)
          )
        } else {
          val bodyDoc = joinChildren(body, TokenKind.Semi -> (text(";") <> line))
          val tailSuffix = if (tail.nonEmpty) space <> tailDoc else empty
          Doc.setLayout(Layout.MultiLine,
            headerDoc <+> text(open) <> nest(Indent, line <> bodyDoc) <> line <> text(close) <> tailSuffix
          )
        }
    }
  }
  ...
  private def prettyParYield(tree: Tree): Doc =
    prettyComprehension(tree, requireParensSameLine = false)
  ...
  private def prettyFor(tree: Tree): Doc =
    prettyComprehension(tree, requireParensSameLine = true)
  ...
  /** True if `child` is a [[Token]] of one of the given `kinds`. */
  private def isToken(child: SyntaxTree.Child, kinds: TokenKind*): Boolean =
    child match {
      case t: Token => kinds.contains(t.kind)
      case _ => false
    }

  /** True if `child` is a [[Tree]] of the given `kind`. */
  private def isTree(child: SyntaxTree.Child, kind: TreeKind): Boolean =
    child match {
      case t: Tree => t.kind == kind
      case _ => false
    }

  /** Index of the first child that is a token of one of `kinds`, or `-1` if none. */
  private def indexOfToken(children: Array[SyntaxTree.Child], kinds: TokenKind*): Int =
    children.indexWhere(isToken(_, kinds: _*))

  /** True if any child is a token of one of `kinds`. */
  private def containsToken(children: Array[SyntaxTree.Child], kinds: TokenKind*): Boolean =
    children.exists(isToken(_, kinds: _*))

  /** Index of the first child that is a tree whose kind is in `kinds`, or `-1` if none. */
  private def indexOfTreeKind(children: Array[SyntaxTree.Child], kinds: Set[TreeKind]): Int =
    children.indexWhere {
      case t: Tree => kinds.contains(t.kind)
      case _ => false
    }

  /** The first child that is a token of `kind`, if any. */
  private def firstToken(children: Array[SyntaxTree.Child], kind: TokenKind): Option[Token] =
    children.collectFirst { case t: Token if t.kind == kind => t }

  /** The last child that is a token of `kind`, if any. */
  private def lastToken(children: Array[SyntaxTree.Child], kind: TokenKind): Option[Token] =
    children.reverseIterator.collectFirst { case t: Token if t.kind == kind => t }

  /**
    * Splits `children` at the first token of one of `kinds` into the children
    * before it, the token itself, and the children after it. `None` if no such
    * token occurs.
    */
  private def splitAtToken(
    children: Array[SyntaxTree.Child],
    kinds: TokenKind*
  ): Option[(Array[SyntaxTree.Child], Token, Array[SyntaxTree.Child])] =
    children.span(c => !isToken(c, kinds: _*)) match {
      case (before, after) =>
        after.headOption match {
          case Some(sep: Token) => Some((before, sep, after.tail))
          case _ => None
        }
    }
  ...
  private def splitAtBracket(children: Array[SyntaxTree.Child]): Option[BracketSplit] = {
    bracketPairs.flatMap {
      case (openKind, closeKind, openText, closeText) =>
        val openIdx = children.indexWhere(isToken(_, openKind))
        val closeIdx = children.lastIndexWhere(isToken(_, closeKind))
        if (openIdx >= 0 && closeIdx > openIdx) Some(BracketSplit(
          header = children.slice(0, openIdx),
          open = openText,
          body = children.slice(openIdx + 1, closeIdx),
          close = closeText,
          tail = children.slice(closeIdx + 1, children.length)
        ))
        else None
    }.headOption
  }
  ...
  private def prettyDeclBracket(
    tree: Tree,
    headerJoin: Array[SyntaxTree.Child] => Doc = defaultHeaderJoin,
    formatBody: Array[SyntaxTree.Child] => Doc = children => joinWithGap(filterEmpty(children)),
    fallback: Tree => Doc = prettyFallback,
    nestLevel: Int = Indent
  ): Doc = wrapWithAnn(tree, rest => {
    val openToken = firstToken(rest, TokenKind.CurlyL)
    val closeToken = lastToken(rest, TokenKind.CurlyR)
    val wrappedFormatBody: Array[SyntaxTree.Child] => Doc = bodyChildren => {
      val base = formatBody(bodyChildren)
      val filtered = filterEmpty(bodyChildren)
  ...
  private def prettyBracket(
    ...
    headerJoin: Array[SyntaxTree.Child] => Doc = defaultHeaderJoin,
    formatBody: Array[SyntaxTree.Child] => Doc = children => joinWithGap(filterEmpty(children)),
    formatTail: Array[SyntaxTree.Child] => Doc = children => if (children.isEmpty) empty else joinWithGap(children),
    fallback: Tree => Doc = prettyFallback,
    flatPad: Doc = empty,
    nestLevel: Int = Indent
    ...
    case Some(split) =>
      ...
      val tailDoc = tailContent match {
        ...
        case _ =>
          val closeIdx = children.length - split.tail.length - 1
          val closeChild = if (closeIdx >= 0 && closeIdx < children.length) Some(children(closeIdx)) else None
  ...
  private def prettySpacedCommaBracket(tree: Tree): Doc =
    prettyBracket(tree, tree.children, formatBody = commaBodyJoin, flatPad = space)
  ...
  private def recordFieldName(field: Tree): Doc =
    splitAtToken(filterEmpty(field.children), TokenKind.Equal) match {
      case None => empty
      case Some((nameParts, _, _)) => joinChildren(nameParts, TokenKind.Plus -> text("+"))
    }
  ...
  private def alignedRecordField(tree: Tree, maxWidth: Int): Doc =
    splitAtToken(filterEmpty(tree.children), TokenKind.Equal) match {
      case None => prettyRecordFieldAssign(tree)
      case Some((_, _, valueParts)) =>
        val nameDoc = recordFieldName(tree)
        val valueDoc = joinWithGap(valueParts)
        val isSingleLine = effectiveLayoutOf(tree) == Layout.SingleLine
  ...
  private def alignedRecordBody(fieldKinds: Set[TreeKind], children: Array[SyntaxTree.Child]): Doc = {
    ...
    filtered.foldLeft(empty) { (result, child) =>
  ...
  private def prettyAlignedRecord(tree: Tree, fieldKinds: Set[TreeKind]): Doc =
    prettyBracket(tree, tree.children, formatBody = children => alignedRecordBody(fieldKinds, children))
  ...
  private def mapKeyDoc(fragment: Tree): Doc =
    splitAtToken(filterEmpty(fragment.children), TokenKind.ArrowThickR) match {
      case None => empty
      case Some((keyParts, _, _)) => joinWithGap(keyParts)
    }
  ...
  private def alignedMapFragment(tree: Tree, maxWidth: Int): Doc =
    splitAtToken(filterEmpty(tree.children), TokenKind.ArrowThickR) match {
      case None => prettyLiteralMapKeyValueFragment(tree)
      case Some((_, _, valueParts)) =>
  ...
  private def prettyEnum(tree: Tree): Doc = wrapWithAnn(tree, rest => {
    val hasBraces = containsToken(rest, TokenKind.CurlyL, TokenKind.CurlyR)
    ...
    trailingComments match {
      case Some(commentList) => appendTrailingCommentList(mainDoc, mainChildren.lastOption, commentList)
      case None => mainDoc
    }
  })
  ...
  private def prettyCase(tree: Tree): Doc = {
    val children = filterEmpty(tree.children)
    val caseKeywordIdx = indexOfToken(children, TokenKind.KeywordCase)
    if (caseKeywordIdx < 0) return prettyFallback(tree)

    val (leading, body) = children.splitAt(caseKeywordIdx)
    ...
    if (leading.isEmpty) {
      ...
      val hasLeadingComma = leading.exists(isToken(_, TokenKind.Comma))
      val comments = leading.filterNot(isToken(_, TokenKind.Comma))
  ...
  private def prettyCaseBody(tree: Tree): Doc =
    splitAtBracket(tree.children) match {
      ...
      case Some(split) =>
        ...
        val tailDoc = tailAfterClose(tree.children, split.tail)
        val bodyStartsWithBrace =
          split.body.headOption.flatMap(leftMostToken).exists(_.kind == TokenKind.CurlyL)
        if (bodyStartsWithBrace)
          localLayout(tree) {
            text(split.open) <> bodyDoc <> text(split.close) <> tailDoc
          }
        else
  ...
  private def prettyInstance(tree: Tree): Doc = {
    val nestLevel = {
      ...
      val braceIdx = indexOfToken(children, TokenKind.CurlyL)
      val header = if (braceIdx > 0) children.take(braceIdx) else children
      val first = header.headOption.flatMap(leftMostToken)
      val last = header.lastOption.flatMap(rightMostToken)
      (first, last) match {
        case (Some(f), Some(l)) if f.start.lineOneIndexed != l.end.lineOneIndexed => 2 * Indent
        case _ => Indent
      }
    }
  ...
  private def structFieldPreColonWidth(field: Tree): Int =
    splitAtToken(filterEmpty(field.children), TokenKind.Colon) match {
      case None => 0
      case Some((nameParts, _, _)) =>
        pretty(Layout.SingleLine, spaceJoin(nameParts, noSpacePairs = Set.empty)).length
    }

  private def alignedStructField(tree: Tree, maxWidth: Int): Doc =
    splitAtToken(filterEmpty(tree.children), TokenKind.Colon) match {
      case None => prettyFallback(tree)
      case Some((nameParts, _, typeParts)) =>
        val nameDoc = spaceJoin(nameParts, noSpacePairs = Set.empty)
        val typeDoc = joinWithGap(typeParts)
        val isSingleLine = effectiveLayoutOf(tree) == Layout.SingleLine
  ...
  private def arrowPatternHeaderJoin(kind: TreeKind): Array[SyntaxTree.Child] => Doc = kind match {
    case TreeKind.Expr.TryCatchRuleFragment =>
      children => spaceJoin(children, noSpacePairs = Set.empty, noSpaceBefore = Set(TokenKind.Colon))
    case TreeKind.Expr.SelectRuleFragment =>
      children =>
        spaceJoin(children, noSpacePairs = Set.empty,
  ...
  private def arrowPatternWidth(rule: Tree): Int =
    splitAtToken(filterEmpty(rule.children), TokenKind.ArrowThickR) match {
      case None => 0
      case Some((patternParts, _, _)) =>
        pretty(Layout.SingleLine, arrowPatternHeaderJoin(rule.kind)(patternParts)).length
    }
  ...
  private def alignedArrowRule(tree: Tree, maxWidth: Int): Doc = {
    ...
    splitAtToken(children, TokenKind.ArrowThickR) match {
      case None => prettyFallback(tree)
      case Some((patternPart, arrowToken, bodyPart)) =>
  ...
  private def prettyTryCatchRuleFragment(tree: Tree): Doc =
    prettyArrowRuleFragment(tree, children => spaceJoin(children, noSpacePairs = Set.empty, noSpaceBefore = Set(TokenKind.Colon)))
  ...
  private def prettyCheckedCast(tree: Tree, kind: TokenKind, keyword: String): Doc =
    prettyKeywordParen(tree,
      headerReplacements = Seq(kind -> text(keyword)),
      ...
      case Some(BracketSplit(header, open, body, close, tail)) =>
        ...
        localLayout(tree) {
          bodyDoc match {
            ...
            case _ => headerDoc <> text(open) <> nest(Indent, pad <> bodyDoc) <> pad <> text(close) <> tailDoc
          }
        }
    }
  }
      ...
      case Some(split) =>
        ...
        localLayout(tree) {
          bodyDoc match {
            ...
            case _ => text(split.open) <> nest(Indent, pad <> bodyDoc) <> pad <> text(split.close)
          }
        }
    }
  ...
  private def prettyFixpointConstraint(tree: Tree): Doc = {
    val children = filterEmpty(tree.children)
    splitAtToken(children, TokenKind.ColonMinus) match {
      case None =>
        joinChildren(children, TokenKind.DotWhiteSpace -> text("."))

      case Some((head, colonToken, afterColon)) =>
        // Split off anything after the terminating `.` so trailing comments
        // attached by the parser's `close()` are not indented by `nest(Indent, ...)`.
        val dotIdx = indexOfToken(afterColon, TokenKind.DotWhiteSpace)
        val (body, trailing) =
          if (dotIdx < 0) (afterColon, Array.empty[SyntaxTree.Child])
          else afterColon.splitAt(dotIdx + 1)
        ...
        val bodyOnSameLine = bodyStartsOnSameLineAs(colonToken, body)
        val mainDoc = localLayout(tree) {
          headDoc <+> text(":-") <> nest(Indent, (if (bodyOnSameLine) space else line) <> bodyDoc)
        }

        if (trailing.isEmpty) mainDoc
        else {
          val prevRight = body.reverse.collectFirst(Function.unlift(rightMostToken))
          val nextLeft = leftMostToken(trailing.head)
          val gap = (prevRight, nextLeft) match {
            case (Some(p), Some(n)) if p.end.lineOneIndexed == n.start.lineOneIndexed => space
            case (Some(p), Some(n)) if n.start.lineOneIndexed - p.end.lineOneIndexed > 1 => hardline <> hardline
            case _ => hardline
          }
          val trailingDoc = trailing.map(prettyChild).reduceLeftOption(_ <> _).getOrElse(empty)
          mainDoc <> gap <> trailingDoc
        }
    }
  }
  ...
  private def prettyFixpointQuery(tree: Tree): Doc = {
    val children = filterEmpty(tree.children)
    val firstSubclauseIdx = indexOfTreeKind(children, FixpointQuerySubclauseKinds)
    ...
    if (firstSubclauseIdx < 0) return joinChildren(children, keywordReplacements: _*)

    val (head, subclauses) = children.splitAt(firstSubclauseIdx)
    val headDoc = joinChildren(head, keywordReplacements: _*)
    val tailDoc = subclauses.map(prettyChild).reduceLeftOption(_ <|> _).getOrElse(empty)
    localLayout(tree) {
      nest(Indent, headDoc <|> tailDoc)
    }
  }
  ...
  /**
    ...
    *
    * @param children the header children to join
    * @return the formatted header as Doc
    */
  private def declHeaderJoin(children: Array[SyntaxTree.Child]): Doc =
    spaceJoin(children,
      noSpacePairs = Set((TreeKind.Ident, TreeKind.TypeParameterList)),
      noSpaceBefore = Set(TokenKind.Colon, TokenKind.BracketL, TokenKind.BracketR),
      noSpaceAfter = Set(TokenKind.BracketL))

  private def instanceHeaderJoin(children: Array[SyntaxTree.Child]): Doc =
    joinWithConstraintBreak(children, parts =>
      spaceJoin(parts,
        noSpacePairs = Set((TreeKind.Ident, TreeKind.TypeParameterList)),
  ...
  private def buildSig(parts: Array[SyntaxTree.Child]): Doc =
    joinWithConstraintBreak(parts, sigParts =>
      spaceJoin(sigParts,
  ...
  private def prettyDef(tree: Tree): Doc = {
    ...
    splitAtToken(rest, TokenKind.Equal) match {
      case None =>
        prepend(annDoc, buildSig(rest))
      case Some((signatureParts, equalityToken, bodyParts)) =>
        val sig = buildSig(signatureParts)
        val body = bodyParts.map(prettyChild).reduceLeftOption(_ <> _).getOrElse(empty)

        val bodyIsPreserved = bodyParts.exists(isPreservedBodyKind)
        val bodyOnSameLine = bodyStartsOnSameLineAs(equalityToken, bodyParts)

        val defDoc = Doc.setLayout(layoutOfChildren(rest),
          attachBody(sig <+> text("="), body, bodyIsPreserved, bodyOnSameLine))
      ...
      case Some((sigParts, eqToken, bodyParts)) =>
        ...
        bodyParts.headOption.flatMap(unwrapToStatement) match {
          ...
          case Some(stmt) =>
            ...
            splitAtToken(stmtChildren, TokenKind.Semi) match {
              case None => prettyDef(tree)

              case Some((bodyExprs, semiToken, contExprs)) =>
                ...
                val bodyOnSameLine = bodyStartsOnSameLineAs(eqToken, bodyExprs)

                val defDoc = Doc.setLayout(defLayout, {
                  val bodyPart =
                    if (bodyIsBraced && bodyOnSameLine) sig <+> text("=") <+> bodyDoc
                    else sig <+> text("=") <> nest(Indent, line <> bodyDoc)
  ...
  /**
    ...
    */
  private def prettyBinary(tree: Tree): Doc = tree.children match {
    case Array(left, op, right) =>
      val opKind = leftMostToken(op).map(_.kind)
      if (opKind.exists(RightAssocBinaryKinds.contains) && hasMatchingChainedRhs(right, opKind.get))
        prettyRightAssocChain(tree, opKind.get)
      else {
        val leftDoc = prettyChild(left)
        val opDoc = prettyChild(op)
        val rightDoc = prettyChild(right)
        val leftEndsWithBrace = rightMostToken(left).exists(_.kind == TokenKind.CurlyR)
        val operatorEndsWithComment = endsWithComment(op)
        val sep = if (operatorEndsWithComment) hardline else space
        localLayout(tree) {
          if (leftEndsWithBrace)
            leftDoc <> space <> opDoc <> sep <> rightDoc
          else if (operatorEndsWithComment)
            leftDoc <+> nest(Indent, opDoc <> hardline <> rightDoc)
          else
            leftDoc <> nest(Indent, line <> opDoc <> sep <> rightDoc)
        }
      }
    case other =>
      other.map(prettyChild).reduceLeftOption(_ <+> _).getOrElse(empty)
  }
  ...
  private def unwrapExpr(child: SyntaxTree.Child): SyntaxTree.Child = child match {
    case t: Tree if t.kind == TreeKind.Expr.Expr =>
      filterEmpty(t.children) match {
        case Array(single) => unwrapExpr(single)
        case _ => child
      }
    case _ => child
  }
  ...
  private def hasMatchingChainedRhs(rhs: SyntaxTree.Child, opKind: TokenKind): Boolean =
    unwrapExpr(rhs) match {
      case t: Tree if t.kind == TreeKind.Expr.Binary =>
        filterEmpty(t.children) match {
          case Array(_, op, _) => leftMostToken(op).map(_.kind).contains(opKind)
          case _ => false
        }
      case _ => false
    }
  ...
  private def prettyRightAssocChain(tree: Tree, opKind: TokenKind): Doc = {
    // Flatten the right-leaning Binary spine into parallel operand/operator
    // lists. `operands` always holds exactly one more element than `operators`.
    @tailrec
    def collect(
      current: SyntaxTree.Child,
      operands: List[SyntaxTree.Child],
      operators: List[SyntaxTree.Child]
    ): (List[SyntaxTree.Child], List[SyntaxTree.Child]) =
      unwrapExpr(current) match {
        case t: Tree if t.kind == TreeKind.Expr.Binary =>
          filterEmpty(t.children) match {
            case Array(left, op, right) if leftMostToken(op).map(_.kind).contains(opKind) =>
              collect(right, operands :+ left, operators :+ op)
            case _ =>
              (operands :+ current, operators)
          }
        case _ =>
          (operands :+ current, operators)
      }
    ...
    filterEmpty(tree.children) match {
      case Array(left, op, right) =>
        val (operands, operators) = collect(right, List(left), List(op))
        val firstDoc = prettyChild(operands.head)
        val chainTail = operators.zip(operands.tail).foldLeft(empty: Doc) {
          case (accumulated, (operator, operand)) =>
            val sep = if (endsWithComment(operator)) hardline else space
            accumulated <> line <> prettyChild(operator) <> sep <> prettyChild(operand)
        }
        localLayout(tree) {
          firstDoc <> nest(Indent, chainTail)
        }
      case _ => prettyFallback(tree)
    }
  }
  ...
  private def prettyLetMatch(tree: Tree): Doc = localLayout(tree) {
    ...
    splitAtToken(children, TokenKind.Equal) match {
      case None =>
        joinChildren(children,
      ...
      case Some((headParts, eqToken, rest)) =>
        ...
        val stmtSplit: Option[(Array[SyntaxTree.Child], Array[SyntaxTree.Child], Token)] =
          rest.headOption.flatMap(unwrapToStatement).flatMap { stmt =>
            splitAtToken(filterEmpty(stmt.children), TokenKind.Semi).map {
              case (bodyExprs, semiToken, contExprs) => (bodyExprs, contExprs, semiToken)
            }
          }
        ...
        stmtSplit match {
          case Some((bodyExprs, contExprs, semiToken)) =>
            ...
            val bodyPart =
              ...
              else headDoc <+> text("=") <> nest(Indent, hardline <> bodyDoc)
            if (contExprs.isEmpty) bodyPart <> text(";")
            else bodyPart <> text(";") <> contSep <> contDoc
          ...
          case None =>
            ...
            val (bodyParts, tailParts) =
              splitAtToken(rest, TokenKind.Semi) match {
                case None => (rest, Array.empty[SyntaxTree.Child])
                case Some((before, semi, after)) => (before, semi +: after)
              }
  ...
  private def joinSemiPreservingBlanks(children: Array[SyntaxTree.Child]): Doc =
    children.indices.foldLeft((empty: Doc, Option.empty[SyntaxTree.Child])) {
      case ((result, prev), i) =>
        ...
        child match {
          case token: Token if token.kind == TokenKind.Semi =>
            ...
            val semiDoc =
              if (hasSameLineComment) text(";") <> space
              else {
            ...
            (result <> gap <> semiDoc, Some(child))
          case _ =>
            val gap =
              if (prev.exists(endsWithComment))
                if (hadBlankLineBetween(prev.get, child)) hardline <> hardline else hardline
              else empty
            (result <> gap <> prettyChild(child), Some(child))
        }
    }._1
  ...
  private def prettyForeach(tree: Tree): Doc =
    splitAtBracket(tree.children) match {
      ...
      case Some(BracketSplit(header, open, body, close, tail)) =>
        ...
        val tailSep = if (isBlock) space <> tailDoc else nest(Indent, line <> tailDoc)
        if (parensSameLine(tree.children) && layoutOfChildren(body) == Layout.SingleLine) {
          ...
          Doc.setLayout(Layout.MultiLine,
            headerDoc <+> text(open) <>
              nest(Indent, Doc.layoutChoice(empty, line) <> bodyDoc) <>
              closeSep <>
              text(close) <>
              tailSep
          )
  ...
  private def prettyLambda(tree: Tree): Doc =
    splitAtToken(filterEmpty(tree.children), TokenKind.ArrowThinRWhitespace, TokenKind.ArrowThinRTight) match {
      case None => prettyFallback(tree)
      case Some((header, _, body)) =>
        ...
        localLayout(tree) {
          ...
          else headerDoc <+> text("->") <> nest(Indent, line <> bodyDoc)
        }
    }
  ...
  private def prettyNewObject(tree: Tree): Doc =
    splitAtBracket(tree.children) match {
      ...
      case Some(BracketSplit(header, open, body, close, tail)) =>
        ...
        localLayout(tree) {
          prefix <+> text(open) <>
            nest(Indent, line <> inner) <>
            line <> text(close) <> suffix
        }
    }

  /**
    * Formatting for `new` expressions left ambiguous by the parser.
    * A `@` token marks a struct literal (`new S @ rc { f = e }`); its absence
    * marks an anonymous object (`new T { def ... }`).
    *
    * @param tree the ambiguous new expression tree
    * @return the formatted new expression as Doc
    */
  private def prettyAmbiguousNew(tree: Tree): Doc = {
    val isStructLiteral = containsToken(tree.children, TokenKind.At)
    if (isStructLiteral) prettyCommaBracket(tree) else prettyNewObject(tree)
  }
  ...
  /**
    ...
    * @param keyword the keyword string to use in the output
    * @return the formatted construct as Doc
    */
  private def keywordSpaced(tree: Tree, kind: TokenKind, keyword: String): Doc =
    joinChildren(tree.children, kind -> (text(keyword) <> space))
  ...
  private def prettyIdent(tree: Tree): Doc = {
    ...
    val mainDoc = mainChildren.foldLeft((empty: Doc, Option.empty[SyntaxTree.Child])) {
      case ((result, prev), child) =>
        ...
        (result <> gap <> childDoc, Some(child))
    }._1
    trailingComments match {
      case Some(commentList) => appendTrailingCommentList(mainDoc, mainChildren.lastOption, commentList)
      case None => mainDoc
  ...
  private def prettyOperator(tree: Tree): Doc = {
    ...
    if (children.isEmpty) empty
    else children.indices.tail.foldLeft(prettyChild(children.head)) { (result, i) =>
      ...
      result <> sep <> prettyChild(nextChild)
    }
  }
  ...
  private def prettyParameterList(tree: Tree): Doc =
    prettyBracket(tree, tree.children,
      formatBody = children => joinChildren(children, TokenKind.Comma -> (text(",") <> line)))
  ...
  private def prettyArgumentList(tree: Tree): Doc = {
    val hasArgs = tree.children.exists(child =>
      isTree(child, TreeKind.Argument) || isTree(child, TreeKind.ArgumentNamed))
    ...
    if (!hasArgs) {
      splitAtBracket(filtered) match {
        ...
        case Some(BracketSplit(_, _, _, _, tail)) =>
          val tailDoc = tailAfterClose(filtered, tail)
    ...
    splitAtBracket(filtered) match {
      ...
      case Some(BracketSplit(header, open, body, close, tail)) =>
        ...
        val singleBlockArg = args.length == 1 &&
          filterEmpty(args.head.children).headOption.exists(child =>
            leftMostToken(child).exists(token => token.kind == TokenKind.CurlyL || token.kind == TokenKind.HashCurlyL))
        localLayout(tree) {
          if (singleBlockArg)
            openDoc <> bodyDoc <> text(close) <> tailDoc
          else
            openDoc <> nest(Indent, Doc.layoutChoice(empty, line) <> bodyDoc) <> Doc.layoutChoice(empty, line) <> text(close) <> tailDoc
        }
    }
  }
  ...
  private def prettyUseOrImportList(tree: Tree): Doc = {
    ...
    items.tail.foldLeft((traverse(items.head), items.head.kind: TreeKind)) {
      case ((result, prevKind), item) =>
        val sep = if (item.kind != prevKind) hardline <> hardline else hardline
        (result <> sep <> traverse(item), item.kind)
    }._1
  }
      ...
      child match {
        case token: Token if replMap.contains(token.kind) =>
          ...
          if (hasSameLineComment) {
            result = result <> gap <> text(token.text)
            prevReplEndsWithLine = false
          } else {
            result = result <> gap <> replDoc
            prevReplEndsWithLine = docEndsWithLine(replDoc)
          }
          prev = Some(token)
          i += 1
        case _ =>
          val isComment = leftMostToken(child).exists(t => isCommentKind(t.kind))
          ...
          val gap = if (prev.isEmpty) empty
          else if (prevReplEndsWithLine) {
  ...
  private def joinWithGap(children: Array[SyntaxTree.Child]): Doc = {
    if (children.isEmpty) return empty
    children.sliding(2).foldLeft(prettyChild(children.head)) {
      case (result, Array(prev, next)) => result <> structuralGap(prev, next) <> prettyChild(next)
      case (result, _) => result
    }
  }
  ...
  private def joinWithGap(children: Array[SyntaxTree.Child], gap: Doc): Doc = {
    ...
    children.sliding(2).foldLeft(prettyChild(children.head)) {
      case (result, Array(prev, next)) =>
        ...
        val separator = if (endsWithComment(prev))
        ...
        result <> separator <> prettyChild(next)
      case (result, _) => result
    }
  }

  /**
    * Attaches a body to a header, choosing how the body hangs off the header.
    *
    * This is the shared shape used by definitions ([[prettyDef]]), run-with
    * rules ([[alignedRunWithRule]]) and arrow rules ([[prettyArrowRuleFragment]]):
    *   - a body that is "preserved" (a braced/apply/if-then-else construct) and
    *     started on the same source line stays inline after a space;
    *   - any other preserved body hangs on a hard newline;
    *   - everything else hangs on a soft [[line]].
    *     `head` already includes any separator token (e.g. the `=` or `=>`).
    */
  private def attachBody(head: Doc, body: Doc, bodyIsPreserved: Boolean, bodyOnSameLine: Boolean): Doc =
    if (bodyIsPreserved && bodyOnSameLine) head <+> body
    else if (bodyIsPreserved) head <> nest(Indent, hardline <> body)
    else head <> nest(Indent, line <> body)

  /**
    * Joins `parts` with `join`, but if a [[TreeKind.Type.ConstraintList]]
    * starts on a later source line than the preceding part, breaks before it
    * with a hard newline and an extra indent. Shared by signatures
    * ([[buildSig]]) and instance headers ([[instanceHeaderJoin]]).
    */
  private def joinWithConstraintBreak(
    parts: Array[SyntaxTree.Child],
    join: Array[SyntaxTree.Child] => Doc
  ): Doc = {
    val constraintIdx = indexOfTreeKind(parts, Set(TreeKind.Type.ConstraintList))
    ...
    if (onNewLine) {
      ...
      join(before) <> nest(Indent, hardline <> join(after))
    } else {
      join(parts)
    }
  }
  ...
  /**
    * Folds already-formatted `docs` (paired positionally with their source
    * `children`) into one document, inserting a [[structuralGap]] between each
    * adjacent pair. Shared by the aligned bodies of matches, structs and handlers.
    */
  private def joinDocsWithGap(children: Array[SyntaxTree.Child], docs: Array[Doc]): Doc = {
    val pairs = children.zip(docs)
    if (pairs.isEmpty) empty
    else pairs.sliding(2).foldLeft(pairs.head._2) {
      case (result, Array((prev, _), (next, nextDoc))) => result <> structuralGap(prev, next) <> nextDoc
      case (result, _) => result
    }
  }

  /**
    * Formats the children appearing after a closing bracket, attaching them to
    * the close token with the appropriate gap (see [[bracketTail]]). Returns
    * [[empty]] when there is no tail. Shared by case bodies and argument lists.
    */
  private def tailAfterClose(children: Array[SyntaxTree.Child], tail: Array[SyntaxTree.Child]): Doc =
    if (tail.isEmpty) empty
    else {
      val closeIdx = children.length - tail.length - 1
      val closeChild = if (closeIdx >= 0 && closeIdx < children.length) Some(children(closeIdx)) else None
  ...
  private def defaultHeaderJoin(children: Array[SyntaxTree.Child]): Doc =
    children.map(prettyChild).reduceLeftOption(_ <+> _).getOrElse(empty)
    ...
    children.sliding(2).foldLeft(prettyChild(children.head)) {
      case (result, Array(prev, next)) =>
        val noSpace = (prev, next) match {
          ...
          case _ if leftMostToken(next).exists(t => noSpaceBefore.contains(t.kind)) => true
          case _ if rightMostToken(prev).exists(t => noSpaceAfter.contains(t.kind)) => true
          case _ => false
        }
  ...
  private def parensSameLine(children: Array[SyntaxTree.Child]): Boolean =
    (firstToken(children, TokenKind.ParenL), lastToken(children, TokenKind.ParenR)) match {
      case (Some(open), Some(close)) => open.start.lineOneIndexed == close.end.lineOneIndexed
      case _ => true
    }
