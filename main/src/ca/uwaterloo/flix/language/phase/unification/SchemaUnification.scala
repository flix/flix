...
import ca.uwaterloo.flix.language.ast.{Kind, RigidityEnv, Type, TypeConstructor, Symbol}
...
object SchemaUnification {
  ...
  /**
    ...
    */
  def unifyRows(tpe1: Type, tpe2: Type, renv: RigidityEnv, senv: Set[Symbol.KindedTypeVarSym])(implicit flix: Flix): Result[Substitution, UnificationError] = (tpe1, tpe2) match {

    case (tvar: Type.Var, tpe) => Unification.unifyVar(tvar, tpe, renv, senv).map(_._1) // TODO ASSOC-TYPES

    case (tpe, tvar: Type.Var) => Unification.unifyVar(tvar, tpe, renv, senv).map(_._1) // TODO ASSOC-TYPES
    ...
    case (row1@Type.Apply(Type.Apply(Type.Cst(TypeConstructor.SchemaRowExtend(_), _), _, _), restRow1, _), row2) =>
      ...
      rewriteSchemaRow(row2, row1, renv, senv) flatMap {
  ...
  private def rewriteSchemaRow(rewrittenRow: Type, staticRow: Type, renv: RigidityEnv, senv: Set[Symbol.KindedTypeVarSym])(implicit flix: Flix): Result[(Substitution, Type), UnificationError] = {
    ...
    def visit(row: Type): Result[(Substitution, Type), UnificationError] = (row, staticRow) match {
      ...
      Type.Apply(Type.Apply(Type.Cst(TypeConstructor.SchemaRowExtend(label1), _), fieldType1, _), _, _)) =>
        ...
        if (label1 == label2) {
          ...
          for {
            (subst, econstrs) <- unifyTypes(fieldType1, fieldType2, renv, senv) // TODO ASSOC-TYPES consider econstrs
