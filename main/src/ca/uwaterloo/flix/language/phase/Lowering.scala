...
object Lowering {
  ...
  private def visitExp(exp0: TypedAst.Expr)(implicit scope: Scope, root: TypedAst.Root, flix: Flix): LoweredAst.Expr = exp0 match {
    ...
    case TypedAst.Expr.JvmType(exp, proxy, tpe, eff, loc) =>
      val e = visitExp(exp)
      val t = visitType(tpe)
      LoweredAst.Expr.JvmType(e, proxy, t, eff, loc)
  ...
  private def substExp(exp0: LoweredAst.Expr, subst: Map[Symbol.VarSym, Symbol.VarSym]): LoweredAst.Expr = exp0 match {
    ...
    case LoweredAst.Expr.JvmType(exp, proxy, tpe, eff, loc) =>
      val e = substExp(exp, subst)
      LoweredAst.Expr.JvmType(e, proxy, tpe, eff, loc)
