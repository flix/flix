  ...
  object Expressions {
    ...
    def weed(exp0: ParsedAst.Expression)(implicit flix: Flix): Validation[WeededAst.Expression, WeederError] = {
      ...
      def visit(e0: ParsedAst.Expression, unsafe: Boolean): Validation[WeededAst.Expression, WeederError] = e0 match {
        ...
        case ParsedAst.Expression.VectorLoad(exp1, index, sp2) =>
          val sp1 = leftMostSourcePosition(exp1)
          val loc = mkSL(sp1, sp2)
        ...
        case ParsedAst.Expression.VectorSlice(exp1, index, optindex, sp2) =>
          val sp1 = leftMostSourcePosition(exp1)
          val loc = mkSL(sp1, sp2)
        ...
        case ParsedAst.Expression.VectorSliceNoEndIndex(exp1, index, sp2) =>
          val sp1 = leftMostSourcePosition(exp1)
          val loc = mkSL(sp1, sp2)
          visit(exp1, unsafe) flatMap {
            case e => index match {
        ...
        case ParsedAst.Expression.VectorSliceNoStartIndex(exp1, index, sp2) =>
          val sp1 = leftMostSourcePosition(exp1)
          val loc = mkSL(sp1, sp2)
          visit(exp1, unsafe) flatMap {
            case e => index match {
...
7
  ...
  private def leftMostSourcePosition(tpe: ParsedAst.Type): SourcePosition = tpe match {
    ...
    case ParsedAst.Type.Succ(sp1, _, _) => sp1
