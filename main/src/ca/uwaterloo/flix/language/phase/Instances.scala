...
object Instances {
  ...
  private def visitInstances(root: TypedAst.Root)(implicit flix: Flix): Validation[Unit, InstanceError] = {
    ...
    def checkOrphan(inst: TypedAst.Instance): Validation[Unit, InstanceError] = inst match {
      case TypedAst.Instance(_, _, sym, tpes, _, _, ns, _) => tpes.last.typeConstructor match { // MATT here we only check the last type
        ...
        case _ => InstanceError.OrphanInstance(tpes.last, sym, sym.loc).toFailure
      }
    }
    ...
    def checkSimple(inst: TypedAst.Instance): Validation[Unit, InstanceError] = inst match {
      case TypedAst.Instance(_, _, sym, tpes, _, _, _, _) => tpes.last match { // MATT only the last one needs to be simple
        case _: Type.Cst => ().toSuccess
        case _: Type.KindedVar => InstanceError.ComplexInstanceType(tpes.last, sym, sym.loc).toFailure
        case _: Type.Apply =>
          Validation.fold(tpes.last.typeArguments, List.empty[Type.KindedVar]) {
            ...
            case (_, _) => InstanceError.ComplexInstanceType(tpes.last, sym, sym.loc).toFailure
          }.map(_ => ())
    ...
    def checkOverlap(inst1: TypedAst.Instance, inst2: TypedAst.Instance)(implicit flix: Flix): Validation[Unit, InstanceError] = {
      Unification.unifyTypes(generifyBools(inst1.tpes.last), inst2.tpes.last) match { // MATT only care about last type
        case Ok(_) =>
    ...
    def findInstanceForType(tpe: Type, clazz: Symbol.ClassSym): Option[(Ast.Instance, Substitution)] = {
        ...
        superInsts.iterator.flatMap {
          superInst => Unification.unifyTypes(tpe, superInst.tpes.last).toOption.map((superInst, _))
        }.nextOption()
    }
    ...
    def checkSuperInstances(inst: TypedAst.Instance): Validation[Unit, InstanceError] = inst match {
      case TypedAst.Instance(_, _, sym, tpes, tconstrs, _, _, _) =>
          ...
          superClass =>
            ...
            findInstanceForType(tpes.last, superClass) match {
              ...
              case None =>
                ...
                InstanceError.MissingSuperClassInstance(tpes.last, sym, superClass, sym.loc).toFailure
            }
        }
    }
