  ...
  object Expressions {
    ...
    def Primary: Rule1[ParsedAst.Expression] = rule {
      Static | Scope | LetMatch | LetMatchStar | LetRecDef | LetUse | LetImport | IfThenElse |
        ...
        Upcast | Supercast | Mask | Intrinsic | New | ArrayLit | VectorLit | FList |
    ...
    def VectorLit: Rule1[ParsedAst.Expression] = rule {
      SP ~ atomic("Array") ~ atomic("#{") ~ optWS ~ zeroOrMore(Expression).separatedBy(optWS ~ "," ~ optWS) ~ optWS ~ "}" ~ SP ~> ParsedAst.Expression.VectorLit
    }
