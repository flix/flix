...
object TypeBuiltinCompleter extends Completer {
  ...
  /**
    ...
    */
  override def getCompletions(implicit context: CompletionContext, index: Index, root: Option[TypedAst.Root], delta: DeltaContext): Iterable[TypeBuiltinCompletion] = {
    if (root.isEmpty) {
      return Nil
    }
