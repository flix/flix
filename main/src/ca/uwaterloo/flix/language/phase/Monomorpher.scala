...
/**
  ...
  *
  * Monomorphization eliminates all type variables and normalizes types. This means that equivalent
  * types have unique representations, without aliases and associated types. Unconstrained type
  * variables are instantiated arbitrarily and things like record types and effect formulas are
  * flattened and ordered. This means that `{b = String, a = String}` becomes
  * `{a = String, b = String}` and that `Print + (Crash + Print)` becomes `Crash + Print`.
  *
  * Monomorphization resolves type class methods into actual function calls and resolves type-match
  * expressions.
  *
  ...
  * types and aliases etc.
  *
  * At a high-level, monomorphization works as follows:
  *
  *   1. Maintain a queue of functions and the concrete, normalized types they must be
  *      specialized to.
  *   2. Initialize the queue based on non-parametric function definitions.
  *   3. Iteratively extract a function from the queue and specialize it:
  *      a. Replace every type variable appearing anywhere in the definition by its concrete
  *         type.
  *      b. Create new fresh local variable symbols (since the function is effectively being
  *         copied).
  *      c. Enqueue (or re-use) other functions referenced by the current function which require
  *         specialization.
  *   4. Reconstruct the AST from the specialized functions and remove all parametric functions.
  *
  * Type normalization details:
  *   - Record fields are in alphabetical order
  *   - Schema fields are in alphabetical order
  *   - Effect formulas are flat unions of effects in alphabetical order or a negation thereof.
  *   - Case set formulas are a single CaseSet literal or a negation thereof.
  */
object Monomorpher {
  ...
  /**
    ...
    * replaced by an appropriate default type. If you could not choose the type arbitrarily, the
    * type would not be polymorphic after type-inference. Additionally the output types of the
    * substitution are normalized.
    *
  ...
  private case class StrictSubstitution(s: Substitution) {
    ...
    /** Applies `this` to `tpe`, returning a normalized type. */
    def apply(tpe0: Type)(implicit root: LoweredAst.Root, flix: Flix): Type = {
      // Performance Note:
      // We are on a hot path. We take extra care to avoid redundant type objects.
      tpe0 match {
        case v@Type.Var(sym, _) => s.m.get(sym) match {
          case None => default(v) // Variable unbound. Use the default type.
        ...
        case assoc0@Type.AssocType(symUse, arg0, kind, loc) =>
          val arg = apply(arg0)
          val assoc1 = if (arg eq arg0) assoc0 else Type.AssocType(symUse, arg, kind, loc)
          val reducedType = reduceAssocType(assoc1)
          // `reducedType` is already ground, but might need normalization.
  ...
  private object StrictSubstitution {
    ...
    /**
      * Returns a [[StrictSubstitution]], instantiating variables in the output of `s` to their
      * default type and converting them to normal form.
      */
  ...
  private class Context {
    ...
    /** Dequeues all elements from the queue and clearing it. */
    def dequeueAllSpecializations: Array[(Symbol.DefnSym, LoweredAst.Def, StrictSubstitution)] = synchronized {
    ...
    /**
      ...
      */
    private val specializedNames: mutable.Map[(Symbol.DefnSym, Type), Symbol.DefnSym] = mutable.Map.empty

    /** Returns the specialized def symbol for `sym` w.r.t. `tpe` if it exists. */
    def getSpecializedName(sym: Symbol.DefnSym, tpe: Type): Option[Symbol.DefnSym] = synchronized {
      specializedNames.get((sym, tpe))
    }

    /** Adds a specialized name, `specializedSym`, for `sym` w.r.t `tpe`. */
    def addSpecializedName(sym: Symbol.DefnSym, tpe: Type, specializedSym: Symbol.DefnSym): Unit = synchronized {
      specializedNames.put((sym, tpe), specializedSym)
    }
    ...
    def addSpecializedDef(defn: MonoAst.Def): Unit = synchronized {
      specializedDefns.put(defn.sym, defn)
    }

    /** Returns the specialized definitions as an immutable map. */
    def getSpecializedDefs: Map[Symbol.DefnSym, MonoAst.Def] = synchronized {
  ...
  private def mkSchemaExtendSorted(label: Name.Pred, tpe: Type, rest: Type, loc: SourceLocation): Type = rest match {
    ...
    case Type.Cst(_, _) | Type.Apply(_, _, _) | Type.Var(_, _) =>
      // Non-schema related types or schema in correct order.
  ...
  /** Performs monomorphization of `root`. */
  def run(root: LoweredAst.Root)(implicit flix: Flix): MonoAst.Root = flix.phase("Monomorpher") {
    ...
    // This is an over-approximation of the entry-point functions, so those are not deleted.
    val nonPolymorphicDefns = root.defs.filter {
      case (_, defn) => defn.spec.tparams.isEmpty
    }

    // Specialize `nonPolymorphicDefns` which will specialization tasks to context as needed.
    ParOps.parMap(nonPolymorphicDefns) {
      case (sym, defn) =>
        // It is important that non-parametric functions keep their symbol to not invalidate the set
        // of entryPoints functions.
        val defnSym = sym
        // `defn` is non-polymorphic, so `empty` works.
        val subst = StrictSubstitution.empty
        ctx.addSpecializedDef(specializeDef(defnSym, defn, subst))
    }

    // Repeatedly specialize the enqueued tasks until there are no more.
    while (ctx.nonEmptySpecializationQueue) {
      val queue = ctx.dequeueAllSpecializations
      ParOps.parMap(queue) {
        case (freshSym, defn, subst) => ctx.addSpecializedDef(specializeDef(freshSym, defn, subst))
      }
    }

    // Now the defs are specialized.
    // For effects, structs, and enums we leave them polymorphic but still simplify their
    // polymorphic types.

    // Remove aliases, associated types, and jvm types from effects.
    ...
    // Remove aliases, associated types, and jvm types from enums.
    ...
    // Remove aliases, associated types, and jvm types from structs.
  ...
  /** Creates an instance map for direct lookups. */
  ...
  private def visitEffectOpSpec(spec: LoweredAst.Spec)(implicit root: LoweredAst.Root, flix: Flix): MonoAst.Spec = spec match {
    case LoweredAst.Spec(doc, ann, mod, _, fparams0, declaredScheme, retTpe, eff, _) =>
      // Effect operations do not have variables, so we can use full type normalization instead of
      // simplify.
      ...
      MonoAst.Spec(
        doc, ann, mod, fparams, declaredScheme.base, StrictSubstitution.empty(retTpe), StrictSubstitution.empty(eff)
      )
  }

  /** Returns the specialized `defn` according to `subst` with the name `freshSym`. */
  private def specializeDef(freshSym: Symbol.DefnSym, defn: LoweredAst.Def, subst: StrictSubstitution)(implicit ctx: Context, instances: Map[(Symbol.TraitSym, TypeConstructor), Instance], root: LoweredAst.Root, flix: Flix): MonoAst.Def = {
    ...
    val spec = defn.spec
    val specializedSpec = MonoAst.Spec(
      spec.doc,
      spec.ann,
      spec.mod,
      specializedFParams,
    ...
    val specializedDefn = MonoAst.Def(freshSym, specializedSpec, specializedExp, defn.loc)

    specializedDefn
  }

  /**
    * Specializes `exp0`, renaming variables with `env0` and instantiating types with `subst`.
    *
    * Creates fresh names for all variables
    *
    * Replaces every reference to a polymorphic function with a reference to its specialized
    * version, enqueuing that specialization if not done already.
    */
  private def specializeExp(exp0: LoweredAst.Expr, env0: Map[Symbol.VarSym, Symbol.VarSym], subst: StrictSubstitution)(implicit ctx: Context, instances: Map[(Symbol.TraitSym, TypeConstructor), Instance], root: LoweredAst.Root, flix: Flix): MonoAst.Expr = exp0 match {
    ...
    case LoweredAst.Expr.Scope(sym, regionVar, exp, tpe, eff, loc) =>
      ...
      MonoAst.Expr.Scope(freshSym, regionVar, e, subst(tpe), subst(eff), loc)
    ...
    case LoweredAst.Expr.Match(exp, rules, tpe, eff, loc) =>
      ...
      MonoAst.Expr.Match(e, rs, subst(tpe), subst(eff), loc)
    ...
    case LoweredAst.Expr.TypeMatch(exp, rules, tpe, _, loc) =>
      ...
      // Make the type variables in `exp`'s type rigid so that `Nil: List[x%123]` can only match
      // `List[_]`.
      ...
      }.get // Safe because the last case will always match.
    ...
    case LoweredAst.Expr.Cast(exp, _, _, tpe, eff, loc) =>
      // Drop the declared type and effect.
      val e = specializeExp(exp, env0, subst)
