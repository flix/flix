  ...
  private def visitExp(exp0: ReducedAst.Expr)(implicit ctx: SharedContext, flix: Flix): ErasedAst.Expr = exp0 match {
    ...
    case ReducedAst.Expr.ApplyAtomic(op, exps, tpe, purity, loc) =>
      ...
      op match {
        ...
        case AtomicOp.StructNew(_, fields, mutability) =>
          ...
          ErasedAst.Expr.ApplyAtomic(AtomicOp.StructNew(specializedStruct.sym, specializedFields, mutability), es, t, purity, loc)
