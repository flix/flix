...
object Indexer {
  ...
  private def visitExp(exp0: Expression): Index = exp0 match {
    ...
    case Expression.RecordSelect(exp, field, _, _, _) =>
      Index.occurrenceOf(field) ++ Index.useOf(field) ++ visitExp(exp) ++ Index.of(exp0)

    case Expression.RecordExtend(field, exp1, exp2, _, _, _) =>
      Index.occurrenceOf(field) ++ Index.defOf(field) ++ visitExp(exp1) ++ visitExp(exp2) ++ Index.of(exp0)

    case Expression.RecordRestrict(field, exp, _, _, _) =>
      Index.occurrenceOf(field) ++ Index.defOf(field) ++ visitExp(exp) ++ Index.of(exp0)
  ...
  private def visitHead(h0: Predicate.Head): Index = h0 match {
    case a@Head.Atom(pred, den, terms, tpe, _) => Index.occurrenceOf(a) ++ Index.useOf(pred) ++ visitExps(terms)
  ...
  private def visitBody(b0: Predicate.Body): Index = b0 match {
    case b@Body.Atom(pred, den, _, terms, tpe, _) => Index.occurrenceOf(b) ++ Index.useOf(pred) ++ visitPats(terms)
