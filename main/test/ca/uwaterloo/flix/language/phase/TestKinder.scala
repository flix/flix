...
class TestKinder extends AnyFunSuite with TestUtils {
  ...
  test("KindError.StructMissingTypeParams.01") {
    val input =
      """
        |struct S { }
        |""".stripMargin
    val result = compile(input, DefaultOptions)
    expectError[KindError.MissingTParams](result)
  }

  test("KindError.NonExistentStruct.01") {
    val input =
      """
        |def f(): Unit = {
        |    region rc {
        ...
        |""".stripMargin
    val result = compile(input, DefaultOptions)
    expectError[KindError.NonExistentStruct](result)
  }


  test("KindError.MissingStructField.01") {
    val input =
      """
        |mod S {
        |    struct S[r] { }
        ...
        |""".stripMargin
    val result = compile(input, DefaultOptions)
    expectError[KindError.NonExistentStructField](result)
  }

  test("KindError.MissingStructField.02") {
    val input =
      """
        |mod S {
        |    struct S[r] { }
  ...
  test("KindError.TooFewFields.01") {
    val input = """
      |struct S[r] {
      |    a: Int32
      |}
      ...
      |""".stripMargin
    val result = compile(input, DefaultOptions)
    expectError[KindError.UnprovidedStructFields](result)
  }

  test("KindError.TooManyFields.01") {
    val input = """
                  |struct S[r] {
                  |    a: Int32
                  |}
                  ...
                  |""".stripMargin
    val result = compile(input, DefaultOptions)
    expectError[KindError.ExtraStructFields](result)
  }
}
