...
object FormatExpression {
  ...
  def format(e0: TypedAst.Expression): String = e0 match {
    ...
    case TypedAst.Expression.UncheckedCast(exp, declaredType, declaredPur, declaredEff, tpe, eff, _, _) => s"Cast($exp, $declaredType, $declaredPur, $declaredEff, $tpe, $eff)"
    case TypedAst.Expression.UncheckedMaskingCast(exp, _, _, _, _) => s"Mask($exp)"
