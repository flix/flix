...
class TestResolver extends AnyFunSuite with TestUtils {
  ...
  test("AccessibleModule.01") {
    // A private module is accessible from its parent.
    val input =
      s"""
         |mod A {
         |    mod B {
  ...
  test("AccessibleModule.02") {
    // A private module is accessible from a sibling: `B` and `C` are both declared in `A`.
    val input =
      s"""
         |mod A {
         |    mod B {
  ...
  test("AccessibleModule.03") {
    // A private module is accessible from a descendant of a sibling.
    val input =
      s"""
         |mod A {
         |    mod B {
  ...
  test("AccessibleModule.04") {
    // Nested private modules: `A.B.C` is accessible from its sibling `A.B.D`.
    val input =
      s"""
         |mod A {
         |    mod B {
  ...
  test("AccessibleModule.05") {
    // A type in a private sibling module is accessible.
    val input =
      s"""
         |mod A {
         |    mod B {
         ...
         |    }
         |    mod C {
         |        pub def f(): A.B.Color = A.B.Color.Red
         |    }
         |}
         |""".stripMargin
    val result = check(input, Options.TestWithLibNix)
  ...
  test("AccessibleModule.06") {
    // A `use` of a def in a private sibling module is accessible.
    val input =
      s"""
         |mod A {
         |    mod B {
