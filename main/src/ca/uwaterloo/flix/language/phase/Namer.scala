  ...
  object Declarations {
    ...
    def namer(decl0: WeededAst.Declaration, ns0: Name.NName, prog0: NamedAst.Root)(implicit genSym: GenSym): Validation[NamedAst.Root, NameError] = decl0 match {
      ...
      case WeededAst.Declaration.Constraint(h, bs, loc) =>
        ...
        mapN(Predicates.namer(h, headEnv, ruleEnv, tenv0), @@(bs.map(b => Predicates.namer(b, headEnv, ruleEnv, tenv0)))) {
          case (head, body) =>
      ...
      case WeededAst.Declaration.LatticeComponents(tpe, bot0, top0, equ0, leq0, lub0, glb0, loc) =>
        ...
        mapN(botVal, topVal, equVal, leqVal, lubVal, glbVal) {
          case (bot, top, equ, leq, lub, glb) =>
            val lattice = NamedAst.LatticeComponents(Types.namer(tpe, Map.empty), bot, top, equ, leq, lub, glb, ns0, loc)
            prog0.copy(latticeComponents = prog0.latticeComponents + (Types.namer(tpe, Map.empty) -> lattice)) // NB: This just overrides any existing binding.
        }
       ...
       * Relation.
       */
      case WeededAst.Declaration.Relation(doc, mod, ident, attr, loc) =>
        // check if the table already exists.
        prog0.relations.get(ns0) match {
          case None =>
            // Case 1: The namespace does not yet exist. So the table does not yet exist.
            val relation = NamedAst.Relation(doc, mod, Symbol.mkRelSym(ns0, ident), attr.map(a => Attributes.namer(a, Map.empty)), loc)
       ...
       * Lattice.
       */
      case WeededAst.Declaration.Lattice(doc, mod, ident, attr, loc) =>
        // check if the table already exists.
        prog0.lattices.get(ns0) match {
          case None =>
            // Case 1: The namespace does not yet exist. So the table does not yet exist.
            val lattice = NamedAst.Lattice(doc, mod, Symbol.mkLatSym(ns0, ident), attr.map(k => Attributes.namer(k, Map.empty)), loc)
  ...
  object Expressions {
    ...
    def namer(exp0: WeededAst.Expression, env0: Map[String, Symbol.VarSym], tenv0: Map[String, Type.Var])(implicit genSym: GenSym): Validation[NamedAst.Expression, NameError] = exp0 match {
      ...
      case WeededAst.Expression.Switch(rules, loc) =>
        val rulesVal = rules map {
          case (cond, body) => seqM(namer(cond, env0, tenv0), namer(body, env0, tenv0))
        }
