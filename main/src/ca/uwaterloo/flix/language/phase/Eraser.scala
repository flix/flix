  ...
  private def visitExp(exp0: Expr): Expr = exp0 match {
    ...
    case ApplyAtomic(op, exps, tpe, purity, loc) =>
      ...
      op match {
        ...
        case AtomicOp.VariantIs(_) => ApplyAtomic(op, es, t, purity, loc)
        case AtomicOp.VariantTag(_) => ApplyAtomic(op, es, t, purity, loc)
        case AtomicOp.VariantUntag(_) => ApplyAtomic(op, es, t, purity, loc)
