...
class ZhegalkinCache[T] {
   ...
   */
  private val cachedInterCst: ConcurrentMap[(ZhegalkinCst, ZhegalkinExpr[T]), ZhegalkinExpr[T]] = new ConcurrentHashMap(1024, 0.5f)
   ...
   */
  def lookupOrComputeInterCst(c: ZhegalkinCst, e: ZhegalkinExpr[T], mkInter: (ZhegalkinCst, ZhegalkinExpr[T]) => ZhegalkinExpr[T]): ZhegalkinExpr[T] = {
