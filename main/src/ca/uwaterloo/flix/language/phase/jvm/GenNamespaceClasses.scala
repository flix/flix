...
object GenNamespaceClasses {
  ...
  private def compileShimMethod(visitor: ClassWriter, defn: Def): Unit = {
    ...
    BackendObjType.Result.unwindSuspensionFreeThunkToType(BackendType.toErasedBackendType(defn.unboxedType.tpe), Some(s"in shim method of $name"), defn.loc)(new BytecodeInstructions.F(method))
