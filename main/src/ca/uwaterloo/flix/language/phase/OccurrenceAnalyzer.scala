...
object OccurrenceAnalyzer {
   ...
   */
  private def visitExp(exp0: LiftedAst.Expression): (OccurrenceAst.Expression, OccurInfo) = exp0 match {
   ...
   */
  private def visitExps(exps: List[LiftedAst.Expression]): (List[OccurrenceAst.Expression], OccurInfo) = {
