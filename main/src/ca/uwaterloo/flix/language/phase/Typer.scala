    ...
    def visitExp(e0: KindedAst.Expression): InferMonad[(List[Ast.TypeConstraint], Type, Type, Type)] = e0 match {
      ...
      case KindedAst.Expression.Scope(sym, varSym, tvar, exp, evar, loc) =>
        ...
        for {
          ...
          (constrs, tpe, pur, eff) <- visitExp(exp)
          purifiedPur <- purifyEffM(sym, pur)
      ...
      case KindedAst.Expression.ArrayLit(exps, exp, tvar, evar, loc) =>
        ...
        for {
          ...
          resultEff = Type.mkUnion(eff2 :: eff1, loc) // MATT need evar?
      ...
      case KindedAst.Expression.ArrayNew(exp1, exp2, exp3, tvar, evar, loc) =>
        ...
        for {
          ...
          resultEff = Type.mkUnion(List(eff1, eff2, eff3), loc) // MATT need evar?
        } yield (constrs1 ++ constrs2 ++ constrs3, resultTyp, resultPur, resultEff)
  ...
  private def reassembleExp(exp0: KindedAst.Expression, root: KindedAst.Root, subst0: Substitution): TypedAst.Expression = {
    ...
    def visitExp(exp0: KindedAst.Expression, subst0: Substitution): TypedAst.Expression = exp0 match {
      ...
      case KindedAst.Expression.Scope(sym, varSym, regionVar, exp, evar, loc) =>
        ...
        val tpe = e.tpe
        val pur = subst0(evar)
        TypedAst.Expression.Scope(sym, varSym, regionVar, e, tpe, pur, loc)
