...
object EntryPoints {
  ...
  private object WrapMain {
    ...
    private def mkEntryPoint(oldEntryPoint: TypedAst.Def, root: TypedAst.Root)(implicit flix: Flix): TypedAst.Def = {
      ...
      // `mainFunc()`
      val mainCall = TypedAst.Expr.ApplyDef(mainSym, List(mainArg), Map.empty, mainArrowType, mainReturnType, mainEffect, SourceLocation.Unknown)
      ...
      val printCall = TypedAst.Expr.ApplyDef(printSym, List(printArg), Map.empty, printArrowType, printReturnType, printCallEffect, SourceLocation.Unknown)
