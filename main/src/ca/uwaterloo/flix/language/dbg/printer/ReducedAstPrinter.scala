...
object ReducedAstPrinter {
  ...
  def print(e: ReducedAst.Expr): DocAst.Expression = e match {
    ...
    case Expr.ApplyClo(exp, exps, ct, _, _, _) => DocAst.Expression.ApplyClo(print(exp), exps.map(print), ct)
    case Expr.ApplyDef(sym, exps, ct, _, _, _) => DocAst.Expression.ApplyDef(sym, exps.map(print), ct)
