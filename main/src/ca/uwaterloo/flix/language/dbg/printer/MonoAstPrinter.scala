...
object MonoAstPrinter {
  ...
  private def print(e: MonoAst.Expr): DocAst.Expr = e match {
    ...
    case Expr.ApplyDef(sym, exps, _, _, _, _) => DocAst.Expr.ApplyDef(sym, exps.map(print).toList)
    case Expr.ApplyLocalDef(sym, exps, _, _, _) => DocAst.Expr.ApplyLocalDef(sym, exps.map(print).toList)
    case Expr.ApplyOp(sym, exps, _, _, _) => DocAst.Expr.ApplyOp(sym, exps.map(print).toList)
