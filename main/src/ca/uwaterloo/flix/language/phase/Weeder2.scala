...
object Weeder2 {
  ...
  private object Exprs {
    ...
    private def visitScopeExpr(tree: Tree)(implicit sctx: SharedContext): Validation[Expr, CompilationMessage] = {
      ...
      val blockVal = visitBlockExpr(pick(TreeKind.Expr.Block, tree))
      mapN(pickNameIdent(tree), blockVal) {
    ...
    private def visitWithoutExpr(tree: Tree)(implicit sctx: SharedContext): Validation[Expr, CompilationMessage] = {
      ...
      val effectNames = pickAll(TreeKind.QName, effectSet).map(visitQName)
      ...
      effectNames match {
        ...
        case Nil =>
          ...
          Validation.success(Expr.Error(Malformed(NamedTokenSet.Effect, SyntacticContext.Expr.OtherExpr, None, tree.loc)))
      }
    }
    ...
    private def visitFixpointLambdaExpr(tree: Tree)(implicit sctx: SharedContext): Validation[Expr, CompilationMessage] = {
      ...
      val params = (pickAll(TreeKind.Predicate.ParamUntyped, paramList) ++ pickAll(TreeKind.Predicate.Param, paramList)).sortBy(_.loc)

      mapN(traverse(params)(Predicates.visitParam), pickExpr(tree)) {
  ...
  private object Types {
    ...
    private def visitApplyType(tree: Tree)(implicit sctx: SharedContext): Validation[Type, CompilationMessage] = {
      ...
      mapN(pickType(tree), traverse(arguments)(pickType)) {
        (tpe, args) =>
          // Curry type arguments
          args.foldLeft(tpe) { case (acc, t2) => Type.Apply(acc, t2, tree.loc) }
      }
    }
  ...
  private def pickQNameIdents(tree: Tree): Validation[List[String], CompilationMessage] = {
    ...
    val result = pickAll(TreeKind.Ident, qname).flatMap(text)
  ...
  private def pick(kind: TreeKind, tree: Tree, synctx: SyntacticContext = SyntacticContext.Unknown): Tree = {
    tryPick(kind, tree) match {
      case Some(t) => t
      case None => throw InternalCompilerException("unexpected invalid tree", tree.loc)
    }
  }
