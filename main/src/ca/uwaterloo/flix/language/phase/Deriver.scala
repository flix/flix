...
import ca.uwaterloo.flix.language.ast.{Ast, Kind, KindedAst, Name, SemanticOperator, SourceLocation, Symbol, Type, TypeConstructor}
...
object Deriver {
  ...
  private def mkEqInstance(enum0: KindedAst.Enum, loc: SourceLocation, root: KindedAst.Root)(implicit flix: Flix): KindedAst.Instance = enum0 match {
    case KindedAst.Enum(_, _, _, _, tparams, _, _, tpeDeprecated, _) =>
  ...
  private def mkEqSpec(enum0: KindedAst.Enum, param1: Symbol.VarSym, param2: Symbol.VarSym, loc: SourceLocation, root: KindedAst.Root)(implicit flix: Flix): KindedAst.Spec = enum0 match {
    case KindedAst.Enum(_, _, _, _, tparams, _, _, tpeDeprecated, _) =>
      ...
      KindedAst.Spec(
        ...
        mod = Ast.Modifiers.Empty,
        tparams = tparams,
        fparams = List(KindedAst.FormalParam(param1, Ast.Modifiers.Empty, tpeDeprecated, loc), KindedAst.FormalParam(param2, Ast.Modifiers.Empty, tpeDeprecated, loc)),
        tpe = Type.mkBool(loc),
        eff = Type.Cst(TypeConstructor.True, loc),
        tconstrs = List(Ast.TypeConstraint(Ast.TypeConstraint.Head(eqClassSym, loc), tpeDeprecated, loc)),
        loc = loc
      )
  }
  ...
  private def mkCompareSpec(enum0: KindedAst.Enum, param1: Symbol.VarSym, param2: Symbol.VarSym, loc: SourceLocation, root: KindedAst.Root): KindedAst.Spec = enum0 match {
    case KindedAst.Enum(_, _, _, _, tparams, _, _, tpeDeprecated, _) =>
      ...
      KindedAst.Spec(
        ...
        eff = Type.Cst(TypeConstructor.True, loc),
        tconstrs = List(Ast.TypeConstraint(Ast.TypeConstraint.Head(orderClassSym, loc), tpeDeprecated, loc)),
        loc = loc
      )
  }
  ...
  private def mkToStringSpec(enum0: KindedAst.Enum, param: Symbol.VarSym, loc: SourceLocation, root: KindedAst.Root)(implicit flix: Flix): KindedAst.Spec = enum0 match {
    case KindedAst.Enum(_, _, _, _, tparams, _, _, tpeDeprecated, _) =>
      ...
      KindedAst.Spec(
        ...
        mod = Ast.Modifiers.Empty,
        tparams = tparams,
        fparams = List(KindedAst.FormalParam(param, Ast.Modifiers.Empty, tpeDeprecated, loc)),
        tpe = Type.mkString(loc),
        eff = Type.Cst(TypeConstructor.True, loc),
        tconstrs = List(Ast.TypeConstraint(Ast.TypeConstraint.Head(toStringClassSym, loc), tpeDeprecated, loc)),
        loc = loc
      )
  }
  ...
  private def mkHashSpec(enum0: KindedAst.Enum, param: Symbol.VarSym, loc: SourceLocation, root: KindedAst.Root)(implicit flix: Flix): KindedAst.Spec = enum0 match {
    case KindedAst.Enum(_, _, _, _, tparams, _, _, tpeDeprecated, _) =>
      ...
      KindedAst.Spec(
        ...
        eff = Type.Cst(TypeConstructor.True, loc),
        tconstrs = List(Ast.TypeConstraint(Ast.TypeConstraint.Head(hashClassSym, loc), tpeDeprecated, loc)),
        loc = loc
      )
  }
