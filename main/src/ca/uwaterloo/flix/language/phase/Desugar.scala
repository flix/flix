...
object Desugar {
  ...
  private def visitExp(exp0: WeededAst.Expr)(implicit flix: Flix): DesugaredAst.Expr = exp0 match {
    ...
    case WeededAst.Expr.FixpointSolveWithProject(exps, solveMode, optIdents, loc) =>
      desugarFixpointSolveWithProject(exps, solveMode, optIdents, loc)
  ...
  /**
    ...
    * }}}
    */
  private def desugarFixpointSolveWithProject(exps0: List[WeededAst.Expr], solveMode: SolveMode, idents0: Option[List[Name.Ident]], loc0: SourceLocation)(implicit flix: Flix): DesugaredAst.Expr = {
    val es = visitExps(exps0)
    ...
    val modelExp = DesugaredAst.Expr.FixpointSolve(mergeExp, solveMode, loc0)

    // Any projections?
    val bodyExp = idents0 match {
      case None =>
        // Case 1: No projections: Simply return the minimal model or prepare provenance, depending on solveMode.
