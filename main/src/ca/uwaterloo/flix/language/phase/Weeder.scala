...
object Weeder {
  ...
  private def visitExp(exp0: ParsedAst.Expression)(implicit flix: Flix): Validation[WeededAst.Expr, WeederError] = exp0 match {
    ...
    case ParsedAst.Expression.Try(sp1, exp, ParsedAst.HandlerList.CatchHandlerList(handlers0), sp2) =>
      ...
      val rulesVal = traverse(handlers0) {
        case ParsedAst.TryHandlers.Catch(rules) => traverse(rules) {
  ...
  /**
    * Performs weeding on the [[ParsedAst.TryHandlers.WithHandler]] `handler0`.
    *
    ...
    * `[x, ...] --> [x, ...]`
    */
  private def visitWithHandler(handler0: ParsedAst.TryHandlers.WithHandler)(implicit flix: Flix): Validation[WeededAst.WithHandler, WeederError] = {
