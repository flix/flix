...
object TypedAst {
  ...
  object Expr {
    ...
    case class ApplyClo(exp1: Expr, exp2: Expr, tpe: Type, eff: Type, pos: ExpPosition, loc: SourceLocation) extends Expr

    case class ApplyDef(symUse: DefSymUse, exps: List[Expr], targs: List[Type], itpe: Type, tpe: Type, eff: Type, pos: ExpPosition, loc: SourceLocation) extends Expr

    case class ApplyLocalDef(symUse: LocalDefSymUse, exps: List[Expr], arrowTpe: Type, tpe: Type, eff: Type, pos: ExpPosition, loc: SourceLocation) extends Expr

    case class ApplyOp(symUse: OpSymUse, exps: List[Expr], tpe: Type, eff: Type, pos: ExpPosition, loc: SourceLocation) extends Expr

    case class ApplySig(symUse: SigSymUse, exps: List[Expr], targ: Type, targs: List[Type], itpe: Type, tpe: Type, eff: Type, pos: ExpPosition, loc: SourceLocation) extends Expr
