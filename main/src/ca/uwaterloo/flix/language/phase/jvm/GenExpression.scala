    ...
    case Expr.ApplyClo(exp, exps, ct, _, purity, loc) =>
      ct match {
        ...
        case CallType.NonTailCall =>
          ...
          if (!Purity.canUseAlgebraicEffects(purity)) BackendObjType.Result.unwindSuspensionFreeThunk("in non control-impure function call", loc)(new BytecodeInstructions.F(mv))
          else {
