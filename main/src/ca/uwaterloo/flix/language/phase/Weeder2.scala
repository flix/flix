...
object Weeder2 {
  ...
  private object Exprs {
    ...
    private def visitExtMatchRule(tree: Tree)(implicit sctx: SharedContext): Validation[Option[ExtMatchRule], CompilationMessage] = {
      ...
      val exprs = pickAll(TreeKind.Expr.Expr, tree)
      flatMapN(Patterns.pickExtTagPattern(tree), traverse(exprs)(visitExpr)) {
        case (Some((label, pats)), expr :: Nil) =>
          // case pat => expr
  ...
  private object Patterns {
    ...
    def pickExtTagPattern(tree: Tree)(implicit sctx: SharedContext): Validation[Option[(Name.Label, List[ExtPattern])], CompilationMessage] = {
    ...
    private def visitExtPattern(tree: Tree, seen: collection.mutable.Map[String, Name.Ident] = collection.mutable.Map.empty)(implicit sctx: SharedContext): Validation[Option[(Name.Label, List[ExtPattern])], CompilationMessage] = {
      expect(tree, TreeKind.Pattern.Pattern)
      tree.children.headOption match {
        case Some(subtree: Tree) => subtree.kind match {
          case TreeKind.Pattern.Tag => mapN(visitExtTagPattern(subtree, seen))(Some.apply)
          // Avoid double reporting errors by returning a success here
          case TreeKind.ErrorTree(_) => Validation.Success(None)
          case _ =>
    ...
    private def visitExtTagPattern(tree: SyntaxTree.Tree, seen: mutable.Map[String, Name.Ident])(implicit sctx: SharedContext): Validation[(Name.Label, List[ExtPattern]), CompilationMessage] = {
      ...
      mapN(pickQName(tree), traverseOpt(maybePat)(visitExtTagTermsPat(_, seen))) {
        (qname, maybePat) =>
          ...
          maybePat match {
            case None => (Name.mkLabel(qname.ident), List.empty)
            case Some(elms) => (Name.mkLabel(qname.ident), elms.toList)
          }
      }
    }
