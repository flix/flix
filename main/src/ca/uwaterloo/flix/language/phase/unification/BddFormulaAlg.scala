...
final class BddFormulaAlg(implicit flix: Flix) extends BoolAlg[DD] {
  ...
  override def isVar(f: DD): Boolean =
    GlobalBddBuilder.synchronized(f.equalsTo(f.getVariable, BddFormulaAlg.GlobalBddBuilder.makeFalse(), BddFormulaAlg.GlobalBddBuilder.makeTrue()))

  override def mkTrue: DD = GlobalBddBuilder.synchronized(BddFormulaAlg.GlobalBddBuilder.makeTrue())

  override def mkFalse: DD = GlobalBddBuilder.synchronized(BddFormulaAlg.GlobalBddBuilder.makeFalse())

  override def mkVar(id: Int): DD = GlobalBddBuilder.synchronized(BddFormulaAlg.GlobalBddBuilder.makeIthVar(id))

  override def mkNot(f: DD): DD = GlobalBddBuilder.synchronized(BddFormulaAlg.GlobalBddBuilder.makeNot(f))

  override def mkOr(f1: DD, f2: DD): DD = GlobalBddBuilder.synchronized(BddFormulaAlg.GlobalBddBuilder.makeOr(f1, f2))

  override def mkAnd(f1: DD, f2: DD): DD = GlobalBddBuilder.synchronized(BddFormulaAlg.GlobalBddBuilder.makeAnd(f1, f2))

  override def mkXor(f1: DD, f2: DD): DD = GlobalBddBuilder.synchronized(BddFormulaAlg.GlobalBddBuilder.makeXor(f1, f2))
  ...
  override def map(f: DD)(fn: Int => DD): DD = {
    if (f.isLeaf) {
      ...
      GlobalBddBuilder.synchronized(BddFormulaAlg.GlobalBddBuilder.makeIte(substDD, highRes, lowRes))
    }
  }
