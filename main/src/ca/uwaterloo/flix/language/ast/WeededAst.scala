...
object WeededAst {
  ...
  case class Root(units: Map[Ast.Source, WeededAst.CompilationUnits], reachable: Set[Symbol.DefnSym])

  case class CompilationUnits(uses: List[WeededAst.Use], decls: List[WeededAst.Declaration], loc: SourceLocation)
