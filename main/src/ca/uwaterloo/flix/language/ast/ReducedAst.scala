...
object ReducedAst {
  ...
  case class FormalParam(sym: Symbol.VarSym, tpe: SimpleType) extends ExpParam

  // Note: We deliberately omit the source location because it (a) is unused and (b) takes memory.
  case class TypeParam(name: Name.Ident, sym: Symbol.KindedTypeVarSym)

  // Note: We deliberately omit the source location because it (a) is unused and (b) takes memory.
  case class LocalParam(sym: Symbol.VarSym, tpe: SimpleType) extends ExpParam
