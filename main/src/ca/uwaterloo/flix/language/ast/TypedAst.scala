...
object TypedAst {
  ...
  object Expression {
    ...
    case class MatchNull(exps: List[TypedAst.Expression], rules: List[TypedAst.MatchNullRule], tpe: Type, eff: Type, loc: SourceLocation) extends TypedAst.Expression
  ...
  object Predicate {
    ...
    object Head {
      ...
      case class Atom(name: String, den: Denotation, terms: List[TypedAst.Expression], tpe: Type, loc: SourceLocation) extends TypedAst.Predicate.Head
    ...
    object Body {
      ...
      case class Atom(name: String, den: Denotation, polarity: Ast.Polarity, terms: List[TypedAst.Pattern], tpe: Type, loc: SourceLocation) extends TypedAst.Predicate.Body
  ...
  case class MatchNullRule(pat: List[Option[Symbol.VarSym]], exp: TypedAst.Expression)
