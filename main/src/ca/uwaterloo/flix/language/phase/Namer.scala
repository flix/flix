...
object Namer extends Phase[WeededAst.Program, NamedAst.Root] {
  ...
  private def visitClass(clazz: WeededAst.Declaration.Class, uenv0: UseEnv, tenv0: Map[String, Type.Var], ns0: Name.NName)(implicit flix: Flix): Validation[NamedAst.Class, NameError] = clazz match {
    case WeededAst.Declaration.Class(doc, mod, ident, tparams0, superClasses0, signatures, laws0, loc) =>
      ...
      // Extend the UseEnv with the class
      val classFqn = Name.QName(ident.sp1, Name.NName(ident.sp1, ns0.idents, ident.sp2), ident, ident.sp2)
      val uenv = uenv0.addTypeOrClass(ident.name, classFqn)
      val superClasses = superClasses0.map(getClass(_, uenv0))
      val tconstr = NamedAst.TypeConstraint(classFqn, NamedAst.Type.Var(tparam.tpe, tparam.loc))
      for {
        sigs <- traverse(signatures)(visitSig(_, uenv, tenv, ns0, classFqn, sym, tparam))
        laws <- traverse(laws0)(visitDef(_, uenv, tenv, ns0, List(tconstr), List(tparam.tpe)))
  ...
  /**
    ...
    */
  private def visitSig(sig: WeededAst.Declaration.Sig, uenv0: UseEnv, tenv0: Map[String, Type.Var], ns0: Name.NName, classFqn: Name.QName, classSym: Symbol.ClassSym, classTparam: NamedAst.TypeParam)(implicit flix: Flix): Validation[NamedAst.Sig, NameError] = sig match {
        ...
        case (tparams, _) =>
          ...
          flatMapN(getFormalParams(fparams0, uenv0, tenv)) {
            case fparams =>
              ...
              val tconstr = NamedAst.TypeConstraint(classFqn, NamedAst.Type.Var(classTparam.tpe, classTparam.loc))
