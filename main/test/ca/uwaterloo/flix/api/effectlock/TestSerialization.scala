...
class TestSerialization extends AnyFunSuite with TestUtils {
  ...
  test("serialize.def.10") {
    ...
    val tpe = Apply(Apply(Apply(Apply(Cst(Arrow(3)), Cst(Pure)), Apply(Apply(Apply(Cst(Arrow(2)), Cst(Pure)), Var(a)), AssocType(tb, Var(a), Nil, StarKind))), Var(a)), AssocType(tb, Var(a), Nil, StarKind))
