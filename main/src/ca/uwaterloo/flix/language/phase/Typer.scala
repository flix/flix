...
object Typer {
  ...
  /**
    ...
    */
  private def checkAssocTypes(spec0: KindedAst.Spec, extraTconstrs: List[TraitConstraint], tenv: TraitEnv)(implicit sctx: SharedContext, flix: Flix): Unit = spec0 match {
    case KindedAst.Spec(_, _, _, tparams, fparams, _, tpe, eff, tconstrs, econstrs) =>
      ...
      tpes.flatMap(getAssocTypes).foreach(checkAssocType(_, tparams, extraTconstrs ::: tconstrs, tenv))
  }

  /**
    * Verifies that all the associated types in the instance are resolvable, according to the declared type constraints.
    */
  private def checkAssocTypes(inst: KindedAst.Instance, tenv: TraitEnv)(implicit sctx: SharedContext, flix: Flix): Unit = inst match {
    case KindedAst.Instance(_, _, _, _, tparams, tpe, tconstrs, econstrs, _, _, _, _) =>
      // get all the associated types in the instance
      val tpes = tpe :: econstrs.flatMap(getTypes)

      // check that they are all covered by the type constraints
      tpes.flatMap(getAssocTypes).foreach(checkAssocType(_, tparams, tconstrs, tenv))
  }
