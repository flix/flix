  ...
  def closuresOf(root: Root): Set[ClosureInfo] = {
    ...
    def visitConstraint(c0: Constraint): Set[ClosureInfo] = {
      ...
      val headClosures = c0.head match {
        case Predicate.Head.RelAtom(_, terms, _, _) => terms.foldLeft(Set.empty[ClosureInfo]) {
          case (sacc, t) => sacc ++ visitHeadTerm(t)
        }
        case Predicate.Head.LatAtom(_, terms, _, _) => terms.foldLeft(Set.empty[ClosureInfo]) {
          case (sacc, t) => sacc ++ visitHeadTerm(t)
        }
    ...
    def visitBodyAtom(b0: Predicate.Body): Set[ClosureInfo] = b0 match {
      case Body.RelAtom(_, _, _, _, _) => Set.empty
      case Body.LatAtom(_, _, _, _, _) => Set.empty
