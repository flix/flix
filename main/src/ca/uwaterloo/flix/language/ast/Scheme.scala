...
object Scheme {
  ...
  def checkLessThanEqual(sc1: Scheme, sc2: Scheme, cenv0: Map[Symbol.ClassSym, Ast.ClassContext], eqEnv0: ListMap[Symbol.AssocTypeSym, Ast.AssocTypeDef])(implicit flix: Flix): Validation[Substitution, UnificationError] = {
    ...
    val (subst, econstrs2_1) = ConstraintResolution.resolve(tconstrs2_0, Substitution.empty, RigidityEnv.empty, cenv0, eqEnv0) match {
    ...
    ConstraintResolution.resolve(baseConstr :: cconstrs ::: econstrs, subst, renv, cenv, eqEnv) match {
