  ...
  object Expressions {
    ...
    def checkPats(tast: TypedAst.Expression, root: TypedAst.Root)(implicit genSym: GenSym): Validation[TypedAst.Expression, CompilationError] = {
      tast match {
        ...
        case Expression.ArrayStore(exp1, exps2, exp3, _, _, _) => for {
          _ <- checkPats(exp1, root)
          _ <- seqM(exps2 map {checkPats(_, root)} )
          _ <- checkPats(exp3, root)
        } yield tast
        ...
        case Expression.VectorLoad(exp1, _, _, _, _) =>
          checkPats(exp1, root)
        case Expression.VectorStore(exp1, _, _, _, _, _) =>
          checkPats(exp1, root)
        case Expression.VectorLength(exp1, _, _, _) =>
          checkPats(exp1, root)
        case Expression.VectorSlice(exp1, _, _, _, _, _) =>
          checkPats(exp1, root)
