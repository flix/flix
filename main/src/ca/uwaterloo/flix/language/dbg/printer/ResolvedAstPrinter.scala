...
object ResolvedAstPrinter {
  ...
  private def print(exp0: ResolvedAst.Expr): DocAst.Expr = exp0 match {
    ...
    case Expr.AscribeType(exp, tpe, _) => DocAst.Expr.AscriptionTpe(print(exp), UnkindedTypePrinter.print(tpe))
    case Expr.AscribeEff(exp, eff, _) => DocAst.Expr.AscriptionEff(print(exp), None, Some(UnkindedTypePrinter.print(eff)))
