...
object Lowering {
  ...
  private def visitExp(exp0: TypedAst.Expr)(implicit scope: Scope, root: TypedAst.Root, flix: Flix): LoweredAst.Expr = exp0 match {
    ...
    case TypedAst.Expr.ApplyDef(symUse, exps, ftpe, tpe, eff, loc) =>
      val es = exps.map(visitExp)
      val ft = visitType(ftpe)
      val t = visitType(tpe)
      LoweredAst.Expr.ApplyDef(symUse, es, ft, t, eff, loc)
  ...
  private def substExp(exp0: LoweredAst.Expr, subst: Map[Symbol.VarSym, Symbol.VarSym]): LoweredAst.Expr = exp0 match {
    ...
    case LoweredAst.Expr.ApplyDef(symUse, exps, ftpe, tpe, eff, loc) =>
      val es = exps.map(substExp(_, subst))
      LoweredAst.Expr.ApplyDef(symUse, es, ftpe, tpe, eff, loc)
