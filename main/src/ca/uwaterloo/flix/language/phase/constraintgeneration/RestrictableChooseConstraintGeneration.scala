...
object RestrictableChooseConstraintGeneration {
  ...
  /**
    ...
    */
  def visitRestrictableTag(exp: KindedAst.Expr.RestrictableTag)(implicit c: TypingContext, root: KindedAst.Root, flix: Flix): (Type, Type) = {
    // Make the context's level available
    // This is a def rather than a val because c is mutable.
    implicit def level: Level = c.level
  ...
  /**
    ...
    */
  def visitOpenAs(exp0: KindedAst.Expr.OpenAs)(implicit c: TypingContext, root: KindedAst.Root, flix: Flix): (Type, Type) = {
