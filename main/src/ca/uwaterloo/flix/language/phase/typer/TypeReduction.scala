    ...
    candidateConstructors.length match {
      ...
      case _ =>
        ...
        exactConstructor.length match {
          ...
          case _ => JavaConstructorResolutionResult.AmbiguousConstructor(exactConstructor.toList) // 0 corresponds to no exact method, 2 or higher should be impossible in Java
        }
    }
  }
