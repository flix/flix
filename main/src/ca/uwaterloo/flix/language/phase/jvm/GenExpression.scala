...
object GenExpression {
  ...
  def compileExpression(exp0: Expr, visitor: MethodVisitor, currentClass: JvmType.Reference, lenv0: Map[Symbol.LabelSym, Label], entryPoint: Label)(implicit root: Root, flix: Flix): Unit = exp0 match {
    ...
    case Expr.App(op, exps, tpe, loc) => op match {
      ...
      case IntrinsicOp.InstanceOf(clazz) =>
        val List(exp) = exps
        addSourceLine(visitor, loc)
      ...
      case IntrinsicOp.Cast =>
        val List(exp) = exps
        addSourceLine(visitor, loc)
        compileExpression(exp, visitor, currentClass, lenv0, entryPoint)
        AsmOps.castIfNotPrim(visitor, JvmOps.getJvmType(tpe))

      case IntrinsicOp.Index(idx) =>
        val List(exp) = exps
      ...
      case IntrinsicOp.RecordSelect(field) =>
        val List(exp) = exps
        // Adding source line number for debugging
        addSourceLine(visitor, loc)
      ...
      case IntrinsicOp.RecordRestrict(field) =>
        val List(exp) = exps
        // Adding source line number for debugging
        addSourceLine(visitor, loc)
        // We get the JvmType of the record interface
      ...
      case IntrinsicOp.GetField(field) =>
        val List(exp) = exps
        addSourceLine(visitor, loc)
        compileExpression(exp, visitor, currentClass, lenv0, entryPoint)
        val declaration = asm.Type.getInternalName(field.getDeclaringClass)
      ...
      case IntrinsicOp.PutStaticField(field) =>
        val List(exp) = exps
        addSourceLine(visitor, loc)
        compileExpression(exp, visitor, currentClass, lenv0, entryPoint)
        val declaration = asm.Type.getInternalName(field.getDeclaringClass)
