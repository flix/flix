...
import ca.uwaterloo.flix.language.phase.unification.shared.*
...
object EffectProvenance {
  ...
  private def findConflictsFromSource(initial: TypeConstraint, constrs0: List[TypeConstraint], initialSub: BoolSubstitution[ZhegalkinExpr[CofiniteIntSet]], idMap: Map[Type, Int], unused: Set[Type], contexts: List[HandlerContext], zhMap: ZhMap)(implicit alg: BoolAlg[ZhegalkinExpr[CofiniteIntSet]], scope: RegionScope, renv: RigidityEnv): (List[EffConflicted], Set[Type]) = {
    ...
    (loop(List(initial), constrs0, Queue.empty, initialSub), s)
  }
          ...
          if (tpe1 == sourceTpe)
            ...
            val errs = prov match {
              case Provenance.ExpectEffect(expected, _, _) => expected match {
                case Type.Var(sym, _) => filtered(sourceTpe, tpe1).map(x => EffConflicted(TypeError.EffectfulFunctionUsesOtherEffect(List(RigidVar(sym)), typeToSym(x).head, expected.loc, source.loc)))
                case Type.Cst(tc, loc) => tc match {
                  case TypeConstructor.Pure => filtered(sourceTpe, tpe1).map(mkPureErrors(_, sProv, loc, prov))
                ...
                case Type.Apply(_, _, _) =>
                  ...
                  notInDef.foldLeft(List.empty[Type])((acc, x) =>
                    if (acc.contains(x)
                      || isFlexible(x)
                      || isPure(x)
                      || contexts.exists(c => c.handled == x && consTypeArgs(c.inRun).contains(x))
                    ) acc else x :: acc).map(x => EffConflicted(TypeError.EffectfulFunctionUsesOtherEffect(typeToSym(expected), typeToSym(x).head, expected.loc, source.loc)))
                case _ => Nil
              }
  ...
  /**
    * Builds the specific `TypeError` for a source effect flowing into a `Pure`
    * sink (i.e. a function that is expected to be pure but uses an effect).
    *
    * Chooses among four variants depending on whether the signature location is
    * "real" (explicitly written by the user, vs. inferred) and whether the
    * offending source effect is `IO` specifically, which gets its own error
    * message.
    *
    * @param sourceEff the effect that leaked into the pure context
    * @param sProv     the provenance of the source effect, used for its location
    * @param sigLoc    the location of the (expected-pure) signature
    * @param prov      the provenance of the sink constraint, used for its location
    * @return          an `EffConflicted` wrapping the appropriate `TypeError` variant
    */
  private def mkPureErrors(sourceEff: Type, sProv: Provenance, sigLoc: SourceLocation, prov: Provenance): EffConflicted = {
    ...
    val err: TypeError = (sigLoc.isReal, isIO) match {
      case (true, true) => TypeError.ExplicitlyPureFunctionUsesIO(sigLoc, sProv.loc)
      case (true, false) => TypeError.ExplicitlyPureFunctionUsesEffect(typeToSym(sourceEff).head, sigLoc, sProv.loc)
  ...
  /**
    ...
    */
  private def initialSub(source: TypeConstraint, zhMap: ZhMap)(implicit alg: BoolAlg[ZhegalkinExpr[CofiniteIntSet]], scope: RegionScope, renv: RigidityEnv): Either[BoolSubstitution[ZhegalkinExpr[CofiniteIntSet]], List[EffConflicted]] = {
    zhMap.get(source) match {
      case Some((x, y)) =>
        try {
          Left(SveAlgorithm.sveAll(List((x, y))))
        } catch {
      ...
      case None => Right(Nil) // Could not generate errors
    }
  }
  ...
  /** Returns `true` iff `tpe` is a type variable that is flexible in `renv`.
    * Non-variable types are never considered flexible.
    */
  ...
  /** Returns `true` iff `tpe` is exactly the `Pure` effect constant. */
