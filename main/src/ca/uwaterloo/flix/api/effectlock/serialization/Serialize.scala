...
object Serialize {
  ...
  private def serializeType(tpe0: Type): SType = tpe0 match {
    ...
    case Type.AssocType(symUse, args, kind, _) => AssocType(serializeAssocTypeSym(symUse.sym), args.map(serializeType), serializeKind(kind))
  ...
  private def serializeEqualityConstraint(econstr0: EqualityConstraint): EqConstr = {
    EqConstr(serializeAssocTypeSym(econstr0.symUse.sym), econstr0.args.map(serializeType), serializeType(econstr0.tpe2))
  }
