...
import ca.uwaterloo.flix.language.ast.{Ast, RigidityEnv, Type}
...
/**
  ...
  */
case class InferMonad[+A](run: (Substitution, List[Ast.EqualityConstraint], RigidityEnv) => Result[(Substitution, List[Ast.EqualityConstraint], RigidityEnv, A), TypeError]) {
  /**
    * Applies the given function `f` to the value in the monad.
    */
  def map[B](f: A => B): InferMonad[B] = {
    def runNext(s0: Substitution, econstrs0: List[Ast.EqualityConstraint], renv0: RigidityEnv): Result[(Substitution, List[Ast.EqualityConstraint], RigidityEnv, B), TypeError] = {
  ...
  def transformError[B](f: TypeError => TypeError): InferMonad[A] = {
    def runNext(s0: Substitution, econstrs0: List[Ast.EqualityConstraint], renv0: RigidityEnv): Result[(Substitution, List[Ast.EqualityConstraint], RigidityEnv, A), TypeError] = {
