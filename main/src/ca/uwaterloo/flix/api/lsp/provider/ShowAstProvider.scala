  ...
  def showAst(phase: String)(implicit index: Index, root: Option[Root], flix: Flix): JObject = root match {
    ...
    case Some(r) =>
      ...
      phase match {
        ...
        case "EarlyTreeShaker" => astObject(phase, AstPrinter.formatLoweredAst(flix.getTreeShaker1Ast))
        case "Monomorph" => astObject(phase, AstPrinter.formatLoweredAst(flix.getMonoDefsAst))
        case "MonomorphEnums" => astObject(phase, AstPrinter.formatLoweredAst(flix.getMonoTypesAst))
        ...
        case "LateTreeShaker" => astObject(phase, AstPrinter.formatLiftedAst(flix.getTreeShaker2Ast))
