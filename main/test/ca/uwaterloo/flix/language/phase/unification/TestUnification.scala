...
class TestUnification extends FunSuite with TestUtils {
  ...
  test("Substitution.Empty.03") {
    val tpe = Type.KindedVar(mkTvarSym(1), Kind.Star, loc)
    val subst = Substitution.empty
    assertResult(Type.KindedVar(mkTvarSym(1), Kind.Star, loc))(subst(tpe))
  }
  ...
  test("Substitution.Singleton.01") {
    val tpe = Type.KindedVar(mkTvarSym(1), Kind.Star, loc)
    val subst = Substitution.singleton(Type.KindedVar(mkTvarSym(2), Kind.Star, loc), Type.Bool)
    assertResult(Type.KindedVar(mkTvarSym(1), Kind.Star, loc))(subst(tpe))
  }

  test("Substitution.Singleton.02") {
    val tpe = Type.KindedVar(mkTvarSym(1), Kind.Star, loc)
    val subst = Substitution.singleton(Type.KindedVar(mkTvarSym(1), Kind.Star, loc), Type.Bool)
    assertResult(Type.Bool)(subst(tpe))
  }

  test("Substitution.Singleton.05") {
    val tpe = Type.KindedVar(mkTvarSym(1), Kind.Star, loc)
    val subst = Substitution.singleton(Type.KindedVar(mkTvarSym(1), Kind.Star, loc), Type.KindedVar(mkTvarSym(2), Kind.Star, loc))
    assertResult(Type.KindedVar(mkTvarSym(2), Kind.Star, loc))(subst(tpe))
  }

  test("Substitution.++.01") {
    val subst1 = Substitution.singleton(Type.KindedVar(mkTvarSym(1), Kind.Star, loc), Type.Bool)
    val subst2 = Substitution.singleton(Type.KindedVar(mkTvarSym(2), Kind.Star, loc), Type.Char)
  ...
  test("Substitution.++.02") {
    ...
    val tpe = Type.KindedVar(mkTvarSym(2), Kind.Star, loc)
    assertResult(Type.Char)((subst1 ++ subst2) (tpe))
  }

  test("Substitution.++.03") {
    val subst1 = Substitution.singleton(Type.KindedVar(mkTvarSym(1), Kind.Star, loc), Type.Bool)
    val subst2 = Substitution.singleton(Type.KindedVar(mkTvarSym(1), Kind.Star, loc), Type.Char)
  ...
  test("Substitution.++.04") {
    ...
    val tpe = Type.mkPureArrow(Type.KindedVar(mkTvarSym(1), Kind.Star, loc), Type.KindedVar(mkTvarSym(2), Kind.Star, loc), loc)
  ...
  test("Substitution.@@.03") {
    val subst1 = Substitution.singleton(Type.KindedVar(mkTvarSym(1), Kind.Star, loc), Type.KindedVar(mkTvarSym(2), Kind.Star, loc))
    val subst2 = Substitution.singleton(Type.KindedVar(mkTvarSym(2), Kind.Star, loc), Type.Bool)
  ...
  test("Substitution.@@.04") {
    val subst1 = Substitution.singleton(Type.KindedVar(mkTvarSym(1), Kind.Star, loc), Type.KindedVar(mkTvarSym(2), Kind.Star, loc))
    val subst2 = Substitution.singleton(Type.KindedVar(mkTvarSym(2), Kind.Star, loc), Type.KindedVar(mkTvarSym(3), Kind.Star, loc))
    val subst3 = Substitution.singleton(Type.KindedVar(mkTvarSym(3), Kind.Star, loc), Type.Bool)
  ...
  test("Unify.Var.01") {
    val result = Unification.unifyTypes(Type.KindedVar(mkTvarSym(1), Kind.Star, loc), Type.Unit)
    assert(isOk(result))
  }

  test("Unify.Var.02") {
    val result = Unification.unifyTypes(Type.Unit, Type.KindedVar(mkTvarSym(1), Kind.Star, loc))
    assert(isOk(result))
  }

  test("Unify.Var.03") {
    val result = Unification.unifyTypes(Type.KindedVar(mkTvarSym(1), Kind.Star, loc), Type.KindedVar(mkTvarSym(1), Kind.Star, loc))
    assert(isOk(result))
  }

  test("Unify.Var.04") {
    val result = Unification.unifyTypes(Type.KindedVar(mkTvarSym(1), Kind.Star, loc), Type.KindedVar(mkTvarSym(2), Kind.Star, loc))
    assert(isOk(result))
  }
  ...
  test("Unify.01") {
    val tpe1 = Type.KindedVar(mkTvarSym(1), Kind.Star, loc)
    val tpe2 = Type.Bool
  ...
  test("Unify.02") {
    val tpe1 = Type.Bool
    val tpe2 = Type.KindedVar(mkTvarSym(1), Kind.Star, loc)
    val result = Unification.unifyTypes(tpe1, tpe2).get
    assertResult(Type.Bool)(result(tpe2))
  }

  test("Unify.03") {
    val A = Type.KindedVar(mkTvarSym(1), Kind.Star, loc)
  ...
  test("Unify.08") {
    val A = Type.KindedVar(mkTvarSym(1), Kind.Star, loc)
    val B = Type.KindedVar(mkTvarSym(2), Kind.Star, loc)
  ...
  test("Unify.11") {
    val A = Type.KindedVar(mkTvarSym(1), Kind.Star, loc)
    val B = Type.KindedVar(mkTvarSym(2), Kind.Star, loc)
    val C = Type.KindedVar(mkTvarSym(3), Kind.Star, loc)
    val tpe1 = Type.mkPureArrow(A, B, loc)
  ...
  test("Unify.12") {
    val tpe1 = Type.KindedVar(mkTvarSym(1), Kind.RecordRow, loc)
    val field = Type.Bool
  ...
  test("Unify.13") {
    val tpe1 = Type.KindedVar(mkTvarSym(1), Kind.SchemaRow, loc)
  ...
  test("Unify.15") {
    val tpe1 = Type.KindedVar(mkTvarSym(1), Kind.Star, loc, Rigidity.Flexible)
    val tpe2 = Type.KindedVar(mkTvarSym(2), Kind.RecordRow, loc, Rigidity.Rigid)
    val result = Unification.unifyTypes(tpe1, tpe2)
    assert(isOk(result))
  }

  test("Unify.16") {
    val tpe1 = Type.KindedVar(mkTvarSym(1), Kind.Star, loc, Rigidity.Rigid)
    val tpe2 = Type.KindedVar(mkTvarSym(2), Kind.Star, loc, Rigidity.Rigid)
    val result = Unification.unifyTypes(tpe1, tpe2)
    assert(!isOk(result))
  }
  ...
  test("seqM.03") {
    val subst0 = Substitution.empty
    val res1 = Unification.unifyTypeM(Type.KindedVar(mkTvarSym(1), Kind.Star, loc), Type.Bool, loc)
    val res2 = Unification.unifyTypeM(Type.KindedVar(mkTvarSym(2), Kind.Star, loc), Type.Char, loc)
    val res3 = Unification.unifyTypeM(Type.KindedVar(mkTvarSym(3), Kind.Star, loc), Type.mkTuple(List(Type.KindedVar(mkTvarSym(1), Kind.Star, loc), Type.KindedVar(mkTvarSym(2), Kind.Star, loc)), loc), loc)
    val result = seqM(List(res1, res2, res3)).run(subst0)
    val (subst, tpe) = result.get
    assertResult(Type.Bool)(subst.m(Type.KindedVar(mkTvarSym(1), Kind.Star, loc)))
    assertResult(Type.Char)(subst.m(Type.KindedVar(mkTvarSym(2), Kind.Star, loc)))
    assertResult(Type.mkTuple(List(Type.Bool, Type.Char), loc))(subst.m(Type.KindedVar(mkTvarSym(3), Kind.Star, loc)))
  }
