...
class Parser(val source: Source) extends org.parboiled2.Parser {
  ...
  object Types {
    ...
    def Schema: Rule1[ParsedAst.Type] = {
      def PredicateType: Rule1[ParsedAst.PredicateType] = rule {
        SP ~ Names.QualifiedPredicate ~ optWS ~ atomic("(") ~ optWS ~ oneOrMore(Type).separatedBy(optWS ~ "," ~ optWS) ~ optWS ~ atomic(")") ~ SP ~> ParsedAst.PredicateType
      }

      rule {
        SP ~ atomic("Schema") ~ optWS ~ atomic("{") ~ optWS ~ zeroOrMore(PredicateType).separatedBy(optWS ~ "," ~ optWS) ~ optional(optWS ~ "|" ~ optWS ~ Names.Variable) ~ optWS ~ "}" ~ SP ~> ParsedAst.Type.Schema
      }
    }
