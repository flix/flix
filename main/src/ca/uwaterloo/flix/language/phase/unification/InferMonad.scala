...
import ca.uwaterloo.flix.language.ast.{Ast, RigidityEnv, Symbol}
...
/**
  ...
  */
case class InferMonad[+A](run: (Substitution, List[Ast.BroadEqualityConstraint], RigidityEnv, Set[Symbol.KindedTypeVarSym]) => Result[(Substitution, List[Ast.BroadEqualityConstraint], RigidityEnv, Set[Symbol.KindedTypeVarSym], A), TypeError]) {
  /**
    * Applies the given function `f` to the value in the monad.
    */
  def map[B](f: A => B): InferMonad[B] = {
    def runNext(s0: Substitution, econstrs0: List[Ast.BroadEqualityConstraint], renv0: RigidityEnv, lenv0: Set[Symbol.KindedTypeVarSym]): Result[(Substitution, List[Ast.BroadEqualityConstraint], RigidityEnv, Set[Symbol.KindedTypeVarSym], B), TypeError] = {
  ...
  def transformError[B](f: TypeError => TypeError): InferMonad[A] = {
    def runNext(s0: Substitution, econstrs0: List[Ast.BroadEqualityConstraint], renv0: RigidityEnv, lenv0: Set[Symbol.KindedTypeVarSym]): Result[(Substitution, List[Ast.BroadEqualityConstraint], RigidityEnv, Set[Symbol.KindedTypeVarSym], A), TypeError] = {
