...
object Parser2 {
  ...
  /**
    ...
    * Use these together with the [[name]] and [[qualifiedName]] helper functions.
    */
  ...
  /**
    ...
    *   - [[TokenKind.isFirstDecl]] and [[TokenKind.isKeyword]] (not consumed)
    *     produces [[TreeKind.ErrorTree]]
    *   - `ANY` is consumed and produces [[TreeKind.ErrorTree]]
    */
  private def name(kinds: Set[TokenKind], context: SyntacticContext)(implicit s: State): Mark.Closed = {
    nameGeneric(kinds, tail = Set.empty, allowQualified = false, context)
  }
  ...
  /**
    ...
    *   - `ANY` is consumed and produces [[TreeKind.ErrorTree]]
    *
    * @param tail if any KIND found is in `tail` then no more dots are parsed
    */
  private def qualifiedName(kinds: Set[TokenKind], tail: Set[TokenKind] = Set(TokenKind.NameLowerCase), context: SyntacticContext)(implicit s: State): Mark.Closed = {
    nameGeneric(kinds, tail, allowQualified = true, context)
  }
  ...
  /**
    ...
    *
    * @param tail if any KIND found is in `tail` then no more dots are parsed
    *             (does nothing if `allowQualified` is false)
    * @param allowQualified if `false` only one KIND is parsed
    */
  private def nameGeneric(kinds: Set[TokenKind], tail: Set[TokenKind], allowQualified: Boolean = false, context: SyntacticContext)(implicit s: State): Mark.Closed = {
  ...
  private def use()(implicit s: State): Mark.Closed = {
    ...
    qualifiedName(NAME_USE, context = SyntacticContext.Use)
  ...
  private def iimport()(implicit s: State): Mark.Closed = {
    ...
    qualifiedName(NAME_JAVA, tail = Set(), context = SyntacticContext.Import)
  ...
  private object Decl {
    ...
    private def moduleDecl(mark: Mark.Opened, nestingLevel: Int = 0)(implicit s: State): Mark.Closed = {
      ...
      qualifiedName(NAME_MODULE, context = SyntacticContext.Decl.Module)
    ...
    private def traitDecl(mark: Mark.Opened)(implicit s: State): Mark.Closed = {
      ...
      name(NAME_DEFINITION, SyntacticContext.Decl.Trait)
      Type.parameters()
    ...
    private def instanceDecl(mark: Mark.Opened)(implicit s: State): Mark.Closed = {
      ...
      qualifiedName(NAME_DEFINITION, context = SyntacticContext.Decl.Instance)
    ...
    private def signatureDecl(mark: Mark.Opened)(implicit s: State): Mark.Closed = {
      ...
      name(NAME_DEFINITION, SyntacticContext.Decl.Module)
    ...
    private def enumerationDecl(mark: Mark.Opened)(implicit s: State): Mark.Closed = {
      ...
      name(NAME_TYPE, SyntacticContext.Decl.Enum)
      if (isRestrictable) {
        expect(TokenKind.BracketL, SyntacticContext.Decl.Enum)
        val markParam = open()
        name(NAME_VARIABLE, SyntacticContext.Decl.Enum)
    ...
    private def enumCases()(implicit s: State): Unit = {
      ...
      while (!eof() && atAny(FIRST_ENUM_CASE)) {
        ...
        name(NAME_TAG, SyntacticContext.Decl.Enum)
        if (at(TokenKind.ParenL)) {
          val mark = open()
          val markTuple = open()
          oneOrMore(
    ...
    private def structDecl(mark: Mark.Opened)(implicit s: State): Mark.Closed = {
      ...
      val nameLoc = currentSourceLocation()
      name(NAME_TYPE, SyntacticContext.Decl.Struct)
      Type.parameters()
      zeroOrMore(
    ...
    private def structField()(implicit s: State): Mark.Closed = {
      ...
      docComment()
      modifiers()
      name(NAME_FIELD, SyntacticContext.Decl.Struct)
    ...
    private def typeAliasDecl(mark: Mark.Opened)(implicit s: State): Mark.Closed = {
      ...
      name(NAME_TYPE, SyntacticContext.Decl.Type)
      if (at(TokenKind.BracketL)) {
        Type.parameters()
      }
      if (eat(TokenKind.Equal)) {
    ...
    private def associatedTypeSigDecl(mark: Mark.Opened)(implicit s: State): Mark.Closed = {
      ...
      name(NAME_TYPE, SyntacticContext.Decl.Module)
      if (at(TokenKind.BracketL)) {
        Type.parameters()
      }
    ...
    private def effectDecl(mark: Mark.Opened)(implicit s: State): Mark.Closed = {
      ...
      name(NAME_EFFECT, SyntacticContext.Decl.Module)
  ...
  private object Expr {
      ...
      while (continue) {
        nth(0) match {
          ...
          case TokenKind.Dot if nth(1) == TokenKind.NameLowerCase => // invoke method
            val mark = openBefore(lhs)
            eat(TokenKind.Dot)
            name(Set(TokenKind.NameLowerCase), SyntacticContext.Expr.OtherExpr)
          ...
          case TokenKind.Hash if nth(1) == TokenKind.NameLowerCase => // record lookup
            val mark = openBefore(lhs)
            eat(TokenKind.Hash)
            name(NAME_FIELD, SyntacticContext.Expr.OtherExpr)
      ...
      if (eat(TokenKind.KeywordWithout)) {
        ...
        if (at(TokenKind.CurlyL)) {
          oneOrMore(
            ...
            getItem = () => qualifiedName(NAME_EFFECT, context = SyntacticContext.Type.Eff),
          ...
          qualifiedName(NAME_EFFECT, context = SyntacticContext.Expr.OtherExpr)
        } else {
    ...
    private def exprDelimited()(implicit s: State): Mark.Closed = {
      ...
      nth(0) match {
        ...
        case TokenKind.Underscore => if (nth(1) == TokenKind.ArrowThinR) unaryLambdaExpr() else name(NAME_VARIABLE, SyntacticContext.Expr.OtherExpr)
        case TokenKind.NameLowerCase if nth(1) == TokenKind.ArrowThinR => unaryLambdaExpr()
        case TokenKind.NameLowerCase => qualifiedName(NAME_FIELD, context = SyntacticContext.Expr.OtherExpr)
        case TokenKind.NameUpperCase
             | TokenKind.NameMath
             | TokenKind.NameGreek => if (nth(1) == TokenKind.ArrowThinR) unaryLambdaExpr() else qualifiedName(NAME_DEFINITION, context = SyntacticContext.Expr.OtherExpr)
    ...
    private def openVariantExpr()(implicit s: State): Mark.Closed = {
      ...
      qualifiedName(NAME_QNAME, context = SyntacticContext.Expr.OtherExpr)
    ...
    private def holeExpr()(implicit s: State): Mark.Closed = {
      ...
      nth(0) match {
        ...
        case TokenKind.HoleNamed =>
          name(Set(TokenKind.HoleNamed), SyntacticContext.Expr.OtherExpr)
          close(mark, TreeKind.Expr.Hole)
    ...
    private def holeVariableExpr()(implicit s: State): Mark.Closed = {
      ...
      val mark = open()
      name(Set(TokenKind.HoleVariable), SyntacticContext.Expr.OtherExpr)
    ...
    private def unaryLambdaExpr()(implicit s: State): Mark.Closed = {
      ...
      val markParams = open()
      val markParam = open()
      name(NAME_PARAMETER, SyntacticContext.Expr.OtherExpr)
      close(markParam, TreeKind.Parameter)
    ...
    private def localDefExpr()(implicit s: State): Mark.Closed = {
      ...
      name(NAME_DEFINITION, SyntacticContext.Expr.OtherExpr)
      Decl.parameters(SyntacticContext.Expr.OtherExpr)
      if (eat(TokenKind.Colon)) {
        Type.typeAndEffect()
      }
    ...
    private def scopeExpr()(implicit s: State): Mark.Closed = {
      ...
      name(NAME_VARIABLE, SyntacticContext.Expr.OtherExpr)
      if (at(TokenKind.CurlyL)) {
        block()
      }
    ...
    private def catchRule()(implicit s: State): Mark.Closed = {
      ...
      name(NAME_VARIABLE, SyntacticContext.Expr.OtherExpr)
      expect(TokenKind.Colon, SyntacticContext.Expr.OtherExpr)
      qualifiedName(NAME_JAVA, tail = Set(), context = SyntacticContext.Expr.OtherExpr)
      expect(TokenKind.ArrowThickR, SyntacticContext.Expr.OtherExpr)
      statement()
    ...
    private def withBody()(implicit s: State): Mark.Closed = {
      ...
      qualifiedName(NAME_EFFECT, context = SyntacticContext.WithHandler)
      if (at(TokenKind.CurlyL)) {
        zeroOrMore(
    ...
    private def jvmMethod()(implicit s: State): Mark.Closed = {
      ...
      val mark = open()
      expect(TokenKind.KeywordDef, SyntacticContext.Expr.OtherExpr)
      name(NAME_JAVA, SyntacticContext.Expr.OtherExpr)
      Decl.parameters(SyntacticContext.Expr.OtherExpr)
      expect(TokenKind.Colon, SyntacticContext.Expr.OtherExpr)
      Type.typeAndEffect()
      expect(TokenKind.Equal, SyntacticContext.Expr.OtherExpr)
    ...
    private def fixpointSolveExpr()(implicit s: State): Mark.Closed = {
      ...
      if (eat(TokenKind.KeywordProject)) {
        name(NAME_PREDICATE, SyntacticContext.Expr.OtherExpr)
        while (eat(TokenKind.Comma) && !eof()) {
          name(NAME_PREDICATE, SyntacticContext.Expr.OtherExpr)
        }
      }
  ...
  private object Pattern {
    ...
    private def variablePat()(implicit s: State): Mark.Closed = {
      val mark = open()
      name(NAME_VARIABLE, SyntacticContext.Pat.OtherPat)
    ...
    private def tagPat()(implicit s: State): Mark.Closed = {
      val mark = open()
      qualifiedName(NAME_TAG, context = SyntacticContext.Pat.OtherPat)
      if (at(TokenKind.ParenL)) {
        tuplePat()
      }
    ...
    private def recordField()(implicit s: State): Mark.Closed = {
      val mark = open()
      name(NAME_FIELD, SyntacticContext.Pat.OtherPat)
      if (eat(TokenKind.Equal)) {
        pattern()
      }
  ...
  private object Type {
    ...
    private def parameter()(implicit s: State): Mark.Closed = {
      val mark = open()
      name(NAME_VARIABLE ++ NAME_TYPE, SyntacticContext.Type.OtherType)
    ...
    private def constraint()(implicit s: State): Mark.Closed = {
      val mark = open()
      qualifiedName(NAME_DEFINITION, context = SyntacticContext.WithClause)
    ...
    def derivations()(implicit s: State): Mark.Closed = {
      ...
      while (continue && !eof()) {
        if (atAny(NAME_QNAME)) {
          qualifiedName(NAME_QNAME, context = SyntacticContext.WithClause)
        } else {
    ...
    private def typeDelimited()(implicit s: State): Mark.Closed = {
      ...
      val mark = open()
      nth(0) match {
        case TokenKind.NameUpperCase => qualifiedName(NAME_TYPE, context = SyntacticContext.Type.OtherType)
        case TokenKind.NameMath
             | TokenKind.NameGreek
             | TokenKind.Underscore => name(NAME_VARIABLE, SyntacticContext.Type.OtherType)
        ...
        case TokenKind.KeywordStaticUppercase => name(Set(TokenKind.KeywordStaticUppercase), SyntacticContext.Type.OtherType)
        case t =>
          val mark = open()
    ...
    private def recordField()(implicit s: State): Mark.Closed = {
      val mark = open()
      name(NAME_FIELD, SyntacticContext.Type.OtherType)
    ...
    private def schemaType()(implicit s: State): Mark.Closed = {
      ...
      zeroOrMore(
        ...
        delimiterR = TokenKind.CurlyR,
        breakWhen = _.isRecoverType,
        optionallyWith = Some(TokenKind.Bar, () => name(NAME_VARIABLE, SyntacticContext.Type.OtherType)),
        context = SyntacticContext.Type.OtherType
      )
    ...
    private def schemaTerm()(implicit s: State): Mark.Closed = {
      val mark = open()
      qualifiedName(NAME_PREDICATE, context = SyntacticContext.Type.OtherType)
      if (at(TokenKind.BracketL)) {
        arguments()
    ...
    private def caseSetType()(implicit s: State): Mark.Closed = {
      ...
      zeroOrMore(
        ...
        getItem = () => qualifiedName(NAME_DEFINITION, context = SyntacticContext.Type.OtherType),
    ...
    def kind()(implicit s: State): Mark.Closed = {
      def kindFragment(): Unit = {
        ...
        name(NAME_KIND, SyntacticContext.Type.OtherType)
  ...
  private object Predicate {
    def head()(implicit s: State): Mark.Closed = {
      val mark = open()
      name(NAME_PREDICATE, SyntacticContext.Expr.Constraint)
      termList()
    ...
    private def functional()(implicit s: State): Mark.Closed = {
      ...
      nth(0) match {
        case TokenKind.ParenL => zeroOrMore(
          ...
          getItem = () => name(NAME_VARIABLE, SyntacticContext.Expr.OtherExpr),
        ...
        case TokenKind.NameLowerCase
             | TokenKind.NameMath
             | TokenKind.NameGreek
             | TokenKind.Underscore => name(NAME_VARIABLE, SyntacticContext.Expr.Constraint)
        case at =>
          val error = UnexpectedToken(
