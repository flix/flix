...
/**
  ...
  */
case class ScopeSubstitution(scopednessMap: Map[Type.Var, Type], schemeMap: Map[ScopeScheme.Var, ScopeScheme]) {

  /**
    * Returns `true` if `this` is the empty substitution.
    */
  val isEmpty: Boolean = scopednessMap.isEmpty && schemeMap.isEmpty

  /**
    * Applies `this` substitution to the given type `tpe0`.
    */
  def apply(tpe0: ScopeScheme): ScopeScheme = {
    // NB: The order of cases has been determined by code coverage analysis.
    def visit(t: ScopeScheme): ScopeScheme =
      t match {
        case x: ScopeScheme.Var => m.get(x) match {
          case None => x
          case Some(t0) => t0 match {
            // NB: This small trick is used to propagate variable names.
//            case tr: ScopeScheme.Var => tr.copy(text = x.text)
            case tr => tr
          }
        }
        case ScopeScheme.Arrow(sc1, sc2) => ScopeScheme.Arrow(visit(sc1), visit(sc2))
        case ScopeScheme.Unit(sc) =>
        case ScopeScheme.Cst(tc, _) => t
        case ScopeScheme.Apply(t1, t2) =>
          val y = visit(t2)
          visit(t1) match {
            // Simplify boolean equations.
            case ScopeScheme.Cst(ScopeSchemeConstructor.Not, _) => BoolUnification.mkNot(y)
            case ScopeScheme.Apply(ScopeScheme.Cst(ScopeSchemeConstructor.And, _), x) => BoolUnification.mkAnd(x, y)
            case ScopeScheme.Apply(ScopeScheme.Cst(ScopeSchemeConstructor.Or, _), x) => BoolUnification.mkOr(x, y)
            case x => ScopeScheme.Apply(x, y)
          }
        case ScopeScheme.Lambda(tvar, tpe) => throw InternalCompilerException(s"Unexpected type '$tpe0'.")
      }
  ...
  /**
    ...
    */
  def unbind(tvar: Type.Var): ScopeSubstitution = Substitution(m - tvar)
