...
object Scheme {
  ...
  /**
    ...
    */
  // TODO can optimize?
  def equal(sc1: Scheme, sc2: Scheme, traitEnv: TraitEnv, eqEnv: ListMap[Symbol.AssocTypeSym, Ast.AssocTypeDef])(implicit scope: Scope, listener: SolverListener, flix: Flix): Boolean = {
  ...
  /**
    ...
    * Θₚ ⊩ (∀α₁.π₁ ⇒ τ₁) ≤ (∀α₂.π₂ ⇒ τ₂)
    */
  def lessThanEqual(sc1: Scheme, sc2: Scheme, tenv0: TraitEnv, eenv0: ListMap[Symbol.AssocTypeSym, Ast.AssocTypeDef])(implicit scope: Scope, listener: SolverListener, flix: Flix): Boolean = {
    ...
    val (subst, econstrs2_1) = ConstraintSolver.resolve(tconstrs2_0, Substitution.empty, RigidityEnv.empty)(scope, tenv0, eenv0, listener, flix) match {
    ...
    val cconstrs2 = cconstrs2_0.map {
      case TraitConstraint(head, arg, loc) =>
        // should never fail
        val (t, _) = TypeReduction.simplify(subst(arg), RigidityEnv.empty, loc)(scope, eenv0, listener, flix).get
    ...
    ConstraintSolver.resolve(baseConstr :: cconstrs ::: econstrs, subst, renv)(scope, cenv, eenv, listener, flix) match {
