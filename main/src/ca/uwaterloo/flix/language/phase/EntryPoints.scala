...
object EntryPoints {
  ...
  /**
    * Returns a new shell function that swaps the unit value for a call to _f.
    *
    * This is necessary because the expression being evaluated may have non primitive effects
    * with default handlers.
    *
    * The set of effects with default handlers is not known from the start unlike the set of primitive effects.
    * In order to support the current way of creating a shell (create some simple wrapping code and execute it),
    * we need to substitute PrimitiveEffs with the real set of effects generated by exp at EntryPoints.
    *
    * Takes
    * {{{
    *   def main(): Unit \ PrimitiveEffs =
    *     def _f()={ println(exp) }
    *     checked_ecast(())
    * }}}
    *
  ...
  private object WrapInDefaultHandlers {
    ...
    /**
      * Wraps an entry point function with all necessary default effect handlers.
      *
      * Transforms a function:
      * def f(arg1: tpe1, ...): tpe \ ef = exp
      *
      * Into:
      * def f(arg1: tpe1, ...): tpe \ (((ef - handledEffect1) - handledEffect2) - ...) + IO =
      * handler1(_ -> handler2(_ ->exp))
      *
      * The wrappers:
    ...
    /**
      ...
      */
    private def wrapInHandler(defn: TypedAst.Def, handlerInfo: DefaultHandler, root: TypedAst.Root)(implicit flix: Flix): TypedAst.Def = {
      // Create synthetic locations
      val syntheticEffLocation = defn.spec.eff.loc.asSynthetic
      val syntheticBaseTypeLocation = defn.spec.declaredScheme.base.loc.asSynthetic
      val syntheticExpLocation = defn.exp.loc.asSynthetic
      ...
      val eff = Type.mkUnion(effDif, Type.IO, syntheticEffLocation)
      ...
      val handlerDefSymUse = SymUse.DefSymUse(handlerInfo.handlerSym, syntheticExpLocation)
      val handlerCall = TypedAst.Expr.ApplyDef(handlerDefSymUse, List(innerLambda), List(innerLambda.tpe), handlerArrowType, defn.spec.retTpe, eff, syntheticExpLocation)
