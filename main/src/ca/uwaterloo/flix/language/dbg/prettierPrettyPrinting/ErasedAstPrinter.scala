...
object ErasedAstPrinter {
  ...
  def doc(f: FormalParam)(implicit i: Indent): Doc = {
    paramf(text(f.sym.toString) <> text("%") <> text(f.sym.getStackOffset.toString), MonoTypePrinterrrr.doc(f.tpe))
  }
  ...
  def returnTypeDoc(tpe: MonoType)(implicit i: Indent): Doc = tpe match {
    case MonoType.Arrow(_, result) => MonoTypePrinterrrr.doc(result)
