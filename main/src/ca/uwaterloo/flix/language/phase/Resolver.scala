...
object Resolver {
  ...
  private def visitTag(caze: NamedAst.Declaration.Case, loc: SourceLocation)(implicit scope: Scope, flix: Flix): ResolvedAst.Expr = {
    ...
    // The definition expression.
    val tagExp = ResolvedAst.Expr.Tag(Ast.CaseSymUse(caze.sym, loc), loc)
  ...
  /**
    ...
    */
  private def visitApplyTag(app: NamedAst.Expr.Apply, caze: NamedAst.Declaration.Case, exps: List[NamedAst.Expr], env0: ListMap[String, Resolution], innerLoc: SourceLocation, outerLoc: SourceLocation)(implicit scope: Scope, ns0: Name.NName, taenv: Map[Symbol.TypeAliasSym, ResolvedAst.Declaration.TypeAlias], root: NamedAst.Root, flix: Flix): Validation[ResolvedAst.Expr, ResolutionError] = {
    if (caze.tpes.length == exps.length) {
      // Case 1: Hooray! We can build the tag directly.
      val esVal = traverse(exps)(resolveExp(_, env0))
      mapN(esVal) {
        case es =>
          val base = ResolvedAst.Expr.Tag(Ast.CaseSymUse(caze.sym, innerLoc), innerLoc)
          ResolvedAst.Expr.Apply(base, es, outerLoc)
      }
    } else {
      // Case 2: We have to curry. (See below).
