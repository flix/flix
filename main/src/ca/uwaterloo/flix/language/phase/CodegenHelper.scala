...
object CodegenHelper {
  ...
  /**
    * Qualified name of an enum case interface
    * @param sym symbol of the enum
    * @param tag tag of the enum
    * @param tpe tpe of the field of the enum
    */
  case class EnumCaseInterfaceName(sym: EnumSym, tag: String, tpe: WrappedType) extends EnumCaseName {
  ...
  /**
    * This helper function implements a method on the class generated by visitor that throws an exception.
    * This is useful for generating `toString()` and `equals()` method for different objects since these methods should
    * not get called.
    * All Fusion, Enum and Tuple classes include the following `toString()` and `equals()` methods:
    *
