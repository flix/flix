...
object SchemaUnification {
  ...
  def unifyRows(tpe1: Type, tpe2: Type, renv: RigidityEnv)(implicit scope: Scope, flix: Flix): Result[(Substitution, List[Ast.BroadEqualityConstraint]), UnificationError] = (tpe1, tpe2) match {
    ...
    case (Type.SchemaRowEmpty, Type.SchemaRowEmpty) => Ok((Substitution.empty, Nil))
  ...
  private def rewriteSchemaRow(rewrittenRow: Type, staticRow: Type, renv: RigidityEnv)(implicit scope: Scope, flix: Flix): Result[(Substitution, Type, List[Ast.BroadEqualityConstraint]), UnificationError] = {
    ...
    def visit(row: Type): Result[(Substitution, Type, List[Ast.BroadEqualityConstraint]), UnificationError] = (row, staticRow) match {
      ...
      Type.Apply(Type.Apply(Type.Cst(TypeConstructor.SchemaRowExtend(label1), _), fieldType1, _), _, _)) =>
        ...
        if (label1 == label2) {
          ...
          visit(restRow2) map {
            case (subst, rewrittenRow, econstrs) => (subst, Type.mkSchemaRowExtend(label2, fieldType2, rewrittenRow, loc), econstrs)
          }
        }
