...
object TupleGen extends Phase[ExecutableAst.Root, ExecutableAst.Root] {
  ...
  def findTuplesInExps(e: Expression): Set[Type] = e match {
    ...
    case Expression.ArrayNew(elm, len, tpe, loc) => findTuplesInExps(elm)
    case Expression.ArrayLit(elms, tpe, loc) => elms.foldLeft(findTuplesInTypes(tpe) + tpe)((acc, elem) => acc ++ findTuplesInExps(elem))
