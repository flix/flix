...
object Resolver extends Phase[NamedAst.Root, ResolvedAst.Root] {
  ...
  def run(root: NamedAst.Root)(implicit flix: Flix): Validation[ResolvedAst.Root, ResolutionError] = flix.phase("Resolver") {
    ...
    for {
      ...
      definitions.toMap, enums.toMap, latticeComponents.toMap, properties.flatten, root.reachable, root.sources
    )
  }
  ...
  /**
    ...
    */
  def resolveScheme(sc0: NamedAst.Scheme, ns0: Name.NName, root: NamedAst.Root)(implicit flix: Flix): Validation[Scheme, ResolutionError] = sc0 match {
