...
/**
  * End-to-end runtime smoke tests for the portable BigInt slice on the JVM backend.
  *
  * Runs a dedicated Flix fixture set to keep BigInt parity across JVM/LLVM without expanding the core
  * portable conformance suite.
  */
class PortableBigIntJvmRuntimeSuite extends AnyFunSuite {
  ...
  private val testDir = Paths.get("main/test/flix/portable_bigint")

  test("portable-bigint-jvm-runtime") {
    val flix = mkFlix()
