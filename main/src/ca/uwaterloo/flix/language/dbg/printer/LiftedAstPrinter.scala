...
object LiftedAstPrinter {
  ...
  def print(e: LiftedAst.Expression): DocAst.Expression = e match {
    ...
    case ApplyDef(sym, args, ct, _, _, _) => ct match {
      case CallType.TailCall => DocAst.Expression.ApplyDefTail(sym, args.map(print))
      case CallType.NonTailCall => DocAst.Expression.ApplyDef(sym, args.map(print))
    }
