...
import ca.uwaterloo.flix.language.ast.ResolvedAst.{ConstraintParam, Expression, Pattern, TypeParam, TypeParams}
...
object Kinder extends Phase[ResolvedAst.Root, KindedAst.Root] {
  ...
  private def reassembleExp(exp000: ResolvedAst.Expression, subst: KindSubstitution, root: ResolvedAst.Root)(implicit flix: Flix): KindedAst.Expression = {
    def visit(exp00: ResolvedAst.Expression): KindedAst.Expression = exp00 match {
      case ResolvedAst.Expression.Wild(tvar0, loc) =>
        val tvar0 = reassembleTypeVar(tvar0, subst, root)
        KindedAst.Expression.Wild(tvar0, loc)
      case ResolvedAst.Expression.Var(sym, tpe0, loc) =>
        val tpe = reassembleType(tpe0, subst, root)
        KindedAst.Expression.Var(sym, tpe, loc)
      case ResolvedAst.Expression.Def(sym, tpe0, loc) =>
        val tpe = reassembleTypeVar(tpe0, subst, root)
        KindedAst.Expression.Def(sym, tpe, loc)
      case ResolvedAst.Expression.Sig(sym, tpe0, loc) =>
        val tpe = reassembleTypeVar(tpe0, subst, root)
        KindedAst.Expression.Sig(sym, tpe, loc)
      ...
      case ResolvedAst.Expression.Apply(exp, exps, tpe0, eff0, loc) =>
      case ResolvedAst.Expression.Lambda(fparam, exp, tpe0, loc) =>
      case ResolvedAst.Expression.Unary(sop, exp, tpe0, loc) =>
      case ResolvedAst.Expression.Binary(sop, exp1, exp2, tpe0, loc) =>
      ...
      case ResolvedAst.Expression.LetRegion(sym, exp, evar0, loc) =>
      case ResolvedAst.Expression.Match(exp, rules, loc) =>
      case ResolvedAst.Expression.Choose(star, exps, rules, tpe0, loc) =>
      case ResolvedAst.Expression.Tag(sym, tag, exp, tpe0, loc) =>
      case ResolvedAst.Expression.Tuple(elms, loc) =>
      case ResolvedAst.Expression.RecordEmpty(tpe0, loc) =>
      case ResolvedAst.Expression.RecordSelect(exp, field, tpe0, loc) =>
      case ResolvedAst.Expression.RecordExtend(field, value, rest, tpe0, loc) =>
      case ResolvedAst.Expression.RecordRestrict(field, rest, tpe0, loc) =>
      case ResolvedAst.Expression.ArrayLit(elms, tpe0, loc) =>
      case ResolvedAst.Expression.ArrayNew(elm, len, tpe0, loc) =>
      case ResolvedAst.Expression.ArrayLoad(base, index, tpe0, loc) =>
      ...
      case ResolvedAst.Expression.Ref(exp, tvar0, loc) =>
      case ResolvedAst.Expression.RefWithRegion(exp1, exp2, tpe0, evar0, loc) =>
      case ResolvedAst.Expression.Deref(exp, tvar0, evar0, loc) =>
      case ResolvedAst.Expression.Assign(exp1, exp2, evar0, loc) =>
      case ResolvedAst.Expression.Existential(fparam, exp, loc) =>
      case ResolvedAst.Expression.Universal(fparam, exp, loc) =>
      case ResolvedAst.Expression.Ascribe(exp, expectedType, expectedEff, tpe0, loc) =>
      case ResolvedAst.Expression.Cast(exp, declaredType, declaredEff, tpe0, loc) =>
      ...
      case ResolvedAst.Expression.NewChannel(exp, tpe0, loc) =>
      case ResolvedAst.Expression.GetChannel(exp, tpe0, loc) =>
      case ResolvedAst.Expression.PutChannel(exp1, exp2, tpe0, loc) =>
      case ResolvedAst.Expression.SelectChannel(rules, default, tpe0, loc) =>
      case ResolvedAst.Expression.Spawn(exp, loc) =>
      case ResolvedAst.Expression.Lazy(exp, loc) =>
      case ResolvedAst.Expression.Force(exp, tpe0, loc) =>
      case ResolvedAst.Expression.FixpointConstraintSet(cs, tpe0, loc) =>
      case ResolvedAst.Expression.FixpointMerge(exp1, exp2, loc) =>
      case ResolvedAst.Expression.FixpointSolve(exp, loc) =>
      case ResolvedAst.Expression.FixpointFilter(pred, exp, tpe0, loc) =>
      case ResolvedAst.Expression.FixpointProjectIn(exp, pred, tpe0, loc) =>
      case ResolvedAst.Expression.FixpointProjectOut(pred, exp1, exp2, tpe0, loc) =>
  ...
  private def inferExp(exp00: ResolvedAst.Expression, root: ResolvedAst.Root)(implicit flix: Flix): KindInferMonad[Unit] = {
    ...
    def visit(exp0: ResolvedAst.Expression): KindInferMonad[Unit] = exp0 match {
      ...
      case ResolvedAst.Expression.LetRegion(sym, exp, evar, loc) =>
        for {
          _ <- visit(exp)
          _ <- unifyKindM(evar.kvar, Kind.Bool, loc)
        } yield ()
      case ResolvedAst.Expression.Match(exp, rules, loc) =>
        for {
          _ <- visit(exp)
      ...
      case ResolvedAst.Expression.Ref(exp, tvar0, loc) =>
        for {
          _ <- visit(exp)
          _ <- unifyKindM(tvar0.kvar, Kind.Star, loc)
        } yield ()
      case ResolvedAst.Expression.RefWithRegion(exp1, exp2, tpe, evar, loc) =>
        for {
          _ <- visit(exp1)
          _ <- visit(exp2)
          _ <- unifyKindM(tpe.kvar, Kind.Star, loc)
