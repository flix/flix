...
object TypePrinter {
  ...
  def print(tpe: Type): DocAst.Type = {
    ...
    (base, args) match {
      ...
      case (Type.RegionToEff(_, arg, _), _) => mkApp(mkApp(DocAst.Type.AsIs("RegionToEff"), List(print(arg))), args.map(print))
      case (Type.AbstractRegionToEff(_, arg, _), _) => mkApp(mkApp(DocAst.Type.AsIs("AbstractRegionToEff"), List(print(arg))), args.map(print))
