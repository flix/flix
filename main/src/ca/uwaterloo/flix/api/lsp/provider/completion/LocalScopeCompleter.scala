...
object LocalScopeCompleter {
  ...
  def getCompletions(err: ResolutionError.UndefinedName): Iterable[Completion] = {
    implicit val range: Range = Range.from(err.loc)
  ...
  /**
    ...
    */
  private def mkVarCompletion(name: String, resolutions: List[Resolution])(implicit range:Range): Iterable[Completion] = {
