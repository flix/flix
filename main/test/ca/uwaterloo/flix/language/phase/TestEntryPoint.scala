...
class TestEntryPoint extends FunSuite with TestUtils {
  ...
  test("Test.IllegalEntryPointArg.Main.01") {
    val input =
      """
        |def main(blah: Array[String, _]): Unit \ IO = unsafe_cast ??? as \ IO
        |""".stripMargin
  ...
  test("Test.IllegalEntryPointArg.Main.02") {
    val input =
      """
        |def main(blah: Array[a, _]): Unit \ IO = unsafe_cast ??? as \ IO
        |""".stripMargin
    val result = compile(input, Options.TestWithLibMin)
    expectError[EntryPointError.IllegalEntryPointArgs](result)
  }
  ...
  test("Test.IllegalEntryPointArg.Main.03") {
    val input =
      """
        |class C[a]
        |
        |def main(blah: Array[a, _]): Unit \ IO with C[a] = unsafe_cast ??? as \ IO
        |""".stripMargin
    val result = compile(input, Options.TestWithLibMin)
    expectError[EntryPointError.IllegalEntryPointArgs](result)
  }
  ...
  test("Test.IllegalEntryPointResult.Main.01") {
    val input =
      """
        |def main(): a \ IO = unsafe_cast ??? as \ IO
        |""".stripMargin
    val result = compile(input, Options.TestWithLibMin)
  ...
  test("Test.ValidEntryPoint.Main.01") {
    val input =
      """
        |def main(): Unit \ ef = unsafe_cast ??? as \ ef
        |""".stripMargin
    val result = compile(input, Options.TestWithLibMin)
    ...
    test("Test.ValidEntryPoint.Main.03") {
      val input =
        """
          |def main(): Int64 \ IO = unsafe_cast ??? as \ IO
          |""".stripMargin
      val result = compile(input, Options.TestWithLibMin)
      expectSuccess(result)
    }
