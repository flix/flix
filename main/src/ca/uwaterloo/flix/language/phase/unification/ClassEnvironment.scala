...
object ClassEnvironment {
  ...
  /**
    ...
    */
  private def byInst(tconstr: Ast.TypeConstraint, classEnv: Map[Symbol.ClassSym, Ast.ClassContext], renv: RigidityEnv)(implicit flix: Flix): Validation[List[Ast.TypeConstraint], UnificationError] = {
    val matchingInstances = classEnv.get(tconstr.head.sym).map(_.instances).getOrElse(Nil)
    val tconstrSc = Scheme.generalize(Nil, Nil, tconstr.arg, renv) // TODO ASSOC-TYPES Nil right?

    def tryInst(inst: Ast.Instance): Validation[List[Ast.TypeConstraint], UnificationError] = {
      val instSc = Scheme.generalize(Nil, Nil, inst.tpe, renv) // TODO ASSOC-TYPES Nil right?
