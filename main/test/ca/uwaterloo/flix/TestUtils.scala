...
trait TestUtils {
  ...
  def expectError[T](result: (Option[TypedAst.Root], List[CompilationMessage]), allowUnknown: Boolean = false)(implicit classTag: ClassTag[T]): Unit = result match {
    case (_, Nil) => fail("Expected at least one error, but found none.")
    case (optRoot, errors) => expectErrorGen[T](errors, optRoot, allowUnknown)
  }
  ...
  /**
    ...
    *
    * Asserts that the non-empty list of `errors` contains an error of the parametric type `T`.
    */
  private def expectErrorGen[T](errors: List[CompilationMessage], rootOpt: Option[TypedAst.Root], allowUnknown: Boolean = false)(implicit classTag: ClassTag[T]): Unit = {
    ...
    val expectedLine = getExpectedErrorLine(errors.head)
    val actuals = errors.map(e => (e, e.getClass))
    actuals.find {
