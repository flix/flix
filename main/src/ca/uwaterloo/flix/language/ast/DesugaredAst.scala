...
object DesugaredAst {
  ...
  val empty: Root = Root(Map.empty, None, Map.empty)

  case class Root(units: Map[Source, CompilationUnit], mainEntryPoint: Option[Symbol.DefnSym], tokens: Map[Source, Array[Token]])
