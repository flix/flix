...
object Eraser extends Phase[FinalAst.Root, ErasedAst.Root] {
  ...
  private def visitExp(exp0: FinalAst.Expression): ErasedAst.Expression = exp0 match {
    ...
    case FinalAst.Expression.Closure(sym, letrecs, freeVars, fnMonoType, tpe, loc) =>
      val newFreeVars = freeVars.map {
        case FinalAst.FreeVar(freeSym, freeTpe) => ErasedAst.FreeVar(freeSym, freeTpe)
      }
      ErasedAst.Expression.Closure(sym, letrecs, newFreeVars, fnMonoType, tpe, loc)
    ...
    case FinalAst.Expression.LetRec(varSym, defSym, exp1, exp2, tpe, loc) =>
      val e1 = visitExp(exp1)
      val e2 = visitExp(exp2)
      ErasedAst.Expression.LetRec(varSym, defSym, e1, e2, tpe, loc)
