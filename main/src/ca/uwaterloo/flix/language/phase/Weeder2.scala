...
object Weeder2 {
  ...
  private def pickAllUsesAndImports(tree: Tree)(implicit sctx: SharedContext): List[UseOrImport] = {
    ...
    maybeTree match {
      ...
      case Some(t) =>
        val uses = pickAll(TreeKind.UsesOrImports.Use, t)
        val imports = pickAll(TreeKind.UsesOrImports.Import, t)
  ...
  private object Decls {
    ...
    private def visitTraitDecl(tree: Tree)(implicit sctx: SharedContext): Declaration.Trait = {
      ...
      val sigsV = sigs.map(visitSignatureDecl)
      val lawsV = laws.map(visitLawDecl)
      val assocs = pickAll(TreeKind.Decl.AssociatedTypeSig, tree).map(visitAssociatedTypeSigDecl(_, tparam))
      Declaration.Trait(doc, ann, mod, ident, tparam, tconstr, assocs, sigsV, lawsV, tree.loc)
    }
    ...
    private def visitEnumDecl(tree: Tree)(implicit sctx: SharedContext): Declaration.Enum = {
      ...
      val allCases = (tpe, cases.map(visitEnumCase)) match {
      ...
      Declaration.Enum(doc, ann, mod, ident, tparams, derivations, allCases, tree.loc)
    }
    ...
    private def visitRestrictableEnumDecl(tree: Tree)(implicit sctx: SharedContext): Declaration.RestrictableEnum = {
      ...
      val tparams = Types.pickParameters(tree)
      val tpe = shorthandBody.map(Types.visitCaseType)
      val allCases = (tpe, cases.map(visitRestrictableEnumCase)) match {
        // Illegal empty singleton enum (`enum A()`)
        case (Some(List(Type.Error(_))), Nil) =>
          // Fall back on no cases. Whoever generated the error must have reported it.
          List.empty
      ...
      Declaration.RestrictableEnum(doc, ann, mod, ident, restrictionParam, tparams, derivations, allCases, tree.loc)
    }
    ...
    private def visitStructDecl(tree: Tree)(implicit sctx: SharedContext): Declaration.Struct = {
      ...
      val filteredFields = visitedFields.distinctBy(_.name.name)
    ...
    private def visitEffectDecl(tree: Tree)(implicit sctx: SharedContext): Declaration.Effect = {
      ...
      val doc = pickDocumentation(tree)
      val tparams = Types.pickParameters(tree)
      Declaration.Effect(doc, ann, mod, ident, tparams, ops.map(visitOperationDecl), tree.loc)
    }
      ...
      tryPick(TreeKind.ParameterList, tree) match {
        case Some(t) =>
          ...
          if (params.isEmpty) {
            ...
            val visited = params.map(visitParameter(_, presence))
            // Check for duplicates
            val paramsWithoutWildcards = visited.filter(!_.ident.isWild)
  ...
  private object Exprs {
    ...
    private def visitApplyExpr(tree: Tree)(implicit sctx: SharedContext): Expr = {
      ...
      val expr = pick(TreeKind.Expr.Expr, tree)
      ...
      maybeIntrinsic match {
        ...
        case None => Expr.Apply(visitExpr(expr), args, tree.loc)
      }
    }
    ...
    private def visitIndexExpr(tree: Tree)(implicit sctx: SharedContext): Expr = {
      ...
      pickAll(TreeKind.Expr.Expr, tree) match {
        case e1 :: e2 :: Nil =>
          val exp1 = visitExpr(e1)
          val exp2 = visitExpr(e2)
    ...
    private def visitLambdaExtMatchExpr(tree: Tree)(implicit sctx: SharedContext): Expr = {
      ...
      Expr.LambdaExtMatch(pat, pickExpr(tree), tree.loc)
    }
    ...
    private def visitIfThenElseExpr(tree: Tree)(implicit sctx: SharedContext): Expr = {
      ...
      pickAll(TreeKind.Expr.Expr, tree) match {
        case exprCondition :: exprThen :: exprElse :: Nil =>
          Expr.IfThenElse(visitExpr(exprCondition), visitExpr(exprThen), Some(visitExpr(exprElse)), tree.loc)
        case exprCondition :: exprThen :: Nil =>
          Expr.IfThenElse(visitExpr(exprCondition), visitExpr(exprThen), None, tree.loc)
        case exprs =>
    ...
    private def visitForApplicativeExpr(tree: Tree)(implicit sctx: SharedContext): Expr = {
      ...
      val fragments0 = pickForFragments(tree)
      val expr = pickExpr(tree)
      fragments0 match {
        case Nil =>
    ...
    private def visitLiteralRecordExpr(tree: Tree)(implicit sctx: SharedContext): Expr = {
      ...
      fields.map(visitLiteralRecordField).foldRight(Expr.Cst(Constant.RecordEmpty, tree.loc.asSynthetic): Expr) {
    ...
    private def visitParYieldExpr(tree: Tree)(implicit sctx: SharedContext): Expr = {
      ...
      if (fragments.isEmpty) {
        ...
        val frags = fragments.map(visitParYieldFragment)
        val expr = pickExpr(tree)
        Expr.ParYield(frags, expr, tree.loc)
      }
    }
    ...
    private def visitFixpointLambdaExpr(tree: Tree)(implicit sctx: SharedContext): Expr = {
      ...
      val paramList = pick(TreeKind.Predicate.ParamList, tree)
      val paramTrees = pickAllMulti(paramList, TreeKind.Predicate.ParamUntyped, TreeKind.Predicate.Param)
    ...
    private def visitFixpointQueryExpr(tree: Tree)(implicit sctx: SharedContext): Expr = {
      ...
      val selectTree = pick(TreeKind.Expr.FixpointSelect, tree)
      val selects = pickAll(TreeKind.Expr.Expr, selectTree).map(visitExpr)
      val fromTree = pick(TreeKind.Expr.FixpointFromFragment, tree)
      val froms = pickAll(TreeKind.Predicate.Atom, fromTree).map(Predicates.visitAtom)
    ...
    private def visitFixpointQueryWithProvenanceExpr(tree: Tree)(implicit sctx: SharedContext): Expr = {
      ...
      val withTree = pick(TreeKind.Expr.FixpointWith, tree)
      val withh = pickAll(TreeKind.Ident, withTree).map(tokenToIdent)
  ...
  private object Patterns {
    ...
    private def visitRecordPat(tree: Tree, seen: collection.mutable.Map[String, Name.Ident])(implicit sctx: SharedContext): Pattern = {
      ...
      maybePattern.map(visitPattern(_, seen)) match {
        ...
        // Pattern { x, ... | r }
        case Some(Pattern.Var(v, l)) if fs.nonEmpty =>
          Pattern.Record(fs, Pattern.Var(v, l), tree.loc)

        // Pattern { x, ... | _ }
        case Some(Pattern.Wild(l)) if fs.nonEmpty =>
          Pattern.Record(fs, Pattern.Wild(l), tree.loc)

        // Illegal pattern: { | r }
        case Some(r) if fs.isEmpty =>
  ...
  private object Predicates {
    ...
    private def visitPatternList(tree: Tree)(implicit sctx: SharedContext): (List[Pattern], Option[Pattern]) = {
      val rawList = pickAll(TreeKind.Pattern.Pattern, tree).map(tree => Patterns.visitPattern(tree))
  ...
  private object Types {
    def pickType(tree: Tree)(implicit sctx: SharedContext): Type = {
      tryPick(TreeKind.Type.Type, tree) match {
        case Some(t) => visitType(t)
        // Fall back on Type.Error. Parser has reported an error here.
    ...
    private def visitRecordRowType(tree: Tree)(implicit sctx: SharedContext): Type = {
      ...
      fields.foldRight(variable: Type) { case ((label, tpe), acc) => Type.RecordRowExtend(label, tpe, acc, tree.loc) }
    }
    ...
    private def visitSchemaRowType(parentTree: Tree)(implicit sctx: SharedContext): Type = {
      ...
      pickAllTrees(parentTree).foldRight(rest) {
        ...
        case (tree, acc) if tree.kind == TreeKind.Type.PredicateWithTypes =>
          ...
          val qname = pickQName(tree)
          val tps = types.map(Types.visitType)
    ...
    private def visitApplyType(tree: Tree)(implicit sctx: SharedContext): Type = {
      ...
      arguments.map(pickType).foldLeft(tpe) { case (acc, t2) => Type.Apply(acc, t2, tree.loc) }
    }
      ...
      types match {
        case t1 :: t2 :: Nil =>
          text(op).head match {
            // ARROW FUNCTIONS
            case "->" =>
              val eff = tryPick(TreeKind.Type.Effect, tree).map(pickType)
              val l = tree.loc.asSynthetic
