...
object Differ {
  ...
  private def compareDefs(oldDef: TypedAst.Def, newDef: TypedAst.Def): Boolean = {
    oldDef.spec.loc.beginCol != newDef.spec.loc.beginCol ||
      oldDef.spec.loc.endCol != newDef.spec.loc.endCol ||
      oldDef.spec.loc.beginLine - oldDef.spec.loc.endLine != newDef.spec.loc.beginLine - newDef.spec.loc.endLine ||
