...
object Deserialize {
  ...
  private def deserializeType(tpe0: SType): Type = tpe0 match {
    ...
    case AssocType(symUse, sel, args, kind) => Type.AssocType(deserializeAssocTypeSymUse(symUse), deserializeType(sel), args.map(deserializeType), deserializeKind(kind), SourceLocation.Unknown)
  }
  ...
  private def deserializeEqualityConstraint(econstr0: EqConstr): EqualityConstraint = econstr0 match {
    case EqConstr(sym, sel, args, tpe2) =>
      EqualityConstraint(deserializeAssocTypeSymUse(sym), deserializeType(sel), args.map(deserializeType), deserializeType(tpe2), SourceLocation.Unknown)
  }
