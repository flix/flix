  ...
  def run(root: SimplifiedAst.Root)(implicit flix: Flix): Validation[SimplifiedAst.Root, CompilationError] = flix.phase("CFA") {
    ...
    * where A may be multiple types and A, B, C are all proper types.
    ...
    def uncurryArrow(tpe0: Type): Type = tpe0 match {
      case Type.Apply(lhs0, Type.Apply(Type.Apply(Type.Arrow(2, eff1), b), c)) => //TODO: arity could be more than 2
        def visitLhs(lhs0: Type): Type = lhs0 match {
          ...
          case _ => ??? //Impossible due to precondition on tpe0
        }
      ...
      case _ => ??? //Impossible since tpe0 is a proper arrow type
    }
    ...
    val uncurriedDefs1 = flix.subphase("Uncurrying step 1") {
      uncurriedDefs0.foldLeft(List.empty[(Symbol.DefnSym, Def)]) {
        case (acc0, (sym0, defn0)) =>
    ...
    val (labeledDefs, _) = flix.subphase("Labeling") {
      State.sequence(uncurriedDefs1 map {
        case (sym, defn) => for {
          e <- labelExp(defn.exp)
