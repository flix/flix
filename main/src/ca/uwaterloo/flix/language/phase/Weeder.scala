...
object Weeder {
  ...
  private def visitEnum(d0: ParsedAst.Declaration.Enum)(implicit flix: Flix): Validation[List[WeededAst.Declaration.Enum], WeederError] = d0 match {
    case ParsedAst.Declaration.Enum(doc0, ann0, mods, sp1, ident, tparams0, tpe0, ParsedAst.Derivations(dsp1, classes, dsp2), cases0, sp2) =>
      val doc = visitDoc(doc0)
      val annVal = visitAnnotations(ann0)
      val modVal = visitModifiers(mods, legalModifiers = Set(Ast.Modifier.Public))
      ...
      mapN(annVal, modVal, tparamsVal, casesVal) {
        case (ann, mod, tparams, cases) =>
          List(WeededAst.Declaration.Enum(doc, ann, mod, ident, tparams, WeededAst.Derivations(classes.toList, mkSL(dsp1, dsp2)), cases.values.toList, mkSL(sp1, sp2)))
      }
  }
  ...
  private def visitRestrictableEnum(d0: ParsedAst.Declaration.RestrictableEnum)(implicit flix: Flix): Validation[List[WeededAst.Declaration.RestrictableEnum], WeederError] = d0 match {
    case ParsedAst.Declaration.RestrictableEnum(doc0, ann0, mods, sp1, ident, index0, tparams0, tpe0, ParsedAst.Derivations(dsp1, classes, dsp2), cases0, sp2) =>
      val doc = visitDoc(doc0)
      val annVal = visitAnnotations(ann0)
      val modVal = visitModifiers(mods, legalModifiers = Set(Ast.Modifier.Public))
      ...
      mapN(annVal, modVal, tparamsVal, casesVal) {
        case (ann, mod, tparams, cases) =>
          List(WeededAst.Declaration.RestrictableEnum(doc, ann, mod, ident, index, tparams, WeededAst.Derivations(classes.toList, mkSL(dsp1, dsp2)), cases.values.toList, mkSL(sp1, sp2)))
      }
  }
