...
object Eraser {
  ...
  private def visitJvmMethod(method: ReducedAst.JvmMethod)(implicit ctx: SharedContext, flix: Flix): ErasedAst.JvmMethod = method match {
    case ReducedAst.JvmMethod(jvmAnnotations, ident, fparams, clo, retTpe, purity, loc) =>
      // return type is not erased to maintain class signatures
      ErasedAst.JvmMethod(jvmAnnotations, ident, fparams.map(visitParam), visitExp(clo), visitType(retTpe), purity, loc)
  }
