...
import ca.uwaterloo.flix.language.ast.{SourceLocation, SpecialOperator, Symbol}
...
//  }
  ...
  /**
    ...
    */
  def compileField(visitor: ClassWriter, fieldName: String, fieldDescriptor: String, isStatic: Boolean, isPrivate: Boolean): Unit = {
    val visibility =
      if (isPrivate) {
        ACC_PRIVATE
      } else {
...
//    * }
//    */
//  def compileGetFieldMethod(visitor: ClassWriter, className: String, fieldName: String, methodName: String, fieldType: String): F[StackNil] => F[StackEnd] = {
//    val method = visitor.visitMethod(ACC_PUBLIC + ACC_FINAL, methodName, s"()$fieldType", null, null)
//
//    method.visitCode()
