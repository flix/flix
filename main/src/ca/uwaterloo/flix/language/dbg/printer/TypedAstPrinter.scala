...
object TypedAstPrinter {
  ...
  private def print(e: TypedAst.Expr): DocAst.Expr = e match {
    ...
    case Expr.ApplyDef(DefSymUse(sym, _), exps, _, _, _, _, _, _) => DocAst.Expr.ApplyDef(sym, exps.map(print).toList)
    case Expr.ApplyLocalDef(LocalDefSymUse(sym, _), exps, _, _, _, _, _) => DocAst.Expr.App(DocAst.Expr.Var(sym), exps.map(print).toList)
    case Expr.ApplyOp(op, exps, _, _, _, _) => DocAst.Expr.ApplyOp(op.sym, exps.map(print).toList)
    case Expr.ApplySig(SigSymUse(sym, _), exps, _, _, _, _, _, _, _) => DocAst.Expr.App(DocAst.Expr.AsIs(sym.name), exps.map(print).toList)
