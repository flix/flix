...
import ca.uwaterloo.flix.language.ast.{LoweredMoreAst, SourceLocation, Symbol}
...
object BackendObjType {
  ...
  case class Namespace(ns: List[String]) extends BackendObjType {
    ...
    def genByteCode(defs: List[LoweredMoreAst.Def])(implicit flix: Flix): Array[Byte] = {
      val cm = ClassMaker.mkClass(this.jvmName, IsFinal)
    ...
    def ShimMethod(defn: LoweredMoreAst.Def): StaticMethod = {
    ...
    private def shimIns(defn: LoweredMoreAst.Def)(implicit mv: MethodVisitor): Unit = {
