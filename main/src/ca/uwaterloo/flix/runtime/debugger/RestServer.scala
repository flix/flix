...
class RestServer(solver: Solver) {
   ...
   */
  class ListRelation(ast: TypedAst.Collection.Relation, relation: IndexedRelation[Value]) extends JsonHandler {
   ...
   */
  class ListLattice(ast: TypedAst.Collection.Lattice, relation: IndexedLattice[Value]) extends JsonHandler {
    def json: JValue = JObject(
