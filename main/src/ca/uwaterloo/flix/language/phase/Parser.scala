...
class Parser(val source: Source) extends org.parboiled2.Parser {
  ...
  // This whitespace rule is for parsing all whitespace types except newline.
  // The rule is used in the NewChannel expression, to fix the parsing error when buffersize is empty, eg. default buffersize.
  // The case is:
  // ```
  //   def g(): Channel[Int] = channel Int
  //
  //   def f(): Unit = ()
  // ```
  // This code throws a parsing error, because it is trying to parse `def f()` as the buffer size expression, by using the normal WS rule.
