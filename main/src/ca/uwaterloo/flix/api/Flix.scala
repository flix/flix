...
class Flix {
  ...
  def codeGen(typedAst: TypedAst.Root): Validation[CompilationResult, CompilationMessage] = try {
    ...
    val treeShaker2Ast = TreeShaker2.run(optimizer1Ast)
    val simplifierAst = Simplifier.run(treeShaker2Ast)
    ...
    val treeShaker3Ast = TreeShaker3.run(optimizerAst)
    val effectBinderAst = EffectBinder.run(treeShaker3Ast)
