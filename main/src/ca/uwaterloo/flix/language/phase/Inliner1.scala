...
import ca.uwaterloo.flix.language.ast.OccurrenceAst1.{DefContext, Expr, Occur}
import ca.uwaterloo.flix.language.ast.OccurrenceAst1.Occur.*
import ca.uwaterloo.flix.language.ast.shared.Constant
import ca.uwaterloo.flix.language.ast.{AtomicOp, OccurrenceAst1, Symbol, Type}
...
object Inliner1 {
  ...
  /**
    ...
    */
  private def visitDef(def0: OccurrenceAst1.Def)(implicit flix: Flix, root: OccurrenceAst1.Root): OccurrenceAst1.Def = def0 match {
    case OccurrenceAst1.Def(sym, fparams, spec, exp, occur, loc) =>
      val e = visitExp(exp, Map.empty, Map.empty, Map.empty, Context.Start)(root, flix)
      OccurrenceAst1.Def(sym, fparams, spec, e, occur, loc)
  }
  ...
  /**
    ...
    */
  private def visitExp(exp00: InExpr, varSubst0: VarSubst, subst0: Subst, inScopeSet0: InScopeSet, context0: Context)(implicit root: OccurrenceAst1.Root, flix: Flix): OutExpr = {

    def visit(exp0: InExpr): OutExpr = exp0 match {
      ...
      case Expr.Lambda((fparam, occur), exp, tpe, loc) => // TODO: Make parameter wild if dead
        val (fp, varSubst1) = freshFormalParameter(fparam)
      ...
      case Expr.ApplyAtomic(op, exps, tpe, eff, loc) =>
        ...
        op match {
          case AtomicOp.Untag(_) =>
            ...
            e match {
              case Expr.ApplyAtomic(AtomicOp.Tag(_), innerExps, _, _, _) => innerExps.head
              case _ => Expr.ApplyAtomic(op, es, tpe, eff, loc)
            }
      ...
      case Expr.ApplyClo(exp, exps, tpe, eff, loc) =>
        ...
        def maybeInline(sym1: OutVar): Expr.ApplyClo = {
          inScopeSet0.get(sym1) match {
            case Some(Definition.LetBound(lambda: Expr.Lambda, Occur.OnceInAbstraction)) =>
              if (isTrivialExp(lambda.exp)) {
                Expr.ApplyClo(lambda, es, tpe, eff, loc)
              } else {
                Expr.ApplyClo(visit(exp), es, tpe, eff, loc)
              }
      ...
      case Expr.IfThenElse(exp1, exp2, exp3, tpe, eff, loc) =>
        ...
        val e3 = visit(exp3)
        e1 match {
          case Expr.Cst(Constant.Bool(true), _, _) => e2
          case Expr.Cst(Constant.Bool(false), _, _) => e3
          case _ => Expr.IfThenElse(e1, e2, e3, tpe, eff, loc)
        }
      ...
      case Expr.TryCatch(exp, rules, tpe, eff, loc) =>
        ...
        val rs = rules.map {
          case OccurrenceAst1.CatchRule(sym, clazz, exp) =>
            ...
            val varSubst1 = varSubst0 + (sym -> freshVarSym)
            val e = visitExp(exp, varSubst1, subst0, inScopeSet0, context0)
            OccurrenceAst1.CatchRule(freshVarSym, clazz, e)
        }
  ...
  private def visitPattern(pattern00: OccurrenceAst1.Pattern)(implicit flix: Flix): (OccurrenceAst1.Pattern, VarSubst) = {

    // TODO: Figure out what to do with occurrence information in Pattern.Var
    def visit(pattern0: OccurrenceAst1.Pattern): (OccurrenceAst1.Pattern, VarSubst) = pattern0 match {
      case OccurrenceAst1.Pattern.Wild(tpe, loc) =>
        (OccurrenceAst1.Pattern.Wild(tpe, loc), Map.empty)
      ...
      case OccurrenceAst1.Pattern.Var(sym, tpe, occur, loc) =>
        if (isDead(occur)) {
          ...
          val freshVarSym = Symbol.freshVarSym(sym)
          (OccurrenceAst1.Pattern.Var(freshVarSym, tpe, occur, loc), Map(sym -> freshVarSym))
        }

      case OccurrenceAst1.Pattern.Cst(cst, tpe, loc) =>
        (OccurrenceAst1.Pattern.Cst(cst, tpe, loc), Map.empty)

      case OccurrenceAst1.Pattern.Tag(sym, pat, tpe, loc) =>
        val (p, varSubst) = visit(pat)
        (OccurrenceAst1.Pattern.Tag(sym, p, tpe, loc), varSubst)
      ...
      case OccurrenceAst1.Pattern.Tuple(pats, tpe, loc) =>
        ...
        (OccurrenceAst1.Pattern.Tuple(ps, tpe, loc), varSubst)
      ...
      case OccurrenceAst1.Pattern.Record(pats, pat, tpe, loc) =>
        ...
        val varSubst2 = varSubsts.foldLeft(varSubst1)(_ ++ _)
        (OccurrenceAst1.Pattern.Record(ps, p, tpe, loc), varSubst2)

      case OccurrenceAst1.Pattern.RecordEmpty(tpe, loc) =>
        (OccurrenceAst1.Pattern.RecordEmpty(tpe, loc), Map.empty)
    }

    def visitRecordLabelPattern(pattern0: OccurrenceAst1.Pattern.Record.RecordLabelPattern): (OccurrenceAst1.Pattern.Record.RecordLabelPattern, VarSubst) = pattern0 match {
      case OccurrenceAst1.Pattern.Record.RecordLabelPattern(label, pat, tpe, loc) =>
        val (p, subst) = visit(pat)
        (OccurrenceAst1.Pattern.Record.RecordLabelPattern(label, p, tpe, loc), subst)
    }
  ...
  /**
    ...
    * Add corresponding symbol from `symbols` to substitution map `env0`, mapping old symbols to fresh symbols.
    */
  private def inlineDef(exp0: Expr, symbols: List[(OccurrenceAst1.FormalParam, Occur)], args: List[OutExpr])(implicit root: OccurrenceAst1.Root, flix: Flix): Expr = {
  ...
  /**
    ...
    * Add corresponding symbol from `symbols` to substitution map `env0`, mapping old symbols to fresh symbols.
    */
  private def bind(exp0: Expr, symbols: List[(OccurrenceAst1.FormalParam, Occur)], args: List[OutExpr], varSubst0: VarSubst, subst0: Subst, inScopeSet0: InScopeSet, context0: Context)(implicit root: OccurrenceAst1.Root, flix: Flix): Expr = {
    def bnd(syms: List[(OccurrenceAst1.FormalParam, Occur)], as: List[OutExpr], env: VarSubst): Expr = (syms, as) match {
      ...
      case ((OccurrenceAst1.FormalParam(sym, _, _, _, _), _) :: nextSymbols, e1 :: nextExpressions) =>
        ...
        val eff = Type.mkUnion(e1.eff, nextLet.eff, e1.loc)
        Expr.Let(freshVar, e1, nextLet, nextLet.tpe, eff, Occur.DontInline, exp0.loc)
  ...
  private def freshFormalParameter(fp0: OccurrenceAst1.FormalParam)(implicit flix: Flix): (OccurrenceAst1.FormalParam, VarSubst) = fp0 match {
  ...
  /**
    ...
    */
  private def isDead(occur: Occur): Boolean = occur match {
    case Dead => true
    case _ => false
  }
  ...
  /**
    ...
    */
  private def isUsedOnceAndPure(occur: Occur, eff0: Type): Boolean = occur match {
  ...
  /**
    ...
    */
  private def isDeadAndPure(occur: Occur, eff0: Type): Boolean = occur match {
