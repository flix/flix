...
object Profiler {
  ...
  /**
    ...
    */
  private final case class Counters(inFlight: AtomicReference[InFlight], callCount: AtomicInteger, totalNanos: AtomicLong, perPhaseNanos: ConcurrentHashMap[String, AtomicLong], perPhaseCount: ConcurrentHashMap[String, AtomicLong], perPhaseAlloc: ConcurrentHashMap[String, AtomicLong], constraints: AtomicLong, tvars: AtomicLong, evars: AtomicLong, inlined: AtomicLong, classBytes: AtomicLong, allocBytes: AtomicLong, loc: AtomicReference[SourceLocation])
  ...
  def snapshot(): Vector[Profiler.DefnStats] = {
    ...
    it.map { e =>
      ...
      Profiler.DefnStats(key.sym, isActive = inFlight.isDefined,
        c.callCount.get(), committed + liveNanos, byPhase, byPhaseCount, byPhaseAlloc,
