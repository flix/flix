...
object HighlightProvider {
  ...
  private case class EffectConsumer(consider: Expr => Unit) extends Consumer {
    override def consumeExpr(exp: Expr): Unit = exp match {
      ...
      case Expr.Unary(sop, _, tpe, eff, loc) => consider(exp)
