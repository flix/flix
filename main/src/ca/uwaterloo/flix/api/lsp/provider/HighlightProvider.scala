...
object HighlightProvider {
  ...
  private case class EffectConsumer(consider: Expr => Unit) extends Consumer {
    override def consumeExpr(exp: Expr): Unit = exp match {
      ...
      case Expr.HoleWithExp(_, _, _, _) => ()
      case Expr.OpenAs(_, _, _, _) => ()
      case Expr.Use(_, _, _, _) => ()
      ...
      case Expr.Tag(_, _, _, _, _) => ()
      case Expr.RestrictableTag(_, _, _, _, _) => ()
      case Expr.Tuple(_, _, _, _) => ()
      case Expr.RecordEmpty(_, _) => ()
      case Expr.RecordSelect(_, _, _, _, _) => ()
      case Expr.RecordExtend(_, _, _, _, _, _) => ()
      case Expr.RecordRestrict(_, _, _, _, _) => ()
      ...
      case Expr.VectorLit(_, _, _, _) => ()
      case Expr.VectorLoad(_, _, _, _, _) => ()
      ...
      case Expr.FixpointMerge(_, _, _, _, _) => ()
      case Expr.FixpointSolve(_, _, _, _) => ()
      case Expr.FixpointFilter(_, _, _, _, _) => ()
      case Expr.FixpointInject(_, _, _, _, _) => ()
      case Expr.FixpointProject(_, _, _, _, _) => ()
