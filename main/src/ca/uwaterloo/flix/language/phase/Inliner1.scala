...
object Inliner1 {
  ...
  /**
    ...
    */
  def run(root: OccurrenceAst1.Root)(implicit flix: Flix): (MonoAst.Root, Optimizer1.Stats) = {
    implicit val sctx: SharedContext = SharedContext.mk()
    val defs = ParOps.parMapValues(root.defs)(visitDef(_)(root, sctx, flix))
    val newRoot = MonoAst.Root(defs, root.enums, root.structs, root.effects, root.mainEntryPoint, root.entryPoints, root.sources)
  ...
  private def visitDef(def0: OccurrenceAst1.Def)(implicit root: OccurrenceAst1.Root, sctx: SharedContext, flix: Flix): MonoAst.Def = def0 match {
    case OccurrenceAst1.Def(sym, _, spec, exp, ctx, loc) =>
