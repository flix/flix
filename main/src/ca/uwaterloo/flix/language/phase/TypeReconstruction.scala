...
object TypeReconstruction {
  ...
  private def visitExp(exp0: KindedAst.Expr)(implicit subst: SubstitutionTree): TypedAst.Expr = exp0 match {
    ...
    case KindedAst.Expr.ApplyDef(symUse, exps, map, itvar, tvar, evar, loc) =>
      val es = exps.map(visitExp)
      val m = MapOps.mapValues(map)(subst.apply)
      TypedAst.Expr.ApplyDef(symUse, es, m, subst(itvar), subst(tvar), subst(evar), loc)
    ...
    case KindedAst.Expr.ApplySig(symUse, exps, svar, map, itvar, tvar, evar, loc) =>
      ...
      val sel = subst(svar)
      val m = MapOps.mapValues(map)(subst.apply)
      TypedAst.Expr.ApplySig(symUse, es, sel, m, subst(itvar), subst(tvar), subst(evar), loc)
