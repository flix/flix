...
object Summary {
  ...
  private def lambdaCount(l: List[Expr]): Int = {
    l.map(lambdaCount).sum
  }
  ...
  /**
    ...
    */
  private def lambdaCount(e: Expr): Int = {
    e match {
      ...
      case Expr.Apply(exp, exps, tpe, eff, loc) => lambdaCount(exp :: exps)
      ...
      case Expr.Match(exp, rules, tpe, eff, loc) => lambdaCount(exp :: rules.flatMap(r => r.exp :: r.guard.toList))
      case Expr.TypeMatch(exp, rules, tpe, eff, loc) => lambdaCount(exp :: rules.map(_.exp))
      case Expr.RestrictableChoose(star, exp, rules, tpe, eff, loc) => lambdaCount(exp :: rules.map(_.exp))
      case Expr.Tag(sym, exp, tpe, eff, loc) => lambdaCount(exp)
      case Expr.RestrictableTag(sym, exp, tpe, eff, loc) => lambdaCount(exp)
      case Expr.Tuple(elms, tpe, eff, loc) => lambdaCount(elms)
      ...
      case Expr.ArrayLit(exps, exp, tpe, eff, loc) => lambdaCount(exp :: exps)
      ...
      case Expr.VectorLit(exps, tpe, eff, loc) => lambdaCount(exps)
      ...
      case Expr.TryCatch(exp, rules, tpe, eff, loc) => lambdaCount(exp :: rules.map(_.exp))
      case Expr.TryWith(exp, effUse, rules, tpe, eff, loc) => lambdaCount(exp :: rules.map(_.exp))
      case Expr.Do(op, exps, tpe, eff, loc) => lambdaCount(exps)
      case Expr.InvokeConstructor(constructor, exps, tpe, eff, loc) => lambdaCount(exps)
      case Expr.InvokeMethod(method, exp, exps, tpe, eff, loc) => lambdaCount(exp :: exps)
      case Expr.InvokeStaticMethod(method, exps, tpe, eff, loc) => lambdaCount(exps)
      ...
      case Expr.NewObject(name, clazz, tpe, eff, methods, loc) => lambdaCount(methods.map(_.exp))
      ...
      case Expr.SelectChannel(rules, default, tpe, eff, loc) => lambdaCount(default.toList ++ rules.flatMap(r => List(r.exp, r.chan)))
      case Expr.Spawn(exp1, exp2, tpe, eff, loc) => lambdaCount(exp1) + lambdaCount(exp2)
      case Expr.ParYield(frags, exp, tpe, eff, loc) => lambdaCount(exp :: frags.map(_.exp))
