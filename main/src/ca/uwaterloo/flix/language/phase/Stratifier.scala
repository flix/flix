...
object Stratifier {
  ...
  private def visitExp(exp0: Expr)(implicit g: LabelledPrecedenceGraph, sctx: SharedContext, root: Root, flix: Flix): Expr = exp0 match {
    ...
    case Expr.ApplyLocalDef(sym, exps, arrowTpe, tpe, eff, loc) =>
      val es = exps.map(visitExp)
      Expr.ApplyLocalDef(sym, es, arrowTpe, tpe, eff, loc)
    ...
    case Expr.LocalDef(ann, sym, fparams, declaredType, declaredEff, exp1, exp2, tpe, eff, loc) =>
      val e1 = visitExp(exp1)
      val e2 = visitExp(exp2)
      Expr.LocalDef(ann, sym, fparams, declaredType, declaredEff, e1, e2, tpe, eff, loc)
