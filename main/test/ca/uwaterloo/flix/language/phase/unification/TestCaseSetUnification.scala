...
class TestCaseSetUnification extends FunSuite with TestUtils {
  ...
  private val S = Symbol.mkRestrictableEnumSym(Name.RootNS, Name.Ident(SourcePosition.Unknown, "S", SourcePosition.Unknown))
  private val C1 = Symbol.mkRestrictableCaseSym(S, Name.Ident(SourcePosition.Unknown, "C1", SourcePosition.Unknown))
  private val C2 = Symbol.mkRestrictableCaseSym(S, Name.Ident(SourcePosition.Unknown, "C2", SourcePosition.Unknown))
  private val C3 = Symbol.mkRestrictableCaseSym(S, Name.Ident(SourcePosition.Unknown, "C3", SourcePosition.Unknown))
  private val C4 = Symbol.mkRestrictableCaseSym(S, Name.Ident(SourcePosition.Unknown, "C4", SourcePosition.Unknown))

  test("Test.SetUnification.01") {
    // ∅ ≐ ∅
    val tpe1 = Type.Cst(TypeConstructor.CaseEmpty(S), loc)
    val tpe2 = Type.Cst(TypeConstructor.CaseEmpty(S), loc)
    assertUnifies(tpe1, tpe2, RigidityEnv.empty, List(C1, C2, C3), S)
  }
...
//
  test("Test.SetUnification.03") {
    // ∅ ≐ ∅ ∪ ∅
    val tpe1 = Type.Cst(TypeConstructor.CaseEmpty(S), loc)
    val tpe2 = Type.mkCaseUnion(Type.Cst(TypeConstructor.CaseEmpty(S), loc), Type.Cst(TypeConstructor.CaseEmpty(S), loc), S, loc)
    assertUnifies(tpe1, tpe2, RigidityEnv.empty, List(C1, C2, C3), S)
  }

  test("Test.SetUnification.04") {
    // e ≐ e ∪ e
    val tpe1 = Type.Var(mkTypeVarSym("e", S), loc)
    val tpe2 = Type.mkCaseUnion(tpe1, tpe1, S, loc)
    assertUnifies(tpe1, tpe2, RigidityEnv.empty, List(C1, C2, C3), S)
  }

  test("Test.SetUnification.05") {
    // e! ≐ e! ∪ e!
    val sym = mkTypeVarSym("e", S)
    val tpe1 = Type.Var(sym, loc)
    val tpe2 = Type.mkCaseUnion(tpe1, tpe1, S, loc)
    val renv = RigidityEnv.empty.markRigid(sym)
    assertUnifies(tpe1, tpe2, renv, List(C1, C2, C3), S)
  }

  test("Test.SetUnification.06") {
    // e ≐ f
    val tpe1 = Type.Var(mkTypeVarSym("e", S), loc)
    val tpe2 = Type.Var(mkTypeVarSym("f", S), loc)
    assertUnifies(tpe1, tpe2, RigidityEnv.empty, List(C1, C2, C3), S)
  }

//  test("Test.SetUnification.07") {
//    // ((E ∪ (e ∩ F)) ∩ Fᶜ) ∪ ((F ∪ (f ∩ E)) ∩ Eᶜ) ≐ E ∪ F
//    val effE = Type.Cst(TypeConstructor.(mkEffectSym("E")), loc)
//    val effF = Type.Cst(TypeConstructor.Effect(mkEffectSym("F")), loc)
//    val varE = Type.Var(mkTypeVarSym("e", S), loc)
//    val varF = Type.Var(mkTypeVarSym("f", S), loc)
//
