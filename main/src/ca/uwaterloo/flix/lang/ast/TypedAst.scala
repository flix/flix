...
object TypedAst {
   ...
   */
  case class Var(name: String, tpe: Type) extends TypedAst.Expression

  /**
   * A typed AST node which represents unary expressions.
   */
  case class Unary(op: UnaryOperator, e: Expression, tpe: Type) extends TypedAst.Expression

  /**
   * A typed AST node which represents binary expressions.
   */
  case class Binary(e1: Expression, op: BinaryOperator, e2: Expression, tpe: Type) extends TypedAst.Expression


  /**
   * A typed AST node which represents an if-then-else expression.
   */
  case class IfThenElse(e1: Expression, e2: Expression, e3: Expression, tpe: Type) extends TypedAst.Expression
