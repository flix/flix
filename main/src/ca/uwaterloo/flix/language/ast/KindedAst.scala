...
object KindedAst {
  ...
  case class AssocTypeSig(doc: Doc, mod: Modifiers, sym: Symbol.AssocTypeSym, tparams: List[TypeParam], kind: Kind, tpe: Option[Type], loc: SourceLocation)

  case class AssocTypeDef(doc: Doc, mod: Modifiers, symUse: AssocTypeSymUse, args: List[Type], tpe: Type, loc: SourceLocation)
