...
object LiftedAstPrinter {
  ...
  def print(e: LiftedAst.Expr): DocAst.Expression = e match {
    ...
    case ApplyClo(exp, exps, ct, _, _, _) => DocAst.Expression.ApplyClo(print(exp), exps.map(print), ct)
    case ApplyDef(sym, args, ct, _, _, _) => DocAst.Expression.ApplyDef(sym, args.map(print), ct)
