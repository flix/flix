...
import ca.uwaterloo.flix.language.ast.Symbol.SigSym
...
object UseSignatureCompleter extends Completer {
  override def getCompletions(context: CompletionContext)(implicit flix: Flix, index: Index, root: TypedAst.Root, delta: DeltaContext): Iterable[UseSignatureCompletion] = {
    stripWord(context) match {
      case Some(word) =>
        val sigs = root.sigs.keys.filter(s => matches(s, word))
        sigs.map(sig => getUseSigCompletion(root.sigs(sig)))
      case _ => Nil
    }
  }
  ...
  private def matches(sym: SigSym, suffix: String): Boolean = {
    sym.name.startsWith(suffix)
  }

  private def getUseSigCompletion(decl: TypedAst.Sig): UseSignatureCompletion = {
    val sym = decl.sym
    Completion.UseSignatureCompletion(sym.toString)
  }
}
