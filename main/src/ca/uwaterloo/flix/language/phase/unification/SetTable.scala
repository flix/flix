...
object SetTable {
  ...
  /**
    ...
    */
  private lazy val Table: Array[SetFormula] = initTable()
  ...
  def minimizeType(tpe: Type)(implicit flix: Flix): Type = {
    ...
    if (tpe.kind != Kind.Effect) {
      throw InternalCompilerException(s"Unexpected non-Effect kind: '${tpe.kind}'.")
    }
    ...
    val m = (tvars ::: effs).zipWithIndex.foldLeft(Bimap.empty[SetFormula.Constant, Variable]) {
  ...
  /**
    ...
    */
  def minimizeFormula(f: SetFormula): SetFormula = {
  ...
  /**
    ...
    */
  private def minimizeFormulaRecursively(f: SetFormula): (SetFormula, SortedSet[Variable]) = f match {
    case All => (All, SortedSet.empty)

    case Empty => (Empty, SortedSet.empty)

    case Var(x) => (Var(x), SortedSet(x))

    case Complement(formula) =>
      val (f, fvs) = minimizeFormulaRecursively(formula)
      (Complement(f), fvs)

    case Intersection(formula1, formula2) =>
      ...
      if (fvs.size <= MaxVars) {
        ...
        (Intersection(minf1, minf2), minf1.freeVars ++ minf2.freeVars)
      }

    case Union(formula1, formula2) =>
      ...
      if (fvs.size <= MaxVars) {
        ...
        val minf1 = alphaRenameAndLookup(f1)
        val minf2 = alphaRenameAndLookup(f2)
        (Union(minf1, minf2), minf1.freeVars ++ minf2.freeVars)
      }
  }
  ...
  /**
    ...
    */
  private def alphaRenameAndLookup(f: SetFormula): SetFormula = {
  ...
  /**
    ...
    */
  private def lookup(f: SetFormula): SetFormula = {
  ...
  /**
    ...
    */
  private def computeSemanticFunction(f: SetFormula, fvs: List[Variable], position: Int, env: Array[Boolean]): Int = fvs match {
    case Nil =>
  ...
  /**
    ...
    */
  private def eval(f: SetFormula, env: Array[Boolean]): Boolean = f match {
    case All => true
    case Empty => false
    case Var(x) => env(x)
    case Complement(f) => !eval(f, env)
    case Intersection(f1, f2) => eval(f1, env) && eval(f2, env)
    case Union(f1, f2) => eval(f1, env) || eval(f2, env)
  }

  /**
    * Parses the built-in table into an S-expression and then into an in-memory table.
    */
  private def initTable(): Array[SetFormula] = {
    val table = loadTable()
  ...
  /**
    ...
    */
  private def loadTable(): Array[SetFormula] = try {
    ...
    val table = new Array[SetFormula](1 << (1 << MaxVars))
  ...
  /**
    ...
    *
    */
  private def parseLine(l: String): SetFormula = {
    @tailrec
    def parse(input: List[Char], stack: List[SetFormula]): SetFormula = (input, stack) match {
      case (Nil, formula :: Nil) => formula
      case ('T' :: rest, stack) => parse(rest, All :: stack)
      case ('F' :: rest, stack) => parse(rest, Empty :: stack)
      ...
      case ('n' :: rest, f :: stack) => parse(rest, Complement(f) :: stack)
      case ('a' :: rest, f2 :: f1 :: stack) => parse(rest, Intersection(f1, f2) :: stack)
      case ('o' :: rest, f2 :: f1 :: stack) => parse(rest, Union(f1, f2) :: stack)
