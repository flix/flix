...
object TypedAstPrinter {
  ...
  private def print(e: TypedAst.Expr): DocAst.Expr = e match {
    ...
    case Expr.AscribeType(exp, _, tpe, _, _) => DocAst.Expr.AscriptionTpe(print(exp), TypePrinter.print(tpe))
    case Expr.AscribeEff(exp, _, tpe, _, _) => DocAst.Expr.AscriptionEff(print(exp), None, Some(TypePrinter.print(tpe)))
