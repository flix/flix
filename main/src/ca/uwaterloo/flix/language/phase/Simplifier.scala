...
object Simplifier extends Phase[TypedAst.Root, SimplifiedAst.Root] {
  ...
  def run(root: TypedAst.Root)(implicit flix: Flix): Validation[SimplifiedAst.Root, CompilationError] = flix.phase("Simplifier") {
    ...
    /**
      ...
      */
    def patternMatchNull(exps0: List[TypedAst.Expression], rules0: List[TypedAst.MatchNullRule], tpe: Type, loc: SourceLocation)(implicit flix: Flix): SimplifiedAst.Expression = {
      //
      // Given the code:
      //
      // match (x, y, ...) with {
