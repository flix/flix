...
object Dependencies {
  ...
  /**
    * Shared context for the dependencies phase.
    * Ideally, we should use something like a concurrentMultiMap, but we don't have that in the standard library.
    * So we just use a concurrentMap with a dummy value and keep the key-value pairs in the map.
    */
