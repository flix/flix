...
object TypedAstPrinter {
  ...
  private def print(e: TypedAst.Expr): DocAst.Expr = e match {
    ...
    case Expr.StructNew(sym, fields, regionOpt, _, _, _) => regionOpt match {
      case Some(region) => DocAst.Expr.StructNew(sym, fields.map { case (k, v) => (k.sym, print(v)) }, print(region))
      case None => DocAst.Expr.PureStructNew(sym, fields.map { case (k, v) => (k.sym, print(v)) })
    }
