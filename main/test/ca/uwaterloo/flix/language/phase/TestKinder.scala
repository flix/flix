...
class TestKinder extends FunSuite with TestUtils {
  ...
  private val DefaultOptions = Options.TestWithLibMin
  ...
  ignore("IllegalUninhabitedType.08") { // MATT causes stack overflow
  ...
  ignore("IllegalUninhabitedType.10") { // MATT stack overflow
  ...
  ignore("IllegalEffect.01") {
    val input = "def f(): Int = 1 as & Int"
    val result = compile(input, DefaultOptions)
    expectError[KindError.MismatchedKinds](result)
  }

  ignore("IllegalEffect.02") {
    val input = "def f(): Int = 1 as Int & Int"
    val result = compile(input, DefaultOptions)
    expectError[KindError.MismatchedKinds](result)
  }

  ignore("IllegalEffect.03") {
    val input = "def f(): Int = 1: & Int"
    val result = compile(input, DefaultOptions)
    expectError[KindError.MismatchedKinds](result)
  }

  ignore("IllegalEffect.04") {
