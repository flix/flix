...
object ResolvedAstPrinter {
  ...
  private def print(exp: ResolvedAst.Expr): DocAst.Expr = exp match {
    ...
    case Expr.ApplyLocalDef(sym, exps, _) => DocAst.Expr.App(printVarSym(sym), exps.map(print))
    ...
    case Expr.LocalDef(_, sym, _, _, _, exp1, exp2, _) => DocAst.Expr.LetRec(printVarSym(sym), None, print(exp1), print(exp2))
