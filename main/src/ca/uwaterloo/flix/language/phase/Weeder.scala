    ...
    flatMapN(argsVal) {
      case args =>
        past.ident.name match {
          ...
          case "LazyIfPure" => WeededAst.Annotation(Ast.Annotation.LazyIfPure(loc), args, loc).toSuccess
          case "ParallelIfPure" => WeededAst.Annotation(Ast.Annotation.ParallelIfPure(loc), args, loc).toSuccess
