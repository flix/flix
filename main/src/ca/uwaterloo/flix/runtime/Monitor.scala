...
object Monitor {
  ...
  /**
    ...
    * @param readTasks  the current number of read tasks.
    * @param writeTasks the current number of write tasks.
    * @param facts      the total number of facts.
    * @param memory     the amount memory used.
    */
  case class Sample(time: Long, readTasks: Int, writeTasks: Int, facts: Int, memory: Long)
...
class Monitor(solver: Solver) {
  ...
  class Sampler extends Runnable {
    ...
    def run(): Unit = {
      ...
      val currentReadTasks = solver.getCurrentReadTasks
      val currentWriteTasks = solver.getCurrentWriteTasks
      ...
      samples ::= Monitor.Sample(elapsedTime, currentReadTasks, currentWriteTasks, totalFacts, usedMemoryInMegaBytes)
    }
