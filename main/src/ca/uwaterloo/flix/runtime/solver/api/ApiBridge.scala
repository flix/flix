...
object ApiBridge {
  ...
  private def visitStratum(stratum: ExecutableAst.Stratum)(implicit root: ExecutableAst.Root, flix: Flix): Stratum = {
    Stratum(stratum.constraints.map(visitConstraint))
  }

  private def visitTables(tables: Map[Symbol.TableSym, ExecutableAst.Table])(implicit root: ExecutableAst.Root, flix: Flix): Map[TableSym, Table] = {
  ...
  private def visitConstraint(c0: ExecutableAst.Constraint)(implicit root: ExecutableAst.Root, flix: Flix): Constraint = {
  ...
  private def visitConstraintParam(c0: ExecutableAst.ConstraintParam)(implicit root: ExecutableAst.Root, flix: Flix): VarSym = c0 match {
  ...
  private def visitHeadPredicate(h0: ExecutableAst.Predicate.Head)(implicit root: ExecutableAst.Root, flix: Flix): Predicate = h0 match {
  ...
  private def visitBodyPredicate(b0: ExecutableAst.Predicate.Body)(implicit root: ExecutableAst.Root, flix: Flix): Predicate = b0 match {
  ...
  private def visitTableSym(sym: Symbol.TableSym)(implicit root: ExecutableAst.Root, flix: Flix): TableSym = new TableSym(sym.toString)

  private def visitHeadTerm(t0: ExecutableAst.Term.Head)(implicit root: ExecutableAst.Root, flix: Flix): Term = t0 match {
  ...
  def visitBodyTerm(t0: ExecutableAst.Term.Body)(implicit root: ExecutableAst.Root, flix: Flix): Term = t0 match {
  ...
  private def visitVarSym(sym: Symbol.VarSym)(implicit root: ExecutableAst.Root, flix: Flix): VarSym = new VarSym(sym.id, sym.getStackOffset)

  private def visitAttribute(a: ExecutableAst.Attribute)(implicit root: ExecutableAst.Root, flix: Flix): Attribute =
    Attribute(a.name)

  private def visitLatOps(tables: Map[Symbol.TableSym, ExecutableAst.Table])(implicit root: ExecutableAst.Root, flix: Flix): Map[TableSym, LatticeOps] = {
