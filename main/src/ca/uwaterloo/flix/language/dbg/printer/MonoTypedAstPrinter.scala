...
object MonoTypedAstPrinter {
  ...
  def print(e: MonoTypedAst.Expr): DocAst.Expression = e match {
    ...
    case Let(sym, exp1, exp2, tpe, _) => DocAst.Expression.Let(printVarSym(sym), Some(MonoTypePrinter.print(exp1.tpe)), print(exp1), print(exp2))
    case LetRec(varSym, _, _, exp1, exp2, tpe, _) => DocAst.Expression.LetRec(printVarSym(varSym), Some(MonoTypePrinter.print(exp1.tpe)), print(exp1), print(exp2))
