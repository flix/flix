...
object CompilerProfiler {
  /**
    ...
    * @param cns          number of type constraints generated by Typer for this def's body.
    * @param loc          the source location of the def's body, used to compute LOC.
    */
  final case class DefnStats(sym: Symbol.DefnSym, isActive: Boolean, callCount: Int, totalNanos: Long, byPhase: Map[String, Long], byPhaseCount: Map[String, Long], cns: Long, loc: SourceLocation) {
  ...
  /**
    ...
    * @param constraints   total type constraints generated by Typer for this def's body. Updated by the listener path on `FlixEvent.NewConstraintsDef`; accumulated across re-typings (e.g. incremental rebuilds).
    * @param loc           set on first `track` call; carries the def-body span so we can show LOC.
    */
  private final case class Counters(inProgress: AtomicInteger, callCount: AtomicInteger, totalNanos: AtomicLong, perPhaseNanos: ConcurrentHashMap[String, AtomicLong], perPhaseCount: ConcurrentHashMap[String, AtomicLong], constraints: AtomicLong, loc: AtomicReference[SourceLocation])
...
final class CompilerProfiler(phaseProvider: () => Option[String]) extends FlixListener {
  ...
  override def notify(e: FlixEvent): Unit = e match {
    case FlixEvent.NewConstraintsDef(sym, tconstrs) =>
      countersFor(sym).constraints.addAndGet(tconstrs.size.toLong)
    case _ => ()
  }
  ...
  def snapshot(): Vector[CompilerProfiler.DefnStats] = {
    ...
    it.map { e =>
      ...
      CompilerProfiler.DefnStats(key.sym, isActive = c.inProgress.get() > 0,
        c.callCount.get(), c.totalNanos.get(), byPhase, byPhaseCount, c.constraints.get(), loc)
    }.toVector
  }
